...
Motivation: 机器人执行复杂任务时，用户可能因行为偏离预期而困惑，解释其推理过程有助于理解意图。
...
Method: 采用异构架构，包括视觉编码器和语言模型，提供7B和32B两种规模。

Result: 32B模型在空间和时间基准测试中表现领先，支持多种实际AI能力。
...
<details>
  <summary>Details</summary>
Motivation: 传统采样规划计算成本高，而完全监督方法仅模仿轨迹，无法学习是否成功达到目标。
...
<details>
  <summary>Details</summary>
Motivation: 现有VLA模型训练成本高，且输出低层控制信号，限制了其效率和通用性。

Method: 采用基于2D图像的VLM预测轨迹路径点，结合深度图像和推理时解码策略，以及演示条件动作生成。

Result: 模型在仿真数据上训练，表现出强大的仿真到现实迁移能力，并在真实机器人系统上验证了有效性。

Conclusion: 该方法轻量高效，能生成可执行的机器人轨迹，具有广泛的应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 解决GPS和IMU数据融合中的航向不可观测性问题，提供漂移自由的定位和可靠的方向估计。

Method: 基于DRIFT算法，引入对称保持的传感器融合流程，利用InEKF整合GPS全局位置更新，并提出新的航向校正机制。

Result: 在BlueBoat上验证，提供开源解决方案，适用于GPS信号弱的环境。

Conclusion: 为未来实验和比较研究奠定基础，提供了一种有效的传感器融合方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统骨架化算法资源消耗大，且主要面向图像处理任务，而路径规划需要平衡路径安全性、响应时间和长度。

Method: 利用基于U-Net架构的DDAE（SkelUnet）生成骨架化地图，并通过一次性采样（OSS）探索工作空间。

Result: 在12,500张二维地图上训练和测试，并在250张未见地图上评估，SkelUnet显著提高了路径安全性、连通性和处理效率。

Conclusion: SkelUnet方法适用于结构化环境中的移动服务机器人，提供高效、安全的路径规划。
...
Method: 利用智能基础设施摄像头和车载LiDAR传感器提取目标列表，生成真实数据，并通过统一接口重现交通参与者检测。
...
Method: 采用基于共享3D边界框属性的后期协作框架，避免直接访问检测模型。
...
Method: 使用离线计算的Control Invariant Set，在线计算控制动作，支持非凸约束。

Result: 通过大规模用户研究验证，任务负载、信任、感知控制和性能均有提升。

Conclusion: 该框架在不牺牲安全性和用户意图的情况下，显著改善了共享控制的多个方面。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法过于注重鲁棒性，导致运动僵硬且能耗高，需在鲁棒性与柔顺性间取得平衡。

Method: 第一阶段训练速度跟踪策略和自编码器提取特征，第二阶段基于阻抗控制学习柔顺动作模块。

Result: 仿真和实验表明，该方法在鲁棒性、能效和安全性上优于现有RL控制器。

Conclusion: 该方法成功平衡了鲁棒性与柔顺性，显著提升了四足机器人的运动性能。
...
<details>
  <summary>Details</summary>
Motivation: 多机器人多人类环境中的协调问题复杂，需高效策略以优化路径、任务和人机交互。

Method: 采用多目标学习为基础的协调方法，综合处理路径规划、导航、任务调度与分配。

Result: 提出了一种适用于MHMR环境的协调框架，优化多机器人协作效率。

Conclusion: 多目标学习方法在多机器人多人类环境中有效，为复杂协调问题提供解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决欠驱动和柔性机器人系统中的驱动与控制挑战，利用预设计的动态行为实现高效运动。

Method: 采用柔性螺旋形腿，仅用两个电机振动驱动，通过仿真和实验分析驱动参数，提出通用控制参数选择标准。

Result: 实验验证了驱动参数的有效性和鲁棒性，实现了可靠的轨迹跟踪和自主导航能力。

Conclusion: Flix-Walker展示了柔性欠驱动系统在小型机器人运动中的潜力，为未来设计提供了参考。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在关节物体操作中因复杂的运动学约束和有限的物理推理能力而表现不佳。

Method: ArtGS通过多视角RGB-D重建和视觉语言模型提取语义与结构信息，利用动态可微分的3D高斯泼溅优化关节参数。
...
Method: 结合多尺度特征提取与对比特征增强，通过Insight Transformer和Empower Transformer实现高低层特征交互，并利用多尺度对比学习确保特征一致性。

Result: 在模拟和真实环境的桌面清理任务中，MISCGrasp表现优于基线方法。

Conclusion: MISCGrasp通过多尺度特征和对比学习实现了高效的自适应抓取，具有实际应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 机器人独轮车在加速、制动和转向等动作上有独特需求，需要专门的路径规划与控制方法。

Method: 将路径分为直线和曲线段，分别用于加速/制动和转向；优化曲线剖面的曲率，考虑控制性能和车轮打滑限制。

Result: 通过数值模拟验证了所提方法的性能。

Conclusion: 集成路径规划与控制的方法有效满足了机器人独轮车的运动需求。
...
<details>
  <summary>Details</summary>
Motivation: 在灾难响应中，机器人团队的起始部署位置对提升搜索和救援效率至关重要，但现有研究未充分解决这一问题。

Method: 通过引入约束条件到遍历优化框架中，确定异构机器人团队的最优起始位置，并假设起始位置可通过专家知识或航拍图像获取。
...
Method: 采用分层规划，首先生成多个无碰撞路径，然后并行采样和优化，使用多项式轨迹和弧长-偏航参数化处理非完整动力学。
...
<details>
  <summary>Details</summary>
Motivation: 解决多模态策略学习中的模拟难题，尤其是声音等难以模拟的感官模态。

Method: 结合大规模生成模型与物理模拟器，生成逼真的音频以配合模拟视频。

Result: 在真实世界倒水任务中实现零样本迁移，适应新容器和液体。

Conclusion: 生成模型能有效模拟复杂感官模态并缩小多模态模拟与现实的差距。
