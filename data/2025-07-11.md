...
Motivation: 评估g2o与Ceres在SLAM框架Cartographer中的性能差异，以优化扫描匹配效果。

Method: 在Cartographer中对比g2o和Ceres的性能，包括速度、收敛效率和地图精度。

Result: Ceres在速度、收敛效率和地图清晰度上优于g2o，而g2o在局部障碍物检测中表现更佳。

Conclusion: Ceres更适合Cartographer的整体优化，而g2o在特定场景（如局部障碍物检测）中有优势。
...
Method: 采用外部尖端安装设计，通过改变生长方向和支撑管道壁实现三自由度转向，结合尖端传感器和连续体位姿估计实现实时定位。

Result: 机器人实现了51.7度的最大转向角，可导航半径小至2.5厘米的管道，适应急转弯，并在GPS缺失环境中实时定位。

Conclusion: 该设计显著提升了藤蔓机器人在复杂管道和洞穴环境中的导航能力，具有实际应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有的大型视觉语言模型在语言导航方面取得了进展，但缺乏专注于语言理解的基准测试。

Method: 提出了LangNav数据集和LangNavBench基准，并开发了MLFM方法，构建可查询的多层语义地图。

Result: MLFM在LangNav数据集上优于现有基于地图的导航基线方法。

Conclusion: LangNav和LangNavBench为语言导航系统提供了首个全面的语言中心评估，MLFM方法在小物体和空间关系指令中表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 解决“涌现”和“群体”定义模糊的问题，帮助研究者和专家统一理解，支持机器人群体系统的设计与部署。

Method: 提出一个框架，分离可观测与不可观测状态，比较现有定义，强调观察者的视角和隐性知识的作用。

Result: 指出“群体”不仅由群体行为定义，还由行为生成过程定义，概念的主观性源于观察者的感知。

Conclusion: 强调“群体”与多机器人系统的区别，为机器人群体系统的设计和部署提供理论基础。
...
<details>
  <summary>Details</summary>
Motivation: 手部触觉反馈虽能提升手术机器人操作性能，但会遮挡操纵器的直接交互。研究旨在探索手腕触觉反馈的可行性及其效果。

Method: 使用软气动手腕触觉设备，将工具-组织交互力传递到用户手腕，并通过实验比较有无触觉反馈时参与者对幻影组织施加力的准确性。

Result: 实验显示，手腕触觉反馈显著降低了力误差，但增加了操作时间，表明触觉反馈可能改变了速度-准确性权衡点。

Conclusion: 手腕触觉反馈能有效提高力控制准确性，但需进一步研究其对操作效率的影响。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于师生框架的方法在策略蒸馏中常丢失运动多样性，且对未见行为的泛化能力有限。

Method: 提出UniTracker框架，将CVAE整合到学生策略中，利用其显式建模运动多样性，并通过学习先验提升鲁棒性。

Result: 实验表明UniTracker在运动质量、泛化能力和部署鲁棒性上显著优于MLP-based DAgger基线。
...
<details>
  <summary>Details</summary>
Motivation: 软体机器人因其高度非线性和复杂行为，运动学建模存在挑战，现有数据驱动方法存在预测不确定性，影响建模精度。

Method: 研究多种线性和非线性机器学习模型，选择非线性集成方法，并开发保形运动学建模框架，利用保形预测量化不确定性。

Result: 非线性集成方法表现最优，保形预测框架提供了理论保证的预测区间。

Conclusion: 提出的框架能有效量化不确定性，提升软体机器人运动学建模的精度和鲁棒性。
...
Method: 提出PILOC框架，利用局部感知和通信，引入信息素反向引导机制，结合深度强化学习实现间接协调。

Result: 实验表明，PILOC在搜索效率、适应性和系统鲁棒性上显著优于现有方法，尤其在动态和通信受限场景中表现更优。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶车辆的运动规划模块在复杂或基于学习的软件中面临功能安全挑战，现有在线验证方法在嵌入式实时环境中的集成有限。

Method: 提出了一种时间安全保障概念，结合约束可行性检查和基于成本的合理性指标，在实时操作系统中评估轨迹候选。

Result: 初步结果表明，安全保障模块在实时范围内运行，并能有效检测不安全轨迹。

Conclusion: 该研究提供了一个模块化和可扩展的运行时轨迹验证框架，未来工作包括完善安全保障逻辑并通过硬件在环仿真和车辆测试验证其有效性。
...
<details>
  <summary>Details</summary>
Motivation: 解决GPS拒绝室内空间中视觉定位误差累积的问题，通过结合SCR和VIO技术实现高精度绝对姿态估计。

Method: 使用基于证据学习的SCR姿态估计器和后退水平轨迹优化器，优化器引导相机朝向预测可靠的场景坐标的像素，同时固定滞后平滑器融合SCR和IMU数据。
...
Method: 设计了一种基于微型倒置Delta机器人的3D可穿戴触觉设备FiDTouch，能够提供接触、压力、皮肤拉伸和振动反馈等多种刺激。

Result: 通过两阶段用户研究评估了设备在静态空间接触刺激和皮肤拉伸刺激感知方面的性能，结果显示设备能够提供精确的触觉反馈。

Conclusion: FiDTouch通过精确的触觉和力刺激，能够显著提升用户在人机和人类-机器人交互中的沉浸感和效率。
...
<details>
  <summary>Details</summary>
Motivation: 在CDPRs的负载操纵任务中，需要在固定姿态下检测异常（如风或电缆碰撞），但传统方法依赖额外传感器。本文旨在探索仅用电机扭矩数据实现高效异常检测的可能性。

Method: 提出了一种基于GMM的自适应无监督算法，通过校准期拟合正常数据模型，实时评估扭矩信号的马氏距离，并动态更新模型参数以适应环境变化。
...
<details>
  <summary>Details</summary>
Motivation: 通过整合高级辅助算法到外科机器人培训课程中，提升学员的临床表现。

Method: 开发了一种触觉增强的虚拟现实模拟器，包含8个外科任务，通过物理引擎和触觉辅助接口实现学员与任务的交互。

Result: 实验研究表明，引入增强的机器人辅助显著提升了培训过程中的表现，并促进了技能向无辅助临床场景的迁移。

Conclusion: 触觉增强的虚拟现实模拟器是外科机器人培训的有效工具，能够显著提升学员的技能水平。
...
<details>
  <summary>Details</summary>
Motivation: 传统静态传感器网络在结构监测中覆盖不足，机器人群体可提供分布式感知解决方案。

Method: 结合有限元分析和物理模拟，使用高斯过程估计器和模态分析检测损伤。

Result: 模拟实验验证了机器人群体在振动检测中的有效性，分析了探索半径对结果的影响。

Conclusion: 微型机器人群体在结构监测中具有潜力，方法在模拟环境中表现良好。
...
<details>
  <summary>Details</summary>
Motivation: LLMs能显著减轻认知负担，本研究旨在探索其在学习示范（LbD）中分类和分割复杂动作的潜力，特别是在水果采摘任务中。

Method: 采用三种不同的微调方法，基于UR10e机器人采集的数据集进行比较。

Result: 未明确提及具体结果，但旨在验证LLMs在此任务中的适用性。

Conclusion: LLMs有望成为替代传统方法的高效工具，适用于实际场景中的动作分类和分割。
...
<details>
  <summary>Details</summary>
Motivation: 现有SLAM系统在动态光照条件下性能下降，需要一种自适应前端以增强鲁棒性。

Method: 提出图像增强、动态特征提取和特征筛选策略，结合光照影响因子优化特征点选择。
...
<details>
  <summary>Details</summary>
Motivation: 尽管技术和器械有所进步，MASO的成功仍高度依赖外科医生的经验，因此需要一种更精确和安全的手术辅助系统。

Method: 将手术分解为三个子任务：机器人主导的位置和方向控制，以及外科医生主导的力控制；使用光学跟踪系统（OTS）实现患者跟踪，并通过注册和校准方法实现可靠导航。
...
Conclusion: 该方法为四足机器人处理未知动态负载提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 探究感知失真对自主机器人内部世界表示的影响，以及机器人如何在失真条件下学习导航策略。

Method: 使用模拟的两轮机器人，配备距离传感器和指南针，在简单方形环境中随机探索，分析传感器数据。

Result: 尽管感知空间存在失真，但机器人仍能学习到与物理环境相关的结构化表示，实现自主导航。

Conclusion: 研究揭示了感知在自主导航中的作用，为具身认知和最小代理研究提供了新见解。
...
<details>
  <summary>Details</summary>
Motivation: 随着机器人系统融入日常生活，ROS的复杂架构和技术消息系统对普通用户构成障碍，导致维护困难和系统中断。

Method: ROS Help Desk提供用户为中心的调试工具、主动错误检测和多模态数据处理功能。
...
<details>
  <summary>Details</summary>
Motivation: 当前AEBS验证中，区分FP和TP激活依赖于人类主观标注，存在偏见和局限性。需要一种更客观的方法来提升验证的可靠性和可重复性。

Method: 提出基于PDP的规则分类方法，应用于简化AEBS，分析其优势和限制。

Result: 该方法揭示了关键的系统需求和实施限制，结合人类标注可提高分类透明度和一致性。

Conclusion: 该方法为AEBS验证提供了补充现有实践的潜力，未来可进一步优化和扩展应用范围。
