...
Motivation: 传统果树修剪劳动密集且成本高，需要自动化解决方案以提高效率。
...
<details>
  <summary>Details</summary>
Motivation: 多传感器平台的自动外参校准是一个基础问题，需要高效、通用且减少人工干预的解决方案。

Method: 提出了一种广义RWHEC问题的全局最优算法，并推导了新的可识别性准则和全局最优性保证。同时引入了一种基于李代数的局部求解器。

Result: 方法性能优于现有解决方案，并提供了开源实现。

Conclusion: 该算法在效率和全局最优性上表现出色，适用于多传感器校准问题。
...
<details>
  <summary>Details</summary>
Motivation: 四足机器人由于参考运动数据多样性不足，难以实现多样化运动。

Method: 1. 使用CVAE生成多风格动态可行运动序列；2. 采用对抗运动先验算法验证运动数据。

Result: 显著提升了速度跟踪性能和部署稳定性，成功在真实机器人上验证了多种复杂运动。

Conclusion: 该框架能有效生成和执行复杂运动，解决了多样性不足的问题。
...
TL;DR: 提出了一种新型感知代理，结合语音提示工程、大语言模型（LLM）和分割模型（SAM），用于实时手术辅助，提升人机交互的自然性。


<details>
  <summary>Details</summary>
Motivation: 现有AI手术辅助系统缺乏灵活性，依赖任务预训练和固定类别，限制了动态手术环境中的人机交互。

Method: 利用语音集成提示工程LLM、SAM和任意点跟踪基础模型，结合记忆库和两种新机制，实现已知和未知元素的实时分割。

Result: 在公共数据集上定量分析显示性能与手动提示策略相当，定性分析展示了在自定义数据集中分割新元素的灵活性。
...
Method: MTBench基于GPU加速模拟器IsaacGym实现，包含70个任务，并整合了四种基础RL算法和七种先进的MTRL算法。

Result: 实验表明，MTBench能显著加速MTRL方法的评估，同时揭示了大规模并行化与MTRL结合时的独特挑战。

Conclusion: MTBench为MTRL研究提供了一个统一的评估框架，推动了大规模并行化在MTRL中的应用。
...
Result: 通过仿真验证了推进器辅助的窄路径行走和侧向推力恢复的有效性。

Conclusion: 推进器辅助显著提升了机器人在窄路径行走和动态稳定性方面的表现。
...
Method: 将装配步骤分为任务相关的核心操作和连接的遍历操作，核心操作是预定的，而遍历操作可优化。通过分解式运动规划策略整合可行的遍历操作，探索了多种解决方案技术。

Result: 提出的方法生成了高效且无碰撞的多机器人装配程序，优于基于分散式机器人个体运动规划的基线。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于相机的3DGS方法存在计算负载高、在低纹理或光照差环境下失效等问题，而LiDAR虽稳健但引入新挑战，如全局对齐需求和数据稀疏性。

Method: 提出GSFusion系统，采用surfel-to-surfel约束的全局位姿图优化，像素感知的高斯初始化策略，以及有界sigmoid约束。

Result: 在公开和自有数据集上，GSFusion在渲染质量和建图效率上优于现有3DGS SLAM系统。

Conclusion: GSFusion通过多传感器融合和优化策略，显著提升了3DGS的性能和适用性。
...
<details>
  <summary>Details</summary>
Motivation: 当前触须传感器缺乏基于直接接触信息的主动控制，难以准确重建物体轮廓，尤其是在尖锐表面上。

Method: 构建了磁感应触须传感器，利用梯度下降提取接触点位置，并通过贝叶斯滤波减少波动；提出主动运动控制策略，使用B样条曲线预测局部曲率。

Result: 算法能有效跟踪物体并实现亚毫米级精度的轮廓重建，仿真和实物实验验证了方法的有效性。

Conclusion: 该方法通过主动控制和传感器优化，显著提升了触须传感器在物体轮廓重建中的性能。
...
<details>
  <summary>Details</summary>
Motivation: 确保自动驾驶车辆在伦理挑战的日常驾驶场景中做出符合人类期望的决策，解决现有规划系统依赖刚性规则导致行为与人类期望不一致的问题。

Method: 提出了一种基于原因的轨迹评估框架，量化人类代理的原因（如法规遵从性、舒适性），并通过可调整的权重和平衡函数评估候选轨迹。

Result: 通过真实世界启发的超车场景展示了框架如何揭示法规遵从性、效率和舒适性之间的冲突，支持可解释的决策评估。
...
<details>
  <summary>Details</summary>
Motivation: 解决强化学习方法在仿真与现实中表现差异大的问题，特别是在车辆漂移控制领域。

Method: 采用GPU加速的并行仿真和系统化领域随机化技术，开发了一种强化学习框架。

Result: 在多种场景下（如稳态圆形漂移、方向转换和变曲率路径跟踪）实现了精确轨迹跟踪，并在仿真和实际环境中均表现良好。

Conclusion: 该方法成功缩小了仿真与现实的差距，为车辆漂移控制提供了一种有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 农业环境中自主移动机器人的需求增长，需要高效、低土壤损伤的路径规划方法。

Method: 结合DTSP（全局路径规划）和NMPC（局部路径控制），生成最短且曲率受限的路径。

Result: 仿真验证显示，该方法路径更平滑、更短（减少约16%），且NMPC能精确导航至目标点。
...
<details>
  <summary>Details</summary>
Motivation: 传统PID控制器在仿真到现实转移中表现不佳，尤其是在高齿轮比、非反向驱动执行器的情况下。

Method: 提出了一种物理引导的增益正则化方案，通过简单实验测量机器人的有效比例增益，并在训练中惩罚神经控制器输入输出敏感度与这些值的偏差。同时，控制器根据当前设备参数进行条件化。

Result: 在硬件实验中，增益正则化、参数条件化的RNN控制器实现了与仿真接近的角度稳定时间，而纯域随机化策略则表现出持续振荡和显著的仿真到现实差距。

Conclusion: 该方法为在低成本机器人硬件上缩小仿真到现实差距提供了一种轻量级、可复现的框架。
...
<details>
  <summary>Details</summary>
Motivation: 机器人模仿学习面临大规模高质量演示数据稀缺的问题，跨机器人形态的统一训练存在挑战。

Method: 提出H-RDT，基于扩散变换器架构，分两阶段训练：预训练人类操作数据，再通过模块化编码器/解码器微调机器人数据。

Result: H-RDT在仿真和现实实验中分别比从头训练提升13.9%和40.5%，优于现有方法如Pi0和RDT。
...
<details>
  <summary>Details</summary>
Motivation: 解决自动驾驶在复杂环境中适应性、鲁棒性和可解释性不足的问题。

Method: 集成多传感器（摄像头、LiDAR、雷达）与GPT-4.1增强的视觉-语言-动作（VLA）架构。
...
<details>
  <summary>Details</summary>
Motivation: 随着社交机器人需求的增长，需要根据用户状态调整行为，而准确评估用户体验（UX）是实现这一目标的关键。现有方法多关注单一维度，缺乏对多维度UX的综合评估。
...
Method: 通过直接使用Planning Domain Definition Language文件和问题文件，对多种语言模型进行系统评估，并与Fast Downward规划器进行比较。
...
<details>
  <summary>Details</summary>
Motivation: 解决手部外骨骼因用户解剖学差异和穿戴不一致导致的跟踪性能下降问题，提升精准任务中的适用性。

Method: 使用冗余关节传感和残差加权优化策略估计虚拟链接参数，并通过运动捕捉数据驱动调整成本函数权重。

Result: 在七名受试者中，相比未校准和均匀加权模型，显著降低了关节和指尖跟踪误差。
...
<details>
  <summary>Details</summary>
Motivation: 解决水下机器人团队在缺乏先验几何信息时的高效闭环检测问题。
...
Conclusion: BGS-SLAM填补了户外3DGS-SLAM的空白，展示了在仅使用RGB立体对时的优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 探索如何将潜在动作（视觉变化的抽象表示）更好地融入视觉-语言-动作模型，以提升机器人操作策略的泛化性。

Method: 提出ViLLA-X框架，改进潜在动作的学习和整合方式，用于视觉-语言-潜在动作模型的预训练。

Result: 在SIMPLER、LIBERO等模拟环境和真实机器人任务（夹爪和灵巧手操作）中表现优异。

Conclusion: ViLLA-X为未来研究提供了坚实基础，展示了视觉-语言-潜在动作模型的潜力。
...
Result: RL显著提升交互成功率4倍，实现零样本泛化，适用于多样环境。

Conclusion: 3D模拟环境中的RL训练对视觉运动代理的空间推理能力有巨大潜力。
...
<details>
  <summary>Details</summary>
Motivation: 蟑螂通过触角提取丰富触觉信息的能力启发了研究人员，但现有传感器技术难以在微型机器人上实现类似功能。

Method: 设计了一种多段式、柔顺层压结构的传感器，嵌入电容式角度传感器，实现轻量化、低功耗和紧凑尺寸。

Result: CITRAS在静态和动态弯曲时角度测量误差极小，并能准确预测距离、估算间隙宽度和区分表面纹理。
...
Result: 在模拟和实际水下任务中表现出优异的适应性和可靠性，优于传统方法。

Conclusion: UROSA不仅推动了水下自主性发展，还为通用认知机器人框架提供了可扩展、安全的解决方案。
