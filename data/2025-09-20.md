...
Method: 使用基于LLM的上下文感知自适应操纵器，通过提示注入和响应破坏等攻击方式，在成功轨迹中系统性地注入可控和可追踪的错误。

Result: 在监督微调、强化学习和对比学习三种范式下，使用AEGIS数据训练的模型都取得了显著性能提升，部分模型性能甚至优于大一个数量级的专有系统。

Conclusion: AEGIS框架为开发更鲁棒和可解释的多智能体系统提供了关键的数据资源，自动化错误生成方法具有重要价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有端到端自动驾驶框架通常依赖隐式学习的BEV特征，缺乏对风险和引导先验的显式建模，无法实现安全可解释的规划。需要同时考虑几何障碍物的硬约束和无明确几何形状的软语义约束。

Method: 提出FlowDrive框架：1）引入物理可解释的能量流场（风险势场和车道吸引力场）编码语义先验和安全提示到BEV空间；2）使用特征级门控的条件扩散规划器，将运动意图预测与轨迹去噪解耦；3）通过流感知特征自适应优化锚定轨迹。

Result: 在NAVSIM v2基准测试中达到最先进性能，EPDMS得分为86.3，在安全性和规划质量方面均超越先前基线方法。

Conclusion: FlowDrive通过显式建模物理可解释的流场，实现了更安全、可解释的自动驾驶运动规划，同时条件扩散规划器的设计有效缓解了任务干扰并增强了多模态多样性。
...
<details>
  <summary>Details</summary>
Motivation: 解决机械独立的机器人仅通过接触力进行协调而无通信或集中控制的挑战性场景，相比依赖刚性机械连接的先前工作

Method: 采用分层策略架构分离基座运动与手臂控制，提出星座奖励公式统一位置和方向跟踪以强制刚性接触行为，通过奖励设计和训练课程鼓励机器人表现得像刚性连接到物体

Result: 在2-10个机器人的不同物体几何形状和质量上展示了鲁棒的搬运性能，并实现了轻量级物体的仿真到现实迁移

Conclusion: 该方法通过共享策略参数和隐式同步线索实现协调，可扩展到任意团队规模而无需重新训练，为无通信的分布式协同操作提供了有效解决方案
...
Method: 开发LeVR模块化软件框架，提供VR遥操作功能，并与LeRobot模仿学习框架集成。发布LeFranX开源实现，支持Franka FER机械臂和RobotEra XHand灵巧手。

Result: 成功收集了100个专家演示的公开数据集，并用其微调了最先进的视觉运动策略，验证了系统的有效性。

Conclusion: LeVR框架有效解决了机器人模仿学习中的数据收集挑战，提供了完整的端到端解决方案，开源框架和数据集将加速机器人社区的模仿学习研究。
...
<details>
  <summary>Details</summary>
Motivation: 解决人形机器人同时控制上下半身并与物体交互的复杂任务，传统RL方法难以发现自然的人类运动模式

Method: 使用人类运动数据训练扩散先验模型，然后在仿真中用该先验指导RL策略学习特定任务

Result: 该方法能够发现直接RL无法获得的解决方案，产生自然的人类运动模式，有助于仿真到现实的迁移

Conclusion: DreamControl在Unitree G1机器人上成功验证了多种复杂任务的有效性，证明了人类运动先验对RL指导的重要价值
...
Method: 使用LLM的规划能力自动分解长时程协调任务为子任务序列，用LLM生成每个子任务的奖励函数，并通过VLM指导的奖励精炼循环进行训练和优化

Result: 在多四足机器人导航和双手操作任务上验证了学习复杂协调行为的能力，并在真实硬件实验中验证了多四足机器人导航策略

Conclusion: CRAFT框架成功利用基础模型的推理能力，为多机器人协调任务提供自主教练功能，能够有效学习复杂的协调行为
...
<details>
  <summary>Details</summary>
Motivation: 机器人部署中视觉分支面临对抗性和自然损坏，现有防御方法通常只在CLIP风格编码器内对齐干净和对抗特征，忽视了更广泛的跨模态对应性，导致增益有限且常损害零样本迁移。

Method: 两阶段框架：第一阶段对干净-对抗对进行无监督微调以强化视觉编码器；第二阶段利用跨模态对应性，最小化干净/对抗特征与文本锚点之间的差异，同时强制跨模态的类间分布对齐。
...
Method: 结合轻量级视觉感知和大型语言模型：将手势姿势转换为结构化文本描述，LLM推断意图并生成机器人特定命令，实时匹配最适合的机器人执行任务
...
<details>
  <summary>Details</summary>
Motivation: 解决基于距离的控制障碍函数(CBF)激活反应急剧、计算成本高的问题，并为实时导航提供主动性的安全避免能力

Method: 将3D高斯拖尾转换为闭式碰撞锥，构建一阶控制障碍函数(CBF)，嵌入二次规划(QP)中，利用Minkowski和扩展处理机器人物理尺寸

Result: 在包170k拖尾的大场景中，计划时间减少3倍，轨迹急动度显著降低，同时保持同等安全水平

Conclusion: 该方法提供了一种分析解、计算高效的主动性安全过滤方案，适用于空间机器人和卫星系统等极端环境的实时导航
...
<details>
  <summary>Details</summary>
Motivation: 解决多智能体强化学习在竞争性环境中同时适应导致训练不稳定、非动能对策在不利条件下失效，以及策略在不同智能体数量环境中泛化能力差的问题。

Method: 提出Local-Canonicalization Equivariant Graph Neural Networks (LEGO)框架，集成图神经网络捕捉排列等变性和泛化能力，通过规范化实现E(n)等变性，使用异构表示编码角色特定的归纳偏置。

Result: 在合作和竞争性群体基准测试中，LEGO优于强基线并提高了泛化能力。在真实世界实验中，LEGO展现出对不同团队规模和智能体故障的鲁棒性。

Conclusion: LEGO框架有效解决了多智能体强化学习中的关键挑战，提供了更好的训练稳定性、泛化能力和鲁棒性，可与主流MARL算法如MAPPO无缝集成。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中监督控制往往不是连续的，而是在已知时间间隔内进行间歇性监控。智能体需要在这种环境下学习如何既实现自身目标，又能在被观察时表现出合规行为。

Method: 利用心智理论建模监督者的期望，提取一个标量值——预期偏差证据，该标量结合了参考策略与当前动作分布的差异程度以及智能体对即将发生观察的信念。将该标量作为状态依赖权重注入到在线强化学习的KL正则化策略改进步骤中。

Result: 在海洋(ASV)和空中(UAV)导航的真实硬件实验中，ToM引导的强化学习方法能够在线运行，实现高回报和成功率，同时观察轨迹证据与监督者期望相校准。

Conclusion: 心智理论可以重新用于引导在线强化学习实现欺骗性行为，通过单一校准标量平滑权衡自利与合规，避免了手工或启发式策略的设计。
...
<details>
  <summary>Details</summary>
Motivation: 解决从视视数据自动发现抽象层次的核心挑战，避免人工编程抽象的工作量和可扩展性限制

Method: 结合重新排列问题的内在二分图结构约束，通过注意力导向的视视距离来引导离散图结构抽象的形成

Result: 在两个模拟重新排列任务中，方法能一致地识别有意义的抽象，支持有效规划且效果超过现有方法

Conclusion: 该统一框架能够从单纯视觉数据中自主发现有用抽象，为机器人规划提供可扩展的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 随着软体机器人手和触觉传感技术的发展，需要更先进的机器学习方法来从高分辨率触觉图像中提取有用信息，以增强软体机器人对环境的理解和交互能力。

Method: 使用GelSight Baby Fin Ray触觉传感器，集成摄像头和软性Fin Ray结构，采用ResNet50、GoogLeNet以及3层和5层CNN结构进行纹理识别、力估计和位置估计的消融研究。

Result: 研究表明机器学习能够有效从触觉图像中提取丰富的接触信息，包括力、物体几何形状和纹理等，不同神经网络结构在性能上有所差异。

Conclusion: 机器学习是从高分辨率触觉图像中提取有用信息的有前景技术，能够赋能软体机器人更好地理解和与环境交互。
...
<details>
  <summary>Details</summary>
Motivation: 事件相机定位研究快速发展，但代码依赖和数据格式多样化使得方法比较和实施变得困难且不可靠。

Method: 使用Pixi包和依赖管理器实现Event-LAB框架，支持单命令行安装和调用多种定位方法与数据集的组合，实现了VPR和SLAM两种常见定位流程。

Result: 框架能够系统可视化分析多种方法和数据集的结果，发现事件收集计数和帧生成窗口大小等参数对性能有显著影响。
...
<details>
  <summary>Details</summary>
Motivation: 草莓自然生长在聚集中，常被叶子和枝条遮挡，给传统机器人采摘系统带来巨大挑战，需要灵活的操作把握周围软物体并精确到达采摘点

Method: 采用4-DoF SCARA机械臂搭配人类远程操控界面收集数据，利用终点位置辅助的动作块化Transformer(ACT)算法开发细粒度视觉运动采摘策略

Result: 在各种遮挡场景下的实验表明，改进方法显著超过了直接实施ACT算法的性能

Conclusion: 该系统在遮挡草莓采摘方面具有实际应用潜力
...
<details>
  <summary>Details</summary>
Motivation: 当前缺乏捕捉人们通过语言指导随时间获取具身技能的数据集，特别是在语言和物理动作深度交织的领域。

Method: 收集29名人类在赛车模拟器中约90分钟的驾驶数据，其中15人接受专业驾驶教练的一对一指导，14人无指导。数据集包含车辆状态、地图信息、语音指导等同步数据，并标注了教练反馈类别、学生遵从度等。

Result: 数据集包含超过20,000条实时反馈话语、400多条最终反馈话语和40多小时的车辆驾驶数据，可用于研究运动学习动态、语言现象和教学计算模型训练。

Conclusion: 该自然主义数据集为研究运动学习、语言现象和计算教学模型提供了宝贵资源，展示了在上下文学习、模仿学习和主题建模等方面的应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 解决重构化机器人在解体约束结构时的配置和协调问题，避免大规模搜索空间中的局部最优解

Method: 集成多台机器臂和旋转台，采用层次优化方法：多目标遗传算法进行序列和任务规划，约束编程进行排程，以及针对约束结构的某初始化方法

Result: 模拟结果证明该方法能够在现实时间内有效解决复杂的重构化机器人解体问题

Conclusion: 该系统集成方法为重构化机器人自动无破坏解体提供了有效的解决方案，能够处理复杂结构并避免局部最优解
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉-语言-动作策略在新领域机器人配置上的统一性能力有限，需要花费高成本的适应过程。这是因为大多数方法重视模型规模和数据集规模，而忽视了动作空间的设计。

Method: 提出了一个框架：(i)建立体现配置对称性理论用于动作空间和策略设计；(ii)引入能够强制配置对称性的动作解码器；(iii)添加几何感知网络架构来增强体现无关的空间推理能力。

Result: 在模拟和真实环境中进行了大量实验，证明该方法能够提高预训练效果，并且在新领域机器人配置上实现高效细调。

Conclusion: 通过重新设计动作空间以实现体现配置对称性，可以有效解决视觉-语言-动作策略的配置统一性问题，为跨体现预训练提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的Bird's-Eye-View方法在单目视觉里程计中存在监督信号稀疏和透视到BEV投影过程中的信息丢失问题，需要一种无需额外标注的改进方案。

Method: 提出密集BEV光流监督（从3-DoF位姿真值构建）和PV-BEV融合（在投影前计算相关体积以保留6-DoF运动线索），采用三级监督机制和增强的旋转采样策略。

Result: 在KITTI、NCLT、Oxford和新收集的ZJH-VO多尺度数据集上达到最先进性能，相对平移误差比之前的BEV方法提高了40%。

Conclusion: BEV-ODOM2有效解决了BEV方法的监督稀疏和信息丢失问题，无需额外标注即可显著提升性能，新发布的ZJH-VO数据集将促进未来研究。
...
<details>
  <summary>Details</summary>
Motivation: 密集3D卷积虽准确但计算费用高，现有三平面方法依赖插值、点查询和隐式MLP，计算重且不适合嵌入式3D推理。

Method: 直接将3D像素投影到平面特征，通过广播和求和重建特征体积，添加低分辨率体积分支捕捉全局上下文，通过轻量融合层结合。

Result: 分类和补全任务保持或提高准确度，分割和检测任务在准确度上有轻微下降但获得显著计算节省，NVIDIA Jetson Orin nano上实现稳健实时速率。

Conclusion: 该方法在保持高效能的同时提供了可见的准确度，适合嵌入式机器人知觉系统的实时推理。
...
Method: 构建了高效低成本的数据收集、模型训练和推理系统；引入专门的VLA基准测试；集成生成模型和3D高斯泼溅技术重建真实环境和机器人模型；实现零样本Sim2Real迁移。
...
<details>
  <summary>Details</summary>
Motivation: 触觉机器人学习面临数据收集困难、数据稀缺稀疏、缺乏力反馈等关键挑战，需要新的硬件和算法解决方案

Method: 开发exUMI可扩展数据收集设备（增强版UMI，含AR动捕、旋转编码器、模块化视觉触觉传感和自动校准），提出TPP触觉预测预训练框架，通过动作感知时序触觉预测进行表示学习

Result: 实现100%数据可用性，收集超过100万触觉帧，真实世界实验显示TPP优于传统触觉模仿学习方法
...
<details>
  <summary>Details</summary>
Motivation: 重新研究过去提出的三环空间连杆组织，以明确其自由度特性和连杆组织的性质，特别是在参考配置下的微分自由度与有限自由度的差异问题。

Method: 进行局部分析，计算运动学切空间向量凸，并对组态空间进行局部近似。通过高阶局部分析确定连杆组织的性质。

Result: 证明该连杆组织的组态空间局部是光滑流形，参考配置不是奇异点。微分自由度局部恒定，说明该连杆组织是震动的（shaky）。

Conclusion: 这种三环空间连杆组织展现出过约束连杆组织的典型特征：有限自由度为3度，但微分自由度较高（为5度），且具有震动性质。这为连杆组织的微分形态学分析提供了重要的理论基础。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于模型的控制器在自主无人机竞速中性能受限于对预计算参考轨迹的依赖，而强化学习研究表明许多控制器优化的是替代目标而非直接最大化通过门进度的主要竞速目标。

Method: 将门进度目标（源自RL奖励塑造）直接整合到模型预测路径积分（MPPI）框架中，利用MPPI的采样特性实时优化不连续且不可微的目标函数。建立了统一框架系统比较三种目标函数。

Result: 提出的无参考方法实现了有竞争力的竞速性能，媲美甚至超越了基于参考的方法。

Conclusion: 参考自由方法通过直接优化门进度目标，在无人机竞速中表现出色，MPPI的采样特性使其能够有效处理不连续目标函数。
...
<details>
  <summary>Details</summary>
Motivation: 在人机共享工作空间中，有效的通信对安全和效率至关重要。研究者想要了解非语言交流（包括光信号和情感显示）如何影响人机互动的效果。

Method: 使用Franka Emika Panda机器人，在其末端执行器上安装LED带，并通过平板显示动态面部表情。进行了包括18名参与者的人机协作实验，测试三种条件：单独LED信号、LED信号+反应式情感显示、LED信号+预先情感显示。通过问卷和位置跟踪收集数据。

Result: 结果显示情感显示虽然提高了机器人的互动性感知，但与单独使用LED信号相比，并没有显著改善碰撞预期、通信清晰度或任务效率。

Conclusion: 情感细微表情虽然可以增强用户参与感，但在共享工作空间的任务性能方面效果有限，LED信号已经能够提供足够的通信效果。
...
<details>
  <summary>Details</summary>
Motivation: 确保基于视觉的控制系统安全性是一个主要挑战，安全过滤器在传统控制系统中已证明有效，但在视觉控制领域的应用还很有限。预训练视觉模型已被证明是有效的感知骨干网络，本文旨在研究其在视觉安全过滤器设计中的有效性。

Method: 使用预训练视觉模型作为分类器骨干来定义故障集，用于基于Hamilton-Jacobi可达性的安全过滤器，以及作为潜在世界模型的骨干。比较了从头训练、微调和冻结预训练模型三种策略的权衡。

Result: 评估了不同预训练视觉模型在所有任务中的表现优劣，比较了学习的世界模型和Q函数在切换到安全策略决策方面的效果，并讨论了在资源受限设备上部署这些模型的实践考虑。

Conclusion: 预训练视觉模型可以作为有效的视觉安全过滤器骨干，但需要根据具体任务选择合适的训练策略和模型架构，同时考虑实际部署时的资源约束。
...
Method: 提出多阶段探索操作框架：1)近场感知扫描构建局部碰撞地图；2)多目标效用函数选择信息丰富且利于后续操作的视点；3)约束操作优化策略生成尊重障碍物约束的操作位姿

Result: 在仿真中比仅考虑信息增益的探索方法提高24.25%的操作成功率，真实世界实验验证了在受限环境中主动感知和操作的能力

Conclusion: COMPASS框架通过集成感知、探索和操作优化，有效解决了受限杂乱环境中的操作挑战，为这类复杂环境提供了系统性的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 传统强化学习控制器将多个任务目标聚合为单一标量奖励，存在奖励调优困难、容易陷入局部最优的问题，且随着目标数量增加，调优成本和次优性都会增长。多目标方法由于计算成本和优化难度在机器人RL中应用较少。

Method: 提出GCR-PPO方法，修改actor-critic优化过程：使用多头critic将actor更新分解为各目标的梯度，并根据目标优先级解决梯度冲突。在IsaacLab操作和运动基准测试以及相关多目标修改任务上进行评估。

Result: 相比并行PPO方法表现出更好的可扩展性(p=0.04)，且没有显著计算开销。在高冲突任务中性能更高，平均改进9.5%，高冲突任务改进更大。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有视觉-语言-动作(VLA)系统的关键局限性，包括领域过拟合、不可解释的推理过程以及辅助生成模型的高延迟问题，使VLA从黑盒控制器转变为真正能与人类协作的智能助手。
...
Conclusion: 这项工作开创性地将VLA从黑盒控制器转变为能够推理、行动并与人类协作的真正辅助智能体，为构建协作式人工智能系统迈出了重要一步。
...
<details>
  <summary>Details</summary>
Motivation: 解决水平安装LiDAR（如MID360）在近地面捕获点云稀少的问题，避免传统方案（静态倾斜、主动旋转或高密度传感器）牺牲水平感知或增加硬件成本

Method: 建模内部差速驱动与传感器姿态的耦合关系，在名义目标跟踪命令上叠加有界非周期性振荡，实现被动LiDAR激励

Result: 在实验室、走廊和战术环境中验证，达到96%地图完整性，轨迹跟踪误差减少27%，实现鲁棒的近地面人体检测

Conclusion: PERAL在重量、功耗和成本方面均优于静态倾斜、主动旋转和固定水平基线方案，设计代码将在接受后开源
...
<details>
  <summary>Details</summary>
Motivation: 传统机器人软件框架在大规模视觉-语言-动作模型训练中成为瓶颈，仿真环境对现实实验的支持有限，需要新的轻量级生态系统来支持机器人学习研究。

Method: 设计了模块化、可扩展的分层架构RCS，提供统一的仿真和物理机器人接口，支持大规模仿真训练和真实世界实验。

Result: RCS框架在VLA和RL策略开发周期中表现出良好的可用性和性能，评估了Octo、OpenVLA和Pi Zero等多个模型在不同机器人上的表现，并展示了仿真数据如何提升真实世界策略性能。

Conclusion: RCS是一个有效的轻量级机器人控制框架，能够支持大规模通用策略的研究和开发，促进仿真到现实的迁移学习。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个系统化的方法来协同优化喷气动力人形机器人的机械设计和控制参数，确保其能够有效执行受动态约束的飞行轨迹

Method: 使用实验设计方法生成5000个几何变体，通过CAD装配确保结构有效性，采用K-means聚类减少计算成本，使用最小加加速度轨迹评估性能，基于动量的线性化MPC控制策略，NSGA-II多目标优化算法

Result: 框架输出一组飞行就绪的人形机器人配置和验证的控制参数，提供了选择和实施可行空中人形设计的结构化方法

Conclusion: 该协同设计框架为喷气动力人形机器人的设计和控制参数优化提供了有效的系统化方法，能够同时最小化轨迹跟踪误差和机械能耗
...
Method: 使用LTL理解任务语义，affordance学习识别最佳交互点，扩散策略泛化交互，基于交互数据的优化方法精炼动作

Result: DART在操作能力、泛化性能、迁移推理和鲁棒性方面优于大多数现有方法

Conclusion: DART框架通过结合语义理解、affordance学习和扩散策略，有效提升了铰接物体操作的效率和跨类别泛化能力
...
<details>
  <summary>Details</summary>
Motivation: 解决多机器人在大型未知环境中协调覆盖的挑战，需要最小化总覆盖路径长度并减少机器人间冲突。现有方法在路径重叠和协调效率方面存在不足。

Method: 采用分层覆盖路径规划算法，构建并动态维护表示环境连接子区域的邻接图。将子区域分配问题建模为车辆路径问题(VRP)，为每个机器人计算不相交的巡回路径，机器人在子区域内独立执行覆盖策略。

Result: 通过仿真和多机器人硬件实验证明，Multi-CAP在覆盖时间、总路径长度和路径重叠率等关键指标上显著优于最先进方法。消融研究验证了连接感知图和全局路径规划器的重要性。

Conclusion: Multi-CAP算法通过连接感知的层次化规划方法，有效解决了多机器人覆盖路径规划问题，在性能和效率方面都有显著提升，为大规模未知环境的多机器人协同覆盖提供了有效解决方案。
...
Method: 使用共享扩展现实环境，让人类操作员直接与机器人3D场景图交互，添加高层语义概念；提出基于图的语义融合方法，将人类干预与机器人感知集成

Result: 在真实建筑工地数据集上的实验表明，相比自动化基线，房间检测准确率、地图精度和语义完整性均有提升

Conclusion: 该方法有效展示了人机协作在语义SLAM中的潜力，为未来扩展提供了基础
...
<details>
  <summary>Details</summary>
Motivation: 受人类探索策略启发，开发用于机器人纹理分类的神经形态触觉感知系统，旨在提高机器人与环境的交互能力

Method: 使用NeuroTac传感器采集神经形态触觉数据，测试六种探索动作（滑动、旋转、敲击及其组合），在固定环境和变化条件下评估纹理分类性能

Result: 滑动+旋转组合动作在变化深度和速度条件下达到87.33%的最高分类准确率，功耗仅为8.04mW
...
<details>
  <summary>Details</summary>
Motivation: 手术中人机协作受到口头沟通固有歧义性的影响，需要开发能够准确理解和消歧外科医生指令的机器人系统，以确保手术安全和效率。

Method: 采用基于可供性的两级推理过程：首先使用多模态视觉语言模型分析手术场景，然后基于工具能力知识库进行指令推理；使用双集符合预测方法提供统计严谨的置信度度量。

Result: 在胆囊切除术视频的歧义性手术请求数据集上评估，实现了60%的通用消歧率，证明了系统在手术室中实现更安全人机交互的有效性。

Conclusion: 该框架通过视觉上下文消歧和统计置信度评估，显著提高了手术机器人对口头指令的理解准确性，为手术室中更安全可靠的人机协作提供了有效解决方案。
...
Method: 提出感知感知MPPI（PA-MPPI）方法，当目标被遮挡时，通过感知成本函数偏置轨迹以感知未知区域，扩展可通行空间映射，增加找到替代路径的可能性。系统以50Hz频率运行，配备高效的感知和建图模块。

Result: 硬件实验表明，PA-MPPI在具有挑战性的环境中比基线方法性能提升高达100%，在现有最先进MPPI失败的情况下仍能成功导航。同时证明PA-MPPI可作为导航基础模型的安全鲁棒动作策略。

Conclusion: PA-MPPI有效解决了四旋翼在未知环境中的导航问题，通过感知感知机制实现了对未知区域的主动探索，显著提升了导航性能，并为导航基础模型提供了可靠的动作执行策略。
...
<details>
  <summary>Details</summary>
Motivation: 现有的单视图方法在非结构化环境中因视野有限、探索和泛化能力不足而失败，传统控制器在奇点附近效率和可操作性差

Method: 提出混合框架：多视图扩散策略生成世界坐标系中的末端执行器目标，ReM-QP控制器执行目标（消除松弛变量提高计算效率，加入可操作性感知偏好增强奇点附近鲁棒性）

Result: 在仿真和真实环境实验中，相比基线方法成功率高7-56%，碰撞减少3-31%，展现出平滑全身协调和强泛化能力

Conclusion: M4Diffuser为在非结构化环境中实现可靠的移动操作铺平了道路
...
TL;DR: 本研究探讨了人手不同区域（指尖、手指其他部位和手掌）的触觉反馈对物体重定向任务的影响，分析了不同触觉传感配置对深度强化学习控制策略鲁棒性的作用，并识别出能提高操作效率和准确性的最优传感器布局。


<details>
  <summary>Details</summary>
Motivation: 虽然指尖是常见的传感器放置位置，但手部其他区域的触觉信息贡献常被忽视。需要研究不同手部区域的触觉反馈如何影响灵巧操作任务的性能，为仿人末端执行器的设计提供指导。

Method: 通过分析不同手部区域（指尖、手指其他部位和手掌）的触觉反馈，使用深度强化学习控制策略进行物体重定向任务实验，研究传感器布局与物体特性之间的关系。

Result: 识别出能显著提高操作效率和准确性的最优触觉传感配置，发现不同手部区域的触觉反馈对控制策略的鲁棒性有重要影响。

Conclusion: 研究结果为具有增强操作能力的仿人末端执行器的设计和使用提供了宝贵见解，强调了全面考虑手部各区域触觉传感的重要性。
...
Method: 提出ExT统一框架，包括大规模演示数据收集、预训练和多任务策略微调（SFT或RLFT），支持从专家混合数据中学习并适应新任务和操作条件。

Result: 仿真和真实实验显示，ExT策略能执行厘米级精度的完整挖掘循环，性能媲美专用单任务控制器，并能快速适应新任务和分布外条件。

Conclusion: ExT展示了作为可扩展和通用自主挖掘基础框架的潜力，为重型工程机械的自主化提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 解决大规模动态环境中可靠、无漂移的全局定位挑战，这对于自动驾驶导航至关重要。

Method: 使用高效的语义体素地图表示和改进的扫描匹配算法，利用全局语义信息减少长期轨迹漂移；通过紧密耦合的多传感器融合迭代误差状态卡尔曼滤波器(iESKF)融合LiDAR、IMU和轮式里程计数据；引入3D自适应缩放策略灵活调整轮式里程计测量权重。

Result: 在100万平方米自动化港口进行了大规模真实世界实验，包含35辆智能导引车的3575小时运行数据，结果表明系统在大规模动态环境中优于最先进的LiDAR定位方法。
...
<details>
  <summary>Details</summary>
Motivation: 解决复杂任务先后关系、机器协同作业和动态环境下的任务分配挑战

Method: 使用任务图模型和奖励函数，基于网络流算法进行近优分配，设计了可重分配的在线算法

Result: 在随机任务和实际模拟环境中证明了方案的有效性，性能超过混合整数解法器和贪心算法

Conclusion: 该方法能够有效模型复杂任务关系，生成高保真度任务计划，具有强壁士性
...
Method: Ask-to-Clarify框架包含协作VLM和动作扩散两个组件，通过连接模块生成扩散条件。采用两阶段知识隔离训练策略：先微调协作组件处理模糊性，然后集成动作组件并冻结协作组件。推理时使用信号检测器作为路由器切换提问和行动模式。
...
TL;DR: 这篇论文提出了一种能量约束的轨迹规划框架，为行星探测户外车在核电源和太阳能混合加供能的情况下提供能量可行的轨迹规划。


<details>
  <summary>Details</summary>
Motivation: 未来行星探测户外车需要在核电源和太阳能混合加供能的情况下长时间运行，而现有的规划方法通常忽视了功率流模型和瞬时功率约束。

Method: 设计了一种能量约束的轨迹规划框架，将物理基础的平移、旋转和阻力功率模型与基线子系统负载相结合，通过SE(2)基础的多项式轨迹优化来同时满足累积能量预算和瞬时功率约束。

Result: 在月球类似地形上的模拟结果显示，该规划器生成的轨迹峰值功率仅超出指定限制的0.55%，而现有方法则超出17%以上。

Conclusion: 该方法为长时间行星任务提供了一种理论坚实且实用的能量意识自主规划方法，能够生成同时满足平滑性、动力学可行性和功率符合性的轨迹。
...
TL;DR: 提出了基于扩散模型的AnoF-Diff方法，用于从时间序列数据中提取力-力矩特征并检测异常，在强力工具使用任务中表现出更好的性能和噪声鲁棒性。
...
Method: 基于扩散模型提取力-力矩特征，并提出并行异常评分评估方法，支持在线异常检测。

Result: 在四个强力工具使用任务上，相比其他最先进方法，在F1分数和AUROC指标上表现更好，对噪声数据集更具鲁棒性。

Conclusion: AnoF-Diff方法能够有效处理强力工具使用任务中的噪声时间序列数据，提供可靠的异常检测性能，并支持在线应用。
...
TL;DR: 这篇论文开发了一种统一的模型框架，用于模拟软体生长机器人的增长、弯曲、驱动咄障碍物接触，并用于设计优化任务。


<details>
  <summary>Details</summary>
Motivation: 虽然软体生长机器人在非结构化环境中表现出良好的交互能力，但需要主动转向控制来应对更复杂的环境。

Method: 扩展梁矩次模型以包含驱动对增长过程中动力学的影响，并开发了快速并行模拟框架，通过实际机器人实验验证模型。

Result: 将模型应用于设计优化任务，找到了能够利用环境接触最小化驱动器数量的设计方案，并验证了设计在环境咄制造不确定性下的稳健性。

Conclusion: 最终制造了优化设计并在障碍物丰富的环境中成功部署，证明了该框架在实际应用中的有效性。
