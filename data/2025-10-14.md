...
Motivation: 现有扩散策略(DP)和动作分块变换器(ACT)方法缺乏时序上下文，导致局部最优陷阱和过多重复动作，需要改进时序任务处理能力。

Method: 集成无分类器引导(CFG)与条件/无条件模型，利用时间步输入跟踪任务进度，动态调整动作预测，通过引导因子平衡时序一致性和动作准确性。

Result: 在人形机器人上的真实实验显示高成功率、最少重复动作，模型能有效终止动作，不同组件和参数调整影响性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉触觉传感器形状尺寸各异，导致大多数模型受限于特定传感器设计，难以开发通用触觉表示。

Method: 1. Touch2Touch：端到端方法，利用配对数据进行跨传感器图像生成；2. T2D2：构建中间深度表示，无需配对数据，实现触觉到深度再到触觉的转换。

Result: 两种方法都能通过跨传感器触觉生成过程，使特定传感器模型能够在多个传感器上使用，并在手内姿态估计和行为克隆等下游任务中验证了有效性。

Conclusion: 这些模型为传感器转换提供了灵活解决方案，可根据数据可用性和应用需求选择合适方法，成功实现了模型在不同传感器间的迁移。
...
<details>
  <summary>Details</summary>
Motivation: 在重复遍历相同空间的机器人应用中，及时更新环境变化区域对保持地图质量至关重要。现有3DGS方法在准确高效更新变化区域方面存在挑战。
...
Method: 框架包含四个互连模块：任务初始化、任务分配、行为树更新和失败节点检测。机器人执行时tick行为树，遇到失败节点时可本地扩展树或调用集中式虚拟协调器重新分配子任务并同步行为树。

Result: 在3个模拟场景的60个任务和真实世界咖啡厅环境中验证，结果显示该方法在任务成功率、鲁棒性和可扩展性方面持续优于基线方法。

Conclusion: 该框架有效支持异构团队在复杂场景中的长期协作执行，证明了结合大语言模型和行为树的方法在多机器人协作中的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的LiDAR里程计方法大多基于子地图架构，会将姿态估计误差传播到固定子地图中，导致轨迹抖动和后续配准性能下降。需要一种既能实时运行又能主动校正局部地图的方法。

Method: 使用固定滞后平滑的密集连接因子图，结合单一迭代地图进行匹配。这种方法允许在姿态估计进一步细化时主动校正局部地图。

Result: 在多种数据集上的评估表明，FORM方法相比现有最先进的LiDAR里程计方法，具有鲁棒性、准确性、实时性，并能提供平滑的轨迹估计。

Conclusion: FORM方法成功解决了传统子地图方法的误差传播问题，实现了实时性能下的平滑轨迹估计和主动地图校正。
...
<details>
  <summary>Details</summary>
Motivation: 轮腿机器人结合了腿式运动的灵活性和轮式运动的高效性，但混合运动的全身体控制仍然具有挑战性。

Method: 基于强化学习技术构建预测策略网络，从本体感知信息估计外部环境状态，然后通过演员-评论家网络生成最优关节命令。

Result: 在多种地形（平地、楼梯、草地）上的仿真和真实实验验证了框架的可行性，在未见地形上表现出鲁棒性能。

Conclusion: 该混合运动框架具有良好的泛化能力，能够适应各种未知地形。
...
<details>
  <summary>Details</summary>
Motivation: 农业部门需要自动化解决方案来应对劳动密集型的水果蔬菜处理任务，减少收获后损失，软体机器人技术为此提供了可行方案。

Method: 采用六指3D笼式效应设计，结合刚性外框和软性拉胀内部晶格，通过实验验证、2D数字图像相关性和非线性有限元分析，评估不同晶格取向（0°、30°、45°、60°）的抓取性能。

Result: 晶格取向显著影响夹爪的顺应性、接触力和能量效率，不同配置具有各自的优势，为优化机器人夹爪性能提供了新见解。

Conclusion: 该研究为软硬混合夹爪设计提供了新思路，推进了精准农业自动化策略，同时最大限度地减少作物损伤。
...
<details>
  <summary>Details</summary>
Motivation: 长期监控多个动态目标对人类操作员来说很繁琐，单个机器人不可行。现有方法要么部署所有可用机器人而不最小化规模，要么忽略电池和内存等资源约束。

Method: 包含三个核心组件：(I)资源约束和监控间隔下的多机器人任务分配建模；(II)资源感知任务协调算法，通过Martin算法在高层次动态目标分配和低层次多目标路由间迭代；(III)针对不可预测目标行为和机器人故障的在线适应算法。

Result: 通过大规模仿真验证，在不同道路网络、机器人速度、充电速率和监控间隔下与多个基线方法进行比较，证明了方法的有效性。

Conclusion: LOMORO方案能够确保所有目标的监控间隔有明确上限，所有机器人的资源水平有下限，同时最小化平均活跃机器人数量。
...
<details>
  <summary>Details</summary>
Motivation: 月球导航中未建模的电离层和等离子体层延迟是重要误差源，特别是在特殊信号几何和长传播路径下，需要系统分析这些延迟的影响。

Method: 使用全球核心等离子体模型(GCPM)和自定义低成本射线追踪算法，模拟GNSS信号在月球轨道和南极接收时的一、二、三阶群延迟以及射线弯曲引起的额外路径长度。

Result: 平均群延迟约为1米，但在高太阳活动期间低高度射线路径可超过100米；弯曲延迟通常较小但对低高度射线路径不可忽略；信号频率、地磁Kp指数和太阳R12指数对延迟有显著影响。

Conclusion: 这些发现为利用地面GNSS信号的稳健定位和定时算法设计提供了重要参考，特别是在考虑电离层和等离子体层延迟影响时。
...
<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶预测模型的评估方法过于依赖简单的ADE和FDE指标，无法捕捉复杂交互和安全关键场景下的细微行为差异，缺乏对多智能体交互和空间分布的鲁棒性测试。

Method: 开发了一个新颖的测试框架，通过不同的场景结构（地图上下文、智能体密度和空间分布）来评估预测性能，量化智能体邻近度对轨迹预测的影响。

Result: 通过实证分析发现了当前最先进预测模型的关键脆弱性，识别了传统指标无法暴露的场景特定故障案例，证明了邻近智能体对目标轨迹预测的差异化影响。

Conclusion: 该框架为严格的安全驱动预测验证奠定了基础，有助于识别易发生故障的极端情况，促进开发鲁棒且可认证的自动驾驶预测系统。
...
<details>
  <summary>Details</summary>
Motivation: 通用机器人策略评估和改进面临真实世界部署成本高、难以扩展的挑战，世界模型提供了一种可扩展的替代方案，但需要构建能够处理多步交互的可控世界模型。

Method: 开发了可控多视角世界模型，采用姿态条件记忆检索机制保持长时一致性，通过帧级动作控制实现精确控制，在DROID数据集（95k轨迹，564场景）上训练。

Result: 模型能在新场景和新相机位置下生成时空一致的轨迹超过20秒，能够准确评估策略性能而无需真实机器人部署，通过想象空间轨迹合成将策略成功率提升44.7%。

Conclusion: 该方法为通用机器人策略的评估和改进提供了一种高效、可扩展的解决方案，显著降低了真实世界部署的成本和复杂性。
...
<details>
  <summary>Details</summary>
Motivation: 当前大型视觉语言模型导航训练主要依赖模仿专家轨迹，将复杂导航任务简化为单一正确路径的序列复制，限制了智能体的探索和泛化能力。

Method: 1) 引入包含22k轨迹的Compass-Data-22k数据集，其RFT子集通过A*测地距离标注所有可行动作；2) 设计gap感知混合奖励函数，根据决策确定性动态调整反馈；3) 采用SFT-then-RFT训练策略。

Result: 7B参数的CompassNav智能体在目标导航基准测试中达到新SOTA，超越更大规模的专有模型，并在物理机器人上实现鲁棒的真实世界目标导航。

Conclusion: 决策理解范式使智能体不再记忆静态路线，而是发展出通过评估所有可能移动的相对质量来持续直觉目标方向的内部"指南针"能力。
...
<details>
  <summary>Details</summary>
Motivation: 解决智能体在真实环境部署后无法获取新知识来提升任务性能的根本限制。

Method: 使用经验反馈网络(EFN)自动识别上下文相关的成功先验动作经验，并基于检索到的指导条件化动作预测。采用基于语义相似度奖励的强化学习来确保预测动作与当前观察下的过去成功行为一致。

Result: 在多样化具身任务实验中，EFN显著提高了适应性、鲁棒性和成功率，优于冻结基线。

Conclusion: Dejavu为具身智能体在部署后持续优化行为提供了一条有前景的路径。
...
<details>
  <summary>Details</summary>
Motivation: 当前单人形机器人方法存在隔离问题，忽略了代理间的动态交互，导致接触错位、相互穿透和不真实的运动。需要开发能够处理多机器人协调交互的框架。

Method: 包含两个关键组件：(1) 接触感知运动重定向，通过将SMPL接触与机器人顶点对齐来恢复身体间协调；(2) 交互驱动运动控制器，利用交互特定奖励来强制执行协调的关键点和物理上合理的接触。

Result: 实验表明Harmanoid显著改善了交互运动模仿效果，超越了现有的单人形机器人框架，这些框架在此类场景中大多失败。

Conclusion: 通过显式建模代理间接触和交互感知动态，Harmanoid捕捉到了单人形框架固有忽略的人形机器人之间的耦合行为，为多机器人交互提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决机器人在不确定状态环境（如模糊物体属性或不可预测交互）中操作的问题，传统模仿学习方法依赖成功示例而忽视失败场景中的不确定性。

Method: UF-RNN结合标准时间序列预测和主动"前瞻"模块，该模块执行多个未来轨迹的内部模拟，并优化隐藏状态以最小化预测方差。

Result: 在仿真和真实机器人开门任务中，模型在没有明确失败演示的情况下表现出鲁棒适应性，利用潜在空间中的自诱导混沌动力学，在环境模糊时激发探索行为，成功率优于传统随机RNN基线。
...
<details>
  <summary>Details</summary>
Motivation: 研究目标是理解结构化、类人注意力行为如何通过自上而下和自下而上机制的相互适应随时间发展，探索感知与内部预测交互在注意力自组织中的重要性。

Method: 提出A^3 RNN注意力模型，采用双向注意力架构整合预测性TD信号和基于显著性的BU线索，在机器人操作任务中使用模仿学习进行评估。

Result: 实验结果显示注意力行为在训练过程中演化：从显著性驱动探索到预测驱动定向。BU注意力最初突出视觉显著区域引导TD过程，随着学习进展，TD注意力稳定并重塑显著性感知。

Conclusion: 该模型虽未显式优化稳定性，但表现出比基线更连贯和可解释的注意力模式，支持发育机制有助于稳健注意力形成的观点，反映了认知科学和自由能框架的原则。
...
<details>
  <summary>Details</summary>
Motivation: TIAGo++ Omni是一款多功能移动机械臂，具有全向移动能力，但此前在Isaac Sim中没有可用的模型。为了支持研究人员在该机器人上进行实验和执行基于学习的控制，需要开发相应的仿真模型。

Method: 开发了两种全向驱动控制模型：物理精确模型（复制真实轮动力学）和轻量级速度模型（优化学习应用）。提出了基于学习的校准方法，使用最小轨迹数据记录来近似真实机器人的S形速度曲线。

Result: 成功在Isaac Sim中集成了TIAGo++ Omni机器人模型，并通过校准使其行为接近真实机器人。提供了公开的集成代码库。

Conclusion: 该仿真模型使研究人员能够在多样化环境中实验该机器人并执行高效的基于学习控制，为机器人研究社区提供了有价值的工具。
...
<details>
  <summary>Details</summary>
Motivation: 解决通用视觉-语言-动作模型在多样化机器人平台上有效训练的问题，利用跨具身异构数据源的丰富性。

Method: 提出软提示方法，为不同数据源引入可学习的嵌入作为具身特定提示，结合流匹配的VLA架构，仅使用软提示的标准Transformer编码器。

Result: 0.9B参数的X-VLA在6个模拟和3个真实机器人上同时实现SOTA性能，在灵活性、跨具身适应等方面表现优越。
...
Method: 使用Soft Actor-Critic(SAC)框架，结合Hindsight Experience Replay(HER)和CrossQ覆盖层，鼓励操控效率同时避免障碍物

Result: 在重型四轮转向漫游车模拟中，学习策略能鲁棒地达到97%的目标位置，同时避免障碍物

Conclusion: 该框架不依赖人工轨迹或专家演示，能有效解决双阿克曼转向机器人在复杂环境中的操控问题
...
<details>
  <summary>Details</summary>
Motivation: 化学领域长期以来依赖手动且耗时的过程，现有自动化程度有限。随着机器人技术和AI的发展，化学领域正面临重大变革，需要开发能够自主执行实验的系统来加速创新。

Method: 整合移动机械臂、先进感知系统、远程操作和数据驱动协议，构建能够协作执行化学实验的自主系统。

Result: 机器人化学家能够以更高的适应性、可重复性和安全性执行实验，在药物开发、材料科学和可持续制造等领域加速创新。

Conclusion: 化学的未来在于人类直觉与机器人精确性、AI洞察力的共生合作关系，机器人化学家是人类的补充合作伙伴而非完全替代。
...
<details>
  <summary>Details</summary>
Motivation: 嵌入式机器人系统资源受限和数值不稳定性是视觉惯性导航系统的关键挑战，平方根协方差滤波能提供数值稳定性、高效内存使用和保证正半定性。

Method: 提出新颖的基于Cholesky分解的平方根滤波更新方法，充分利用系统结构保持三角结构；设计快速鲁棒的动态初始化方法，先恢复最小状态而不三角化3D特征，然后执行迭代SRF更新优化完整状态。

Result: 在数值研究中验证了优越的数值稳定性，在32位单精度浮点数上实现鲁棒高效性能，运行速度是SOTA方法的2倍；在移动工作站和Jetson Nano上测试，在100ms窗口内实现高成功率的初始化。

Conclusion: sqrtVINS在不同场景下广泛验证，展示了强大的效率、鲁棒性和可靠性，开源实现支持未来研究和应用。
...
<details>
  <summary>Details</summary>
Motivation: 现有的动态操纵研究主要关注物体的着陆位置，而忽略了最终方向。传统的旋转机器人抛掷物体会产生寄生旋转，导致着陆姿态受限且不可控。

Method: 基于冲量-动量原理设计抛掷运动来解耦寄生旋转，结合自由飞行的物理模型和回归学习方法处理未建模效应。

Result: 真实机器人实验表明，该方法能在数十次试验内学会将物体抛掷翻转到(±5cm, ±45度)精度的目标姿态。数据同化使样本复杂度比端到端学习方法平均降低40%。

Conclusion: 该方法成功实现了对物体着陆姿态的精确控制，知识重用能显著加速新物体的学习过程，在物流操作中具有应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 协调异构机器人舰队实现多目标在多机器人系统中具有挑战性，需要降低构建可扩展多机器人系统的门槛。

Method: 提供规划、调度和执行的抽象层，使用容器化服务部署机器人，维护共享声明式世界状态和双向通信，利用LLMs进行开放世界推理。

Result: 开发了一个集中式多机器人任务规划和调度框架，支持异构机器人舰队完成多个任务。

Conclusion: RobotFleet通过模块化自主堆栈各层和使用LLMs，降低了构建可扩展多机器人系统的障碍。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个低成本的便携式微机器人实验平台，降低微机器人实验的门槛，使其不仅适用于专业研究实验室，也适合教育和推广环境。

Method: 使用Python和Arduino C++开发定制控制软件，处理实时视频采集、微机器人跟踪，并生成电磁线圈和声学换能器的控制信号。

Result: 创建了一个多模态驱动的平台，集成了嵌入式计算机、显微镜、电源和控制电路，实现了实时闭环控制功能。

Conclusion: 该系统通过降低微机器人实验的门槛，为生物医学、组织工程和机器人学领域的研究、教育和转化应用创造了新的机会。
...
<details>
  <summary>Details</summary>
Motivation: 单机器人系统在受限感知能力下难以在大空间区域内持续跟踪多个动态目标，现有方法依赖短规划视野且假设小范围环境，导致大规模场景中跟踪性能差和目标丢失。

Method: 分层规划器结合运动模型和不确定性传播，将多目标跟踪分解为单目标搜索子任务，包含低层覆盖规划器搜索演化信念区域，以及评估子任务成功概率的估计方法，将任务转换为MDP问题并用树基算法求解。

Result: 在模拟验证中，提出的规划器相比现有方法表现更优，在不同环境中实现了11-70%的最终不确定性降低。

Conclusion: 该分层规划方法通过长期不确定性管理和任务分解策略，有效解决了大规模多动态目标跟踪的挑战。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法生成特定步态序列需要专家大量调参，包括触地/抬腿事件和每条腿的约束条件，过程繁琐

Method: 利用动态腿式系统的内在对称性和速度-周期关系，设计对称性引导的奖励函数，包含时间、形态和时间反转对称性

Result: 在Unitree Go2机器人上实现，在仿真和硬件测试中均表现出鲁棒性能，能平滑过渡多种步态模式

Conclusion: 揭示了对称性在机器人步态设计中的关键作用，为动态运动策略提供了新见解
...
<details>
  <summary>Details</summary>
Motivation: 伽利略对称性是牛顿物理学中惯性运动的自然对称性，但在机器人学领域缺乏与刚体对称性类似的系统阐述。本文旨在填补这一空白，让机器人社区能够从这一经典理论中受益。
...
Conclusion: 现在是机器人社区重新发现和扩展这一经典材料，并将其应用于现代问题的合适时机，伽利略对称性可以为机器人学提供重要的理论基础和应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 解决机器人在资源受限环境下高维连续控制任务中的能效和性能平衡问题，传统DRL方法计算需求大、能耗高。

Method: 使用群体编码脉冲演员网络(PopSAN)将高维观测编码为神经元群体活动，结合SNN的事件驱动异步计算特性和DRL的稳健策略优化能力。

Result: 在Isaac Gym平台的PixMC基准测试中，Franka机械臂实验显示相比传统人工神经网络能耗降低96.10%，同时保持可比的操控性能，实现稳定的手指位置跟踪和目标高度维持。
...
<details>
  <summary>Details</summary>
Motivation: 四通道双边控制能够通过同步两个机械臂的位置和力来实现带力反馈的远程控制，改善接触丰富任务中的操作性。笛卡尔坐标系中的四通道双边控制特别适合不同结构的机械臂，并能通过调整控制参数实现直观的操作性。

Method: 提出了一种四通道双边控制方法，在笛卡尔坐标系中解耦每个维度，无论缩放因子如何都能实现期望的动态特性。

Result: 该方法能够实现期望的动态特性，提高远程控制的操作性，特别是在接触丰富的任务中。

Conclusion: 所提出的四通道双边控制方法在笛卡尔坐标系中有效，能够通过解耦各维度实现期望的动态特性，不受缩放因子限制，提高了远程控制的操作性。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于采样的轨迹规划器使用手动调优的静态权重成本函数，在不同场景下需要战术妥协，无法实现最优性能。

Method: 提出强化学习代理作为高层行为选择器，在运行时动态切换分析性低层轨迹规划器的成本函数参数。

Result: 在自动驾驶赛车仿真中，RL规划器实现了0%碰撞率，同时将超车时间减少高达60%，能够动态切换激进和保守行为。

Conclusion: 强化学习作为高层选择器的集成解决了自动驾驶赛车规划器中安全性与竞争力的固有权衡，为更广泛的自动驾驶应用提供了自适应且可解释的运动规划路径。
...
Method: 利用SPAD相机的独特成像能力，详细阐述其工作原理和性能特征，评估其在解决月球探测关键感知挑战中的优势和局限。

Result: 在代表性光照条件下对SPAD相机性能进行基准测试，证明其作为行星导航替代技术的潜力。

Conclusion: SPAD相机为解决行星机器人导航中的感知挑战提供了有前景的解决方案，特别适用于高纬度月球区域等具有挑战性的环境。
...
<details>
  <summary>Details</summary>
Motivation: 传统机器人抓取系统依赖将传感器数据转换为显式3D点云，这种计算步骤在生物智能中不存在。本文探索一种根本不同的、受神经启发的范式。

Method: 引入SpikeGrasp框架，模拟生物视觉运动通路，处理来自立体脉冲相机的原始异步事件，使用循环脉冲神经网络迭代优化抓取假设，无需重建点云。

Result: 实验表明SpikeGrasp超越传统基于点云的基线方法，特别是在杂乱和无纹理场景中，并显示出卓越的数据效率。

Conclusion: 通过验证这种端到端、受神经启发方法的可行性，SpikeGrasp为未来能够实现自然界中流畅高效操作的系统铺平了道路，特别是对于动态物体。
...
Method: 使用3D高斯溅射重建场景外观，网格基元处理交互物体物理模拟，利用多模态大语言模型自动推断物体物理属性和运动学结构。

Result: 在RoboSimGS生成的数据上训练的策略实现了成功的零样本模拟到真实世界迁移，并能显著提升现有方法的性能和泛化能力。

Conclusion: RoboSimGS是弥合模拟与现实差距的强大且可扩展的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有通用机器人策略要么基于视觉语言理解模型，要么基于生成模型，但语义理解和视觉动态建模对机器人同等重要。统一的理解和生成模型已证明在理解和生成方面都有强大能力，机器人策略学习同样可以从理解、规划和连续未来表示学习的结合中受益。

Method: UniCoD首先在超过100万互联网规模的教学操作视频上进行预训练，学习高维视觉特征的动态建模能力；然后在机器人本体收集的数据上进行微调，学习从预测表示到动作标记的映射。

Result: 广泛实验表明，UniCoD在模拟环境和真实世界分布外任务中分别比基线方法提升了9%和12%。

Conclusion: UniCoD通过结合理解和生成能力，证明了在机器人策略学习中统一理解和规划方法的有效性，为构建能够处理开放环境中多样化任务的通用机器人策略提供了有前景的方向。
...
<details>
  <summary>Details</summary>
Motivation: 替代传统的MC架构，统一符号决策（提供可理解、可重复、可证明的行为）与机器学习技术及反应行为，实现跨海洋学平台的现场就绪性。

Method: 基于认知架构和AUV控制系统，使用Python3实现，具有可扩展性、模块化和可重用性。支持声明式任务规范和灵活的行为规范，同时使用实时任务规划和硬编码用户指定计划。

Result: 在Sentry AUV上进行了实地测试，并进行了多种模拟案例验证，证明了该系统的功能特性。

Conclusion: DINOS-R成功统一了符号决策与机器学习技术，为海洋学和机器人算法的未来研究提供了可扩展的基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法缺乏生成可控多样化轨迹的方案，而随机多样化的轨迹对自动驾驶安全规划不够有用。人类行为具有内在不确定性和多模态特性，需要可控的语义多样性。

Method: 使用条件变分自编码器(CVAE)框架，通过弱标签偏好对来增强模型，使潜在变量具有语义属性（如平均速度）。

Result: PrefCVAE能够实现可控的、语义上有意义的预测，同时不降低基线准确性。

Conclusion: 偏好监督是增强基于采样的生成模型的一种成本效益高的有效方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有机器人运动学习技术严重依赖离线选择奖励权重增益，无法保证训练过程中的约束满足（即约束违反）。

Method: ROGER方法基于在具体交互过程中收到的惩罚来在线调整奖励权重增益。当学习接近约束阈值时，正奖励与负奖励增益的比率自动降低以避免违反；在安全状态下则增加比率以优先考虑性能。

Result: 在60公斤四足机器人上，ROGER在多次学习试验中实现了接近零约束违反，主要奖励比最先进技术高出50%。在MuJoCo连续运动基准测试中，性能提高达100%，扭矩使用和方向偏差减少60%。物理四足机器人的真实世界运动学习在1小时内完成且无摔倒。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法针对特定任务，缺乏通用环境表示。USD作为XML格式的场景图，既适合LLM理解又足够丰富，能支持各种机器人任务

Method: 使用四足机器人搭载LiDAR和RGB相机构建室内环境的USD表示，然后通过Google Gemini解析USD进行场景理解、推理和规划

Result: 成功在真实室内环境和模拟仓库、医院场景中验证了系统的有效性，展示了复杂推理和规划能力

Conclusion: USD是连接LLM与机器人的有效通用表示，为LLM驱动的机器人任务提供了统一框架
...
<details>
  <summary>Details</summary>
Motivation: 传统Voronoi覆盖控制在应急场景中存在三个关键限制：异构代理能力（速度差异大）、集群初始部署配置以及紧急时间约束需要快速响应而非渐进收敛。
...
Conclusion: 该方法有效解决了应急响应场景中的关键挑战，显著提高了混合空中-地面机器人团队的部署效率和响应速度。
...
<details>
  <summary>Details</summary>
Motivation: 机器人触觉感知需要将传感器数据转换为适合高层计算的结构化数据，这种转换直接影响任务执行效果。现有研究通常借用计算机视觉等领域的技术，但缺乏对数据表示方法的系统分析。

Method: 通过文献综述分析触觉信息表示方法，研究硬件、表示结构和高层计算之间的关系，识别出六种常用数据结构。

Result: 识别出六种常用的触觉数据表示结构，并提供了根据操作条件（硬件、所需触觉信息、具体任务）选择适当表示的指导原则。

Conclusion: 数据表示是触觉感知流程中的关键环节，选择合适的表示方法需要考虑硬件特性、信息编码需求和具体任务要求，论文为此提供了系统性的指导框架。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法依赖复杂的摩擦模型或基于电机电流的扭矩估计，精度有限。需要更直接、准确的扭矩测量方法来提高接触检测和定位的可靠性。

Method: 设计了低成本应变片式关节扭矩传感器，安装在每个关节上提供直接扭矩测量。采用广义动量观测器框架，结合分布式关节扭矩传感器和单个髋部FT传感器来检测和定位接触。

Result: 仿真验证了框架能准确恢复大腿和小腿连杆上的接触力和位置。扭矩传感器校准后达到96.4%的平均精度。硬件实验显示亚厘米级的接触定位精度和低于0.2N的力误差。

Conclusion: 该方法通过分布式关节扭矩传感器和广义动量观测器，实现了高精度的接触检测和定位，为机器人腿部接触感知提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有RL方法主要强调鲁棒性，产生抵抗外部力的策略但缺乏顺应性，这对本就不稳定的人形机器人特别具有挑战性

Method: 将人形运动建模为多目标优化问题，引入偏好条件MORL框架，通过速度阻力因子建模外部力，使用编码器-解码器结构从可部署观测中推断特权特征
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖静态先验或有限记忆，在部分可观测性和语义模糊性下适应性受限。需要整合感知、符号推理和空间规划来泛化到不同布局和物体类别。

Method: GRIP框架包含三个变体：GRIP-L（轻量级，通过语义占用网格优化符号导航）、GRIP-F（完整版，支持多跳锚链和LLM自省）、GRIP-R（现实世界版，在感知不确定性下支持物理机器人部署）。整合了动态2D网格构建、开放词汇物体定位、共现感知符号规划以及使用行为克隆、D*搜索和网格条件控制的混合策略执行。

Result: 在AI2-THOR和RoboTHOR基准测试中，GRIP在长视野任务上实现了最高9.6%的成功率提升和超过2倍的路径效率（SPL和SAE）改进。在Jetbot上的现实世界部署进一步验证了其在传感器噪声和环境变化下的泛化能力。

Conclusion: GRIP作为一个鲁棒、可扩展和可解释的框架，成功连接了仿真和现实世界导航，在动态复杂环境中表现出优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 标准自动驾驶流水线中，全局规划器的优化信息在传递到局部规划器时丢失，导致局部规划器在偏离最优路径时缺乏全局视野。

Method: 通过QuayPoints区域保留全局知识，这些区域标识偏离最优路径会导致显著性能损失的关键位置。

Result: 集成QuayPoints的规划器能够在四个不同赛道上持续超越以自我车辆75%速度行驶的对手。

Conclusion: QuayPoints框架有效提升了局部规划器的决策质量，特别是在战略超车场景中。
...
Method: 将共享控制建模为合作微分博弈，引入驾驶员特定状态追踪矩阵，基于时间和追踪能力动态调整控制权。

Result: 在ISO标准车道变换场景中，自适应过渡策略相比传统方法减少了轨迹偏差和驾驶员控制负担，同时提高了车辆稳定性。

Conclusion: 基于实时偏差连续调整控制权的自适应过渡策略能有效提升接管过程的自然性和安全性，降低驾驶员负担。
...
TL;DR: 这篇论文对机器人操作领域进行了全面综述，涵盖了基础背景、任务导向的基准测试和数据集，并提出了现有方法的统一分类法。


<details>
  <summary>Details</summary>
Motivation: 机器人操作是具身智能中的核心挑战，需要将感知、规划和控制无缝集成，以在多样化和非结构化环境中实现交互。

Method: 扩展了经典的高层规划与低层控制划分，将高层规划扩展到语言、代码、运动、功能性和3D表示，同时基于训练范式提出了低层学习控制的新分类法。

Result: 提供了首个专注于数据收集、利用和泛化等关键瓶颈的分类法，并对实际应用进行了广泛回顾。

Conclusion: 与之前的综述相比，这项工作提供了更广的覆盖范围和更深入的见解，为新研究人员提供了易于理解的路线图，为经验丰富的研究人员提供了结构化参考。
...
TL;DR: RoboMAP是一个语言引导机器人框架，使用连续自适应的affordance热图表示空间目标，解决了离散点表示对感知噪声和语义模糊的脆弱性问题。


<details>
  <summary>Details</summary>
Motivation: 传统语言引导机器人系统将空间推理简化为离散点，对感知噪声和语义模糊非常脆弱。需要一种能捕捉空间定位不确定性的表示方法。

Method: 提出RoboMAP框架，使用连续、自适应的affordance热图来表示空间目标，这种密集表示能捕获空间定位的不确定性。

Result: 在多数grounding基准测试中超越先前SOTA，速度提升高达50倍，真实世界操作成功率82%，在模拟和物理实验中表现稳健，具有强大的零样本导航泛化能力。

Conclusion: RoboMAP通过连续affordance热图表示显著提升了任务成功率和可解释性，为语言引导机器人系统提供了更鲁棒的空间推理方法。
...
Method: GTR2L框架包含多级博弈论世界模型、自适应滚动时域、不确定性感知屏障机制，以及专门的风险建模方法，显式捕捉认知和随机不确定性。

Result: 在多种安全关键交通场景中的广泛评估表明，GTR2L在成功率、碰撞和违规减少以及驾驶效率方面显著优于现有最佳基准方法，包括人类驾驶员。

Conclusion: GTR2L框架通过博弈论风险塑造方法有效解决了自动驾驶中的安全挑战，在复杂动态环境中实现了更好的安全性和性能平衡。
...
TL;DR: RoVer是一个用于提升视觉-语言-动作模型性能的测试时扩展框架，通过机器人过程奖励模型作为验证器来优化动作选择，无需修改基础模型架构或权重。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉-语言-动作模型性能提升通常依赖于扩大训练数据和模型规模，这在机器人领域成本过高且受限于数据收集成本。

Method: 使用机器人过程奖励模型评估候选动作的可靠性并预测动作空间方向，在推理时并行生成多个候选动作，沿预测方向扩展，然后通过PRM评分选择最优动作执行。

Result: 通过缓存共享感知特征，可在相同测试计算预算下评估更多候选动作，将可用计算资源转化为更好的动作决策。

Conclusion: RoVer提供了一个通用、即插即用的测试时扩展框架，通过联合提供标量过程奖励和动作空间方向的PRM，以及高效的方向引导采样策略，实现了无需额外训练开销的测试时扩展效益。
...
<details>
  <summary>Details</summary>
Motivation: 室内环境中的磁干扰会显著降低无人机航向估计精度，需要一种能在磁干扰环境下可靠工作的航向估计算法。

Method: 使用EKF框架结合惯性传感器和磁传感器，通过自适应过程噪声协方差补偿陀螺仪误差，并基于实时磁偏差检测应用缩放因子来缓解磁干扰影响。

Result: 在真实室内环境中的实验表明，该算法在磁干扰条件下能提供精确的航向估计。

Conclusion: AMO-HEAD算法在磁干扰环境中表现出良好的航向估计性能，具有实际应用价值。
...
TL;DR: 提出了一种基于采样的方法，利用预训练生成模型为零样本规划提供概率先验，能够从部分观测恢复完整的RGB-D点云，用于配置空间规划。


<details>
  <summary>Details</summary>
Motivation: 在部分可观测环境下规划需要先验知识，但实践中难以获取。需要一种能够捕捉环境不确定性和空间语义关系的零样本方法。

Method: 使用大规模预训练生成模型构建采样流水线，基于部分观测生成完整的RGB-D点云样本，包含占据和目标语义信息。

Result: 在Matterport3D基准测试中，该方法能够恢复与真实情况一致的常识性空间语义，生成多样化、干净的3D点云，可用于运动规划。

Conclusion: 生成模型为机器人规划提供了丰富的先验知识来源，展示了其在处理部分可观测环境中的潜力。
...
TL;DR: Refinery框架通过贝叶斯优化微调策略和高斯混合模型采样，将机器人装配任务的模拟学习成功率从约80%提升到91.51%，并支持多部件装配链式执行。


<details>
  <summary>Details</summary>
Motivation: 现有模拟学习方法在接触密集型任务中成功率约80%，虽满足研究需求但未达工业标准，且策略链式执行脆弱，需要提高策略在不同初始条件下的鲁棒性。

Method: 提出Refinery框架：1）使用贝叶斯优化指导策略微调；2）部署时采用高斯混合模型采样选择最大化执行成功率的初始化条件。

Result: 在机器人装配任务中，相比最先进方法平均成功率提升10.98%，模拟环境下达到91.51%，真实世界性能相当；成功链式执行最多8个部件的装配任务。

Conclusion: Refinery框架有效提升了模拟学习策略的鲁棒性和成功率，支持无需显式多步训练的长时域多部件装配任务。
...
Method: 采用分层两阶段架构：第一阶段使用抓取点预测器(GPP)识别最佳抓取位置；第二阶段使用角度-宽度预测器(AWP)通过局部特征细化抓取角度和宽度，AWP模块中的对比学习实现零样本泛化。

Result: 实验结果显示在各种夹爪类型上具有竞争力的抓取成功率，同时相比现有夹爪感知方法在推理速度上有显著提升。

Conclusion: XGrasp提供了一个模块化框架，能够无缝集成视觉基础模型，为未来的视觉-语言能力提供途径，有效解决了多夹爪抓取检测问题。
...
TL;DR: 提出基于深度强化学习的自主协同规划框架(DRLACP)，使用软演员-评论器(SAC)和门控循环单元(GRU)来处理感知、规划和通信不确定性，在CARLA仿真平台验证优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有自主协同规划策略无法充分处理多车辆交互中的各种不确定性，包括感知、规划和通信不确定性，需要更有效的解决方案。

Method: 采用软演员-评论器(SAC)结合门控循环单元(GRU)的深度强化学习方法，学习在状态信息不完美情况下的确定性最优时变动作。

Result: 在CARLA仿真平台上的评估结果显示，提出的DRLACP方法能够有效学习和执行协同规划，在不同场景下状态信息不完美时优于其他基线方法。

Conclusion: DRLACP框架能够有效处理自主车辆协同运动规划中的各种不确定性，为智能交通系统中的多车辆交互提供了更可靠的解决方案。
...
TL;DR: PhysHSI是一个物理世界人形机器人-场景交互系统，通过仿真训练和现实部署相结合，使人形机器人能够自主执行多样交互任务并保持自然行为。


<details>
  <summary>Details</summary>
Motivation: 将人形机器人部署到现实环境中进行物体搬运、椅子就坐等交互任务，需要泛化的、逼真的运动能力和鲁棒的场景感知能力，现有方法难以统一实现这些能力。

Method: 采用基于对抗运动先验的策略学习来模仿自然的人形-场景交互数据，实现泛化和逼真行为；在现实部署中引入粗到细的物体定位模块，结合LiDAR和相机输入提供连续鲁棒的场景感知。

Result: 在仿真和现实环境中验证了四个代表性交互任务（搬运箱子、就坐、躺下、站立），表现出高成功率、强泛化能力和自然运动模式。

Conclusion: PhysHSI系统成功实现了人形机器人在物理世界中的自主交互能力，结合了自然运动生成和鲁棒场景感知，为现实世界部署提供了可行方案。
...
TL;DR: Flow Planner通过轨迹标记化、时空融合架构和流匹配技术，在复杂驾驶场景中有效建模交互行为，在nuPlan和interPlan数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 解决复杂场景中交互驾驶行为建模的挑战，传统方法依赖过度设计的架构且缺乏专门的交互机制，交互数据稀缺使模仿学习方法难以捕捉高价值交互行为。

Method: 采用细粒度轨迹标记化分解轨迹，设计专门的时空融合架构整合规划与场景信息，结合流匹配和分类器自由引导进行多模态行为生成。

Result: 在大规模nuPlan数据集和挑战性interPlan数据集上，Flow Planner在学习基方法中达到最先进性能，能有效建模复杂驾驶场景中的交互行为。

Conclusion: Flow Planner通过数据建模、模型架构和学习方案的协调创新，成功解决了复杂驾驶场景中交互行为建模的挑战。
...
TL;DR: 本文提出了一种基于折纸启发气动执行器的软体外骨骼康复系统，使用深度Koopman网络建模人机交互动力学，并结合模型预测控制实现膝关节康复训练。


<details>
  <summary>Details</summary>
Motivation: 传统刚性外骨骼笨重且穿戴不便，需要额外合规控制保证安全；软体外骨骼虽然穿戴舒适且具有内在合规性，但其复杂非线性人机交互动力学给控制带来挑战。

Method: 设计基于折纸气动执行器的膝关节康复外骨骼，使用深度Koopman网络建模人机交互动力学（以EMG信号和PWM占空比为输入），并基于Koopman模型采用模型预测控制进行实时康复训练。

Result: 实验表明，将EMG信号整合到Koopman模型中显著提高了模型精度；个性化Koopman模型优于非个性化模型；所提控制框架在被动和主动训练模式下均优于传统PID控制。
...
TL;DR: 系统评估了SO(3)动作表示在强化学习中的影响，发现局部坐标系下的切向量表示在多种算法中表现最可靠。


<details>
  <summary>Details</summary>
Motivation: 机器人控制任务需要在SO(3)上操作，但SO(3)缺乏全局光滑的最小参数化，现有表示方法各有约束和失效模式，其在强化学习中的影响尚不明确。

Method: 在三种标准连续控制算法(PPO、SAC、TD3)下，使用密集和稀疏奖励，系统评估SO(3)动作表示，比较表示如何影响探索、与熵正则化的交互以及训练稳定性。

Result: 表示诱导的几何结构强烈影响探索和优化，局部坐标系下的切向量表示在算法间表现最可靠。

Conclusion: 为选择和旋转动作提供了简单、可立即实施的指南，强调表示选择对强化学习性能的重要性。
...
TL;DR: DemoHLM是一个人形机器人运动操作框架，通过单一仿真演示实现真实人形机器人的通用运动操作，结合底层全身控制器和基于视觉反馈的高层操作策略。


<details>
  <summary>Details</summary>
Motivation: 人形机器人的运动操作是实现与人类环境灵活交互的关键挑战，现有方法依赖硬编码任务定义或昂贵的真实世界数据收集，限制了自主性和泛化能力。

Method: 采用分层架构：底层通用全身控制器将全身运动指令映射到关节扭矩，提供全方位移动性；高层操作策略通过数据生成和模仿学习在仿真中学习，利用闭环视觉反馈控制全身控制器执行任务。

Result: 实验显示合成数据量与策略性能呈正相关，验证了数据生成管道的有效性。在配备RGB-D相机的Unitree G1机器人上的真实实验证明了DemoHLM的仿真到现实迁移能力，在十个运动操作任务中表现出鲁棒性能。

Conclusion: DemoHLM框架成功实现了从单一仿真演示到真实人形机器人的通用运动操作，展示了数据高效的方法和良好的仿真到现实迁移能力。
...
TL;DR: 提出了一个转子故障感知的四旋翼导航系统，通过快速故障检测与诊断、非线性模型预测控制和时空联合优化的规划器，在未知复杂环境中实现转子故障下的自主飞行。


<details>
  <summary>Details</summary>
Motivation: 四旋翼转子故障会导致高速旋转和振动，给未知环境中的自主飞行带来重大挑战，现有方法主要依赖容错控制和预定义轨迹跟踪，无法实现在线故障检测、诊断、轨迹规划和容错控制的综合解决方案。

Method: 1. 基于复合故障检测与诊断的非线性模型预测控制器，结合电机动力学；2. 转子故障感知规划器，利用故障检测结果进行时空联合优化；3. 配备四个抗扭矩板的LiDAR四旋翼平台，确保高速旋转下的可靠感知。

Result: 在密集房间和未知森林等挑战性环境中，首次实现了转子故障下的自主四旋翼飞行，包括螺旋桨卸载和电机停转等故障情况，性能优于现有最先进方法。

Conclusion: 该研究成功开发了一个完整的转子故障感知导航系统，突破了在未知复杂环境中转子故障下自主飞行的技术瓶颈，为四旋翼的安全可靠运行提供了创新解决方案。
...
TL;DR: 提出了一种自适应轨迹采样方法，通过生成密集候选点并使用多目标成本函数评估，结合预测感知MPPI控制器，解决了动态拥挤环境中机器人跟随任务中的遮挡问题。


<details>
  <summary>Details</summary>
Motivation: 解决机器人跟随任务中因移动障碍物（如行人）造成的复杂遮挡问题，现有方法依赖固定点跟随或稀疏候选点选择，无法充分处理动态拥挤环境中的频繁遮挡。

Method: 自适应轨迹采样方法生成社交感知区域内的密集候选点，使用多目标成本函数评估；基于最优点估计相对于目标预测运动的人员跟随轨迹；设计预测感知MPPI控制器同时跟踪轨迹并主动避障。

Result: 在平滑性、安全性、鲁棒性和人类舒适度方面优于最先进基线方法，在真实世界移动机器人场景中验证了有效性。

Conclusion: 该方法能够有效处理动态拥挤环境中的遮挡问题，实现更平滑、安全、鲁棒的机器人跟随性能。
...
TL;DR: 提出了一种自监督的层次化操作概念学习框架，通过跨模态相关性和多时间尺度抽象来捕捉操作中的不变模式，无需人工标注即可学习可转移的操作概念。


<details>
  <summary>Details</summary>
Motivation: 机器人操作需要能够捕捉环境和任务间不变交互模式的表示，以实现有效泛化。现有方法通常依赖人工标注，难以扩展到复杂场景。

Method: 结合跨模态相关网络识别跨感官模式的持久模式，以及多时间尺度预测器在时间尺度上层次化组织表示，形成双结构框架。

Result: 在模拟基准和真实部署中的实证评估显示，概念增强策略带来显著性能提升，学习到的概念类似于人类可解释的操作基元。

Conclusion: 该工作推进了操作表示学习的理解，并为增强机器人在复杂场景中的性能提供了实用方法。
...
<details>
  <summary>Details</summary>
Motivation: 解决人形机器人在工业检查任务中需要同时实现高效率和高精度的问题，特别是在多位置任务中减少时间成本并提高成功率。

Method: 采用分层规划策略，利用逆运动学和混合整数规划降低计算复杂度；新颖的MIP公式优化站立位置选择和轨迹长度；集成MPC系统简化运动学并进行单步位置校正。

Result: 在Kuavo 4Pro人形平台上通过仿真和实验验证，框架在多位置任务中表现出低时间成本和高成功率。

Conclusion: 该框架能够高效精确地执行复杂工业操作，为人形机器人的实际工业应用提供了可行解决方案。
...
TL;DR: 本文提出了一种AI驱动的物联网机器人遥操作系统，专为智慧城市应用设计，集成了跨平台移动界面、MQTT控制信号、WebRTC视频流和轻量级YOLOv11目标检测模型，实现了低延迟的远程操控和智能视觉监控。


<details>
  <summary>Details</summary>
Motivation: 为满足智慧城市中远程基础设施检查、公共设备维护和城市自动化等需求，需要开发一个能够实现实时远程操作和智能视觉监控的机器人遥操作系统，克服传统平台在模块化部署、实时AI感知和适应性通信方面的不足。

Method: 系统采用Flutter跨平台移动界面，通过MQTT协议传输控制信号，使用WebRTC视频流传输技术，部署YOLOv11-nano模型进行轻量级目标检测，控制命令通过ESP8266节点和Arduino Mega2560控制器协调多轴机械臂运动，后端基础设施托管在DigitalOcean云平台上。

Result: 在本地和国际VPN场景下的延迟评估显示，执行器响应时间低至0.2秒，总视频延迟低于1.2秒，即使在高速延迟网络下也能保持性能。系统实现了响应式闭环交互和分布式环境中的稳健性能。

Conclusion: 该系统通过模块化部署、实时AI感知和适应性通信策略，为智慧城市应用提供了有效的解决方案。未来将重点发展边缘设备部署、自适应路由和城市级物联网网络集成，以增强系统的弹性和可扩展性。
...
TL;DR: SIM是一种共享内存传输方案，专为自动驾驶车辆中的主机内管道设计，通过在ROS 2旁运行，显著降低数据传输延迟，提高控制频率，缩短紧急制动距离。


<details>
  <summary>Details</summary>
Motivation: 高速自动驾驶车辆需要快速控制循环和严格受限的感知到执行延迟。现有ROS 2中间件在多个传感器和控制节点共享计算单元时，会引入显著的序列化、复制和发现开销，缩小可用时间预算。

Method: SIM采用共享内存传输，保持传感器数据的原生内存布局，使用无锁有界双缓冲区优先保证数据新鲜度，仅需四行代码即可集成到ROS 2节点中。

Result: 在NVIDIA Jetson Orin Nano上，SIM相比ROS 2零拷贝传输减少数据传输延迟达98%，平均延迟降低约95%，尾部延迟降低约96%。在Level 4车辆测试中，定位频率从7.5Hz提升到9.5Hz，感知到决策平均延迟从521.91ms降至290.26ms，40mph时速下紧急制动距离减少4.14米。

Conclusion: SIM为自动驾驶车辆提供了优化的数据传输方案，在数据新鲜度和低延迟优先于保证完整性的应用场景中表现优异，显著提升了系统响应速度和安全性。
...
TL;DR: 提出了一种用于3D多智能体空战的分层强化学习框架，通过分层决策和多种训练技术提升学习效率和战斗性能


<details>
  <summary>Details</summary>
Motivation: 解决真实空战模拟中因态势感知不完善和飞行动力学非线性带来的挑战，实现任务目标

Method: 结合异构智能体动力学、课程学习、联盟博弈和新训练算法的分层多智能体强化学习框架，分为低级精确控制策略和高级战术命令策略

Result: 经验结果表明该分层方法在复杂空战场景中提高了学习效率和战斗性能

Conclusion: 分层强化学习框架有效解决了复杂空战环境中的决策挑战，提升了智能体的作战能力
...
TL;DR: 提出了一种模块化的成本感知调节器，通过平滑的动作调节而非覆盖策略来减少约束违反，同时保持探索能力。


<details>
  <summary>Details</summary>
Motivation: 现有安全强化学习方法要么使用单一策略优化奖励和安全性，导致目标冲突和不稳定；要么使用需要先验系统知识的外部安全过滤器来覆盖动作。

Method: 训练一个调节器来基于预测的约束违反情况缩放智能体的动作，最小化约束违反同时避免动作的退化抑制。该方法可与SAC、TD3等离策略RL方法无缝集成。

Result: 在Safety Gym运动任务上实现了最先进的回报-成本比，约束违反减少了最多126倍，同时回报提高了超过一个数量级。

Conclusion: 模块化成本感知调节器通过平滑动作调节有效解决了安全强化学习中的探索-安全权衡问题，优于现有方法。
...
TL;DR: 提出了一种基于对偶四元数的新型非线性模型预测控制框架（DQ-NMPC），用于四旋翼飞行器的精确控制，显著提升了跟踪性能并改善了数值条件。


<details>
  <summary>Details</summary>
Motivation: 四旋翼飞行器在复杂动态环境中具有巨大应用潜力，但由于其欠驱动特性和平移与旋转动力学的强耦合，实现精确控制仍然是一个重大挑战。

Method: 使用对偶四元数表示四旋翼动力学和姿态误差，在双四元数流形上构建紧凑且全局非奇异的控制框架，准确捕捉四旋翼的耦合动力学特性。

Result: 相比传统基线NMPC方法，位置和姿态误差分别降低了56.11%和56.77%，在11m×4.5m×3.65m的受限空间内达到13.66m/s的最大速度和4.2g的加速度，而基线控制器在此条件下失效。

Conclusion: DQ-NMPC框架通过双四元数表示有效解决了四旋翼控制中的动力学耦合问题，在数值条件和跟踪性能方面均优于传统方法，能够处理激进轨迹。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据驱动模拟器难以有效建模城市交叉路口的密集异构交互，特别是在中国等国家常见的机动车辆、非机动车辆和行人混合交通场景，存在模式崩溃和长期模拟不稳定的问题。

Method: 基于CiCross数据集，提出交互解耦策略(IDS)，通过从智能体子集学习组合动态，实现从边缘到联合的模拟，并集成到场景感知Transformer网络中。

Result: 在CiCross数据集上的实验表明，IntersectioNDE在模拟保真度、稳定性以及复制复杂分布级城市交通动态方面优于基线方法。

Conclusion: IntersectioNDE通过交互解耦策略显著增强了异构交互建模的鲁棒性和长期稳定性，为复杂城市交叉路口场景提供了有效的模拟解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 在动态不确定环境中，腿式和空中机器人的状态估计面临关键挑战，即过程噪声和测量噪声协方差的设定通常未知或需要手动调整。

Method: 采用双层优化框架，上层将噪声协方差和模型参数作为优化变量，下层执行全信息估计器，通过估计器的微分实现轨迹级目标的直接优化。

Result: 在四足和人形机器人上验证，相比手动调优基线，显著提高了估计精度和不确定性校准效果。
...
TL;DR: NaviGait是一个分层框架，结合了轨迹优化的结构和强化学习的适应性，用于实现鲁棒直观的双足运动控制。


<details>
  <summary>Details</summary>
Motivation: 强化学习虽然能学习鲁棒控制策略，但奖励设计复杂且不直观；而离线轨迹优化方法虽然可调性好，但对真实世界扰动脆弱。

Method: 利用离线优化步态库，在高维命令下平滑插值生成连续参考运动，策略提供关节级和速度命令残差修正来调制和稳定参考轨迹。

Result: NaviGait相比传统和基于模仿的强化学习训练更快，产生的运动最接近原始参考，简化了奖励设计。

Conclusion: 通过解耦高层运动生成和低层修正，NaviGait为实现动态鲁棒运动提供了更可扩展和泛化的方法。
...
TL;DR: 提出了一种带有外部跟踪系统的教育用全向机器人套件，能够解决现有机器人套件局限于自我中心感知的问题，让学生接触更高级的机器人学概念。


<details>
  <summary>Details</summary>
Motivation: 当前教育机器人套件大多局限于机器人的自我中心感知，难以让学生接触涉及坐标或导航等高级问题。

Method: 开发了一种带有外部跟踪系统的全向机器人套件，减轻了嵌入式系统的限制。

Result: 该套件能够让学生探索原本无法触及的机器人学高级方面。

Conclusion: 这种外部跟踪系统的方法为教育机器人学提供了接触高级概念的可行途径。
...
TL;DR: SCOOP'D是一种使用模拟和扩散策略学习通用机器人舀取技能的方法，能够零样本应用于现实世界的各种舀取场景。


<details>
  <summary>Details</summary>
Motivation: 开发通用自主的机器人舀取策略具有挑战性，需要理解复杂的工具-物体交互，特别是涉及可变形物体（如颗粒介质或液体）时，这些物体具有无限维配置空间和复杂动力学。

Method: 使用OmniGibson模拟器收集基于特权状态信息的舀取演示，然后通过扩散策略从观测输入中模仿这些演示。

Result: 在465次零样本部署试验中，该方法在不同难度级别的物体上表现出色，优于所有基线和消融实验。

Conclusion: SCOOP'D是获取机器人舀取技能的有前景方法，能够直接应用于多样化的现实世界场景。
...
TL;DR: 提出了一种用于液压挖掘机的高精度、可改造的2D力和载荷估计算法，无需额外操作要求，实现了1%的载荷精度和383N的力测量精度。


<details>
  <summary>Details</summary>
Motivation: 准确估计液压挖掘机末端执行器的交互力是实现重型机械高级自动化的关键，可以改进精确的平整和挖掘操作。

Method: 提出了一种基于优化的载荷估计算法，通过识别动态模型来估计末端执行器交互力和铲斗载荷，仅需压力传感器和惯性测量，无需机器特定动态特性的先验知识。

Result: 在标准25吨挖掘机上，载荷估计达到1%的满量程精度，力测量达到13度的方向精度和383N的幅度精度。在两个不同类型和重量等级的挖掘机平台上验证了方法的准确性和泛化能力。

Conclusion: 该系统在准确性和精度方面优于经典的准静态方法和商用系统，具有高精度、可改造性和最小校准要求的优势。
...
TL;DR: ManiAgent是一个用于通用操作任务的代理架构，通过多代理协作实现从任务描述和环境输入到机器人操作动作的端到端输出，在复杂推理和长时程任务规划方面表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉-语言-动作模型在机器人操作中表现出强大能力，但在复杂推理和长时程任务规划方面受到数据稀缺和模型容量的限制。

Method: 提出ManiAgent代理架构，通过多代理间的通信来执行环境感知、子任务分解和动作生成，从而有效处理复杂操作场景。

Result: 在SimplerEnv基准测试中达到86.8%的成功率，在真实世界拾放任务中达到95.8%的成功率，能够高效收集数据，训练出的VLA模型性能可与基于人工标注数据集的模型相媲美。

Conclusion: ManiAgent通过代理架构有效解决了VLA模型在复杂操作任务中的局限性，为机器人操作提供了高效的端到端解决方案。
...
TL;DR: 提出结合学习世界模型与采样MPC的框架，用于人形机器人接触感知规划，在离线数据集上训练，支持多任务接触感知行为。
...
Method: 结合学习的世界模型与采样模型预测控制(MPC)，在无演示的离线数据集上训练，在压缩潜在空间中预测未来结果。使用学习的替代价值函数处理稀疏接触奖励和传感器噪声。

Result: 单一可扩展模型支持多种接触感知任务，包括扰动后墙壁支撑、阻挡来袭物体和穿越高度受限拱门，相比在线强化学习提高了数据效率和多任务能力。在物理人形机器人上实现实时接触规划。

Conclusion: 该框架实现了从本体感觉和自我中心深度图像的鲁棒实时接触规划，为人形机器人在非结构化环境中的自主操作提供了有效解决方案。
...
TL;DR: Phys2Real提出了一种实-仿-实强化学习流程，通过视觉语言模型推断物理参数并结合交互式自适应，有效解决机器人操作任务中的sim-to-real迁移挑战。


<details>
  <summary>Details</summary>
Motivation: 直接在真实世界中学习机器人操作策略成本高昂且耗时，而仿真训练的强化学习策略虽然可扩展，但精确动力学任务的sim-to-real迁移仍然困难。

Method: Phys2Real包含三个核心组件：基于3D高斯泼溅的高保真几何重建、VLM推断的物理参数先验分布、以及从交互数据进行的在线物理参数估计。该方法通过集成不确定性量化来精炼VLM预测。

Result: 在具有不同质心的T形块和具有偏心质量分布的锤子的平面推动任务中，Phys2Real相比域随机化基线取得显著改进：底部加重T形块成功率100% vs 79%，顶部加重T形块成功率57% vs 23%，锤子推动任务平均完成时间快15%。

Conclusion: 消融研究表明，VLM和交互信息的结合对于成功至关重要。该方法通过可解释的物理参数来条件化策略，有效提升了sim-to-real迁移性能。
