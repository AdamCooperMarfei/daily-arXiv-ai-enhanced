...
Method: 使用果园模拟训练控制器，通过腕戴相机获取光学流图像，引导切割器到达指定位置并垂直切割。

Result: 在仿真和现实环境中实现了30%的成功率，约为专家规划器性能的一半。
...
Method: 引入了一种广义的RWHEC问题求解算法，结合了全局最优性和局部求解器，并提供了开源实现。

Result: 该方法在性能上优于现有解决方案，并提供了全局最优性的理论保证。

Conclusion: 提出的算法在标定问题上表现出色，具有高效性和通用性，适用于实际应用。
...
TL;DR: 提出了一种新型感知代理，结合语音提示、大语言模型和基础跟踪模型，提升手术中的人机交互自然性。
...
Method: 利用语音集成提示工程的大语言模型、SAM和跟踪基础模型，结合记忆库和分割新元素的机制。
...
<details>
  <summary>Details</summary>
Motivation: 多任务强化学习（MTRL）需要通用且鲁棒的策略，而现有研究多限于低并行化的离策略方法。MTRL可以利用GPU加速模拟的大规模并行化优势，结合高渐进性能的在线策略算法。

Method: 提出MTBench基准，使用GPU加速模拟器IsaacGym实现70个任务，并整合四种基础RL算法和七种先进MTRL算法。

Result: 实验显示MTBench能快速评估MTRL方法，同时揭示了大规模并行化与MTRL结合的新挑战。
...
<details>
  <summary>Details</summary>
Motivation: 提升腿式机器人的稳定性和运动可塑性，尤其是在狭窄路径行走和动态稳定方面。

Method: 基于质心动力学模型设计控制器，利用QP求解器调节推进器和足部接触力，采用模型预测控制框架。

Result: 成功模拟了四足机器人在狭窄路径上的稳定行走，并验证了推进器在侧向推力恢复中的稳定作用。

Conclusion: 推进器辅助行走显著提升了机器人在复杂环境中的稳定性，为未来多模态机器人设计提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 可重构多机器人单元能应对波动的装配需求，但其配置的反复规划带来新挑战，特别是生成优化的多机器人运动序列以减少装配时间。

Method: 将装配步骤分为核心操作和连接操作，核心操作固定，连接操作可优化；采用分解式运动规划策略，结合采样启发式、树搜索和无梯度优化。

Result: 提出的方法生成高效无碰撞的多机器人装配流程，优于分散式机器人个体运动规划的基线方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于相机传感器（如RGB-D）的3DGS方法存在计算负载高、在低纹理或光照环境下失效以及操作范围短的问题，而LiDAR的引入又带来了全局对齐和高斯优化时间的挑战。

Method: GSFusion采用全局位姿图优化中的surfel-to-surfel约束确保地图一致性，并利用像素感知的高斯初始化策略和边界sigmoid约束处理稀疏数据。
...
<details>
  <summary>Details</summary>
Motivation: 当前触须传感器在机器人感知中缺乏基于直接接触信息的主动控制，难以精确重建物体轮廓。

Method: 构建磁导触须传感器，利用梯度下降提取接触点，并通过贝叶斯滤波减少波动；提出主动运动控制策略，使用B样条曲线预测表面曲率。

Result: 算法能有效跟踪物体并实现亚毫米级精度的轮廓重建。

Conclusion: 方法在仿真和实际实验中验证了其有效性，适用于不同物体的表面跟踪。
...
<details>
  <summary>Details</summary>
Motivation: 确保自动驾驶车辆在伦理挑战性场景中做出符合人类期望的决策，避免现有刚性规则系统导致的冲突。

Method: 提出了一种基于原因的轨迹评估框架，量化人类代理的原因（如法规遵从性），并通过可调权重和平衡函数评估轨迹。

Result: 通过实际超车场景展示了框架如何揭示法规遵从性、效率和舒适性之间的冲突。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有强化学习方法在漂移控制中仿真与现实的显著差距问题。

Method: 采用GPU加速的并行仿真和系统域随机化技术。

Result: 在仿真和真实1/10比例IWD RC车上验证了方法的有效性，实现了复杂操作中的精确轨迹跟踪和可控侧滑角。
...
<details>
  <summary>Details</summary>
Motivation: 满足农业环境中自主移动机器人导航的需求，特别是在无序坐标中高效规划路径，同时减少移动距离并遵守曲率约束。

Method: 结合基于DTSP的全局路径规划和NMPC的局部路径规划与控制，生成最短且曲率受限的路径。

Result: 仿真验证表明，该框架比解耦方法路径更平滑、更短（减少约16%），NMPC控制器能有效引导机器人到达目标点。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于低齿轮比、可反向驱动的执行器的仿真到现实转移方法在差距扩大时失效，需要更有效的方法。

Method: 重新解释PID增益为复杂未建模动力学的替代，通过实验测量有效比例增益，并在训练中正则化神经控制器的输入输出敏感性。

Result: 在带有110:1齿轮箱的两轮平衡机器人上，增益正则化的参数条件化RNN实现了与仿真接近的硬件性能。
...
<details>
  <summary>Details</summary>
Motivation: 机器人模仿学习面临大规模高质量数据的稀缺问题，跨具身数据集训练存在形态和动作空间多样性挑战。

Method: 采用两阶段训练：预训练于大规模人类操作数据，跨具身微调于机器人数据，基于扩散变换器架构和流匹配。
...
Method: 构建UX数据集，开发基于Transformer的模型，利用面部表情和语音信号，通过多实例学习框架捕捉交互的短期和长期模式。

Result: 实验结果表明，该方法在UX评估上优于第三方人工评估。

Conclusion: 提出的方法能更全面地捕捉UX的时序动态，为人机交互中的用户体验评估提供了更有效的解决方案。
...
Method: 系统评估多种最先进语言模型，通过PDDL文件直接提示，并与Fast Downward规划器在多个基准测试中比较。
...
<details>
  <summary>Details</summary>
Motivation: 由于用户解剖学差异和佩戴不一致，手部外骨骼的精确跟踪面临挑战，导致运动学偏差和性能下降。

Method: 使用冗余关节传感和残差加权优化策略估计虚拟链接参数，并通过运动捕捉数据驱动调优成本函数权重。

Result: 在七名受试者中，该方法显著降低了关节和指尖跟踪误差，优于未校准和均匀加权模型。
...
<details>
  <summary>Details</summary>
Motivation: 改进原版DRACo-SLAM，解决水下环境中多机器人闭环检测的效率问题，适应水下扫描匹配需求。

Method: 引入对象图表示和匹配，提出增量GCM方法替代PCM，处理共享相似配准误差的闭环场景。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统点云配准方法在长期环境变化中鲁棒性不足的问题，提高动态场景下的定位可靠性。

Method: 提出DuLoc方法，结合LiDAR-惯性里程计与离线地图定位，引入恒定速度运动模型以减少噪声。

Result: 在大规模动态港口环境中，DuLoc优于其他先进LiDAR定位系统。

Conclusion: DuLoc在复杂动态环境中表现出色，为自动驾驶系统提供了更可靠的定位解决方案。
...
Method: 利用预训练的深度立体网络估计深度，通过多损失策略优化3D高斯，提升几何一致性和视觉质量。

Result: 在多个数据集上的实验表明，BGS-SLAM在复杂户外环境中比其他3DGS方案具有更高的跟踪精度和建图性能。

Conclusion: BGS-SLAM填补了户外3DGS-SLAM的空白，展示了在仅使用RGB立体图像时的优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 探索潜在动作在视觉-语言-动作（VLA）模型中的作用，以增强机器人操作策略的泛化能力。

Method: 改进潜在动作的学习方法，并将其融入VLA预训练中，形成ViLLA-X框架。
...
<details>
  <summary>Details</summary>
Motivation: 解决RL模型在视觉运动代理中过拟合特定任务或环境的问题，探索其在3D世界中泛化空间推理和交互能力的潜力。

Method: 采用跨视图目标规范作为统一多任务目标空间，提出自动任务合成和分布式RL框架。

Result: RL将交互成功率提升4倍，实现跨环境的零样本空间推理泛化。
...
<details>
  <summary>Details</summary>
Motivation: 微型机器人受限于尺寸、重量和功耗，难以实现类似自然系统的触觉感知能力。

Method: 设计了一种多段柔性层压传感器CITRAS，嵌入电容角度传感器，通过被动弯曲响应环境刺激。

Result: CITRAS在静态和动态弯曲中误差极小，能准确预测距离、估计间隙宽度并区分表面纹理。
