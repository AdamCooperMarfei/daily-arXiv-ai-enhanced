...
Motivation: 解决通信能力有限（发送速率和包载荷受限）的多智能体系统在高度部分可观测环境中协调的挑战，特别是处理需要频繁重新分配任务的动态场景和非对称障碍物

Method: 基于市场机制的任务分配方法，提出新颖的分布式协调算法，专门考虑非对称障碍物特性，适用于低通信场景

Result: 在仿真和真实NAO机器人RoboCup比赛中验证，在有限通信环境下显著减少任务重叠，最频繁重新分配任务减少52%

Conclusion: 该架构能有效处理主动非对称障碍物、通信条件差和部分可观测环境的复杂场景，提升了分布式多智能体系统的协调效率
...
<details>
  <summary>Details</summary>
Motivation: 传统复合材料制造方法难以实现复杂三维结构，并需组装多个部件导致缺陷，同时维持连续纤维增强也面临挑战。

Method: 采用3D纤维缘缆(3DFiT)技术，使用连续单纽纤维构建面心立方格子结构，确保精确的纤维排列和消除传统组装缺陷。

Result: 制造的无人机架体仅重260克，比DJI F450商业架体轻10%，比强度是金属和热性塑料的4-8倍，飞行时间延长3分钟，稳定性和耐用性经过飞行测试验证。

Conclusion: 单纽格子架构无人机架体具有很大潜力，3DFiT技术作为一种可扩展的高效制造方法，为航空航天和机器人领域的轻量化高强度复合材料结构提供了新的解决方案。
...
Method: 将运动流场表示为动态系统，用Koopman算子参数化来模仿期望轨迹，利用学习到的流场的发散特性获得平滑的运动场，使机器人偏离期望轨迹时能收敛到参考轨迹并跟踪至终点。

Result: 在LASA手写数据集和3D机械臂末端轨迹数据集上评估显示高效性，仅需3%的LASA数据就能生成密集运动规划。在物理机器人实验中，在非静态流体环境中验证了有效性，在时空动态建模指标上显著优于基线方法。

Conclusion: KoopMotion方法具有高度样本效率，能够有效生成收敛到期望轨迹的运动规划，在复杂环境中表现出良好的性能。
...
TL;DR: 提出了一种欠驱动变形加载机械臂(UMLM)，将变形臂与被动自适应夹爪结合，通过几何约束实现拓扑重构和灵活运动轨迹，无需额外驱动器。


<details>
  <summary>Details</summary>
Motivation: 解决传统固定自由度加载机构存在的执行器过多、控制复杂、对动态任务适应性有限的问题。

Method: 集成变形臂和被动自适应夹爪，利用几何约束实现拓扑重构，进行运动静力学分析，使用粒子群优化(PSO)优化夹爪尺寸参数。

Result: 仿真验证了UMLM易于实现的控制策略、操作多功能性以及在动态环境中抓取多样化物体的有效性。

Conclusion: 欠驱动变形机构在需要高效自适应加载解决方案的应用中具有实际潜力，所提出的建模和优化框架可扩展到更广泛的机械臂类别。
...
<details>
  <summary>Details</summary>
Motivation: 解决双足机器人在非结构化户外环境中遇到的复杂地形和外部干扰导致的稳定性挑战

Method: 采用LIPM奖励设计来调节质心高度和躯干方向，使用奖励融合模块动态调整运动与稳定性的权衡，采用双评利结构分别评估稳定性和运动目标

Result: 在模拟和实际户外环境中验证，展现了优异的地形适应能力、干扰拒绝能力和在广泛速度与感知条件下的一致性能

Conclusion: 该方法有效解决了双足机器人在复杂户外环境中的稳定感知运动问题，为野外自主运动提供了可靠解决方案
...
<details>
  <summary>Details</summary>
Motivation: 解决无人机LiDAR感知的局限性：窄视场角、载荷限制，以及传统固定转速扫描系统缺乏场景感知和任务适应性，导致在复杂遮挡环境中里程计和建图性能下降

Method: 混合架构结合模型预测控制(MPC)和强化学习(RL)：分析不确定性模型预测未来位姿可观测性，轻量神经网络从全景深度表示学习隐式代价图指导探索

Result: 在仿真和真实环境中的大量实验表明，AEOS相比固定速率、纯优化和全学习基线方法显著提高了里程计精度，同时保持实时性能

Conclusion: AEOS框架通过生物启发和计算高效的混合方法，成功解决了无人机LiDAR感知的适应性挑战，实现了从仿真到现实的迁移
...
<details>
  <summary>Details</summary>
Motivation: 解决在动态不确定环境中自主代客停车的挑战，现有方法仅依赖即时观测或静态假设，无法准确预测未来停车位可用性

Method: 提出概率性停车位占用估计器，考虑有限视野下的部分和噪声观测，以及未观测区域的不确定性演化；设计策略规划器，根据信息收益动态平衡目标导向的停车操作和探索导航，并在有前景停车位采用等待-行动行为

Result: 通过模拟大型停车场环境的随机模拟实验，证明该框架在停车效率、安全距离和轨迹平滑性方面显著优于现有方法

Conclusion: 该方法通过显式预测未来停车位可用性和智能策略规划，有效提升了自主代客停车在动态不确定环境中的性能表现
...
<details>
  <summary>Details</summary>
Motivation: 户外环境中四足机器人的视觉定位面临环境预测不准确和深度传感器噪声等挑战，严重限制了算法的实际应用。需要解决视觉运动控制中的部署稳定性问题。

Method: 采用双估计器架构（Redundant Estimator Network），通过在线估计器自适应实现估计模块间的无缝切换，处理视觉感知不确定性。

Result: 在真实机器人上的实验验证表明，该框架在复杂户外环境中有效，特别是在视觉感知退化场景中表现出优势。

Conclusion: 该框架展示了在具有挑战性的野外条件下实现可靠机器人部署的潜力，是解决视觉定位部署问题的实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于MLLM的具身系统存在两个关键问题：几何适应性差距（2D输入或硬编码3D几何注入导致空间信息不足或泛化受限）和具身约束差距（忽视真实机器人的物理约束，导致理论上有效但实际不可行的任务规划）。

Method: 提出了两个核心创新：(1)任务自适应的3D接地机制，使用门控路由器根据上下文需求进行选择性3D融合；(2)具身感知推理框架，将任务目标和具身约束共同纳入推理循环。

Result: 在广泛的实验评估中，OmniEVA不仅实现了最先进的通用具身推理性能，还在多种下游场景中表现出强大的能力，包括原始任务和复合任务的基准测试。

Conclusion: OmniEVA通过创新的3D接地和具身感知推理方法，有效解决了现有MLLM具身系统的局限性，展现出强大且通用的规划能力，为具身智能发展提供了重要进展。
...
<details>
  <summary>Details</summary>
Motivation: 当前人形机器人平台大多闭源或成本高昂，需要开发一个既高性能又易于获取的开源解决方案。

Method: 使用现成的可反向驱动驱动器和高功率密度标准电子元件，设计轻量化结构（110cm/14.5kg），无需龙门架即可单人操作。

Result: 通过行走、跳跃、冲击缓解和起身等实验验证了机器人的可行性。

Conclusion: AGILOped成功提供了一个开源、高性能且易于使用的人形机器人平台，适合研究用途。
...
<details>
  <summary>Details</summary>
Motivation: 传统VLA模型依赖大规模视觉-语言模型预训练，训练成本高昂。本文旨在降低对大规模VLM的依赖，减少预训练需求

Method: 提出VLA-Adapter范式，系统分析不同VL条件的有效性，设计轻量级策略模块和桥接注意力机制，自动注入最优条件到动作空间

Result: 仅使用0.5B参数主干网络，无需机器人数据预训练，在仿真和真实机器人基准测试中达到SOTA性能，推理速度最快，单消费级GPU仅需8小时训练

Conclusion: VLA-Adapter显著降低了VLA模型的部署门槛，为高效视觉-语言-动作建模提供了新的轻量级解决方案
...
<details>
  <summary>Details</summary>
Motivation: 线驱动连续体机器人在长期使用中受到机械疲劳影响，包括塑性变形和材料降解，影响性能并带来结构失效风险。目前疲劳估计方面研究较少，限制了长期运行。

Method: 提出三项关键创新：(1)混合轴导-梁结构，通过TwistBeam和BendBeam解耦扭扭和弯曲，被动转动副减少应力集中；(2)被动限位器，通过机械约束安全限制运动，利用电机扭矩感知检测极限扭矩；(3)实时疲劳感知方法，从极限位置的电机扭矩估计刚度，无需额外传感器。

Result: 实验结果显示，与传统设计相比，提出的设计将疲劳积累减少约49%，被动机械限制结合电机侧感知能够准确估计结构疲劳和损坏。

Conclusion: 这些结果证实了提出的架构对于安全可靠的长期运行的有效性，为连续体机器人的疲劳感知和长期稳定性提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决变形袋子复杂和不可预测性带来的工业袋装挑战

Method: 使用GMM估计SOI状态，优化技术生成SOI，CBiRRT运动规划，MPC双臂协调

Result: 系统能够在各种物体上进行精确和稳健的袋装操作，展现良好的适应性

Conclusion: 为机器人变形物体操作提供了新的解决方案，特别是在自动化袋装任务中
...
Method: 开发了开源硬件平台SMapper，集成了同步LiDAR、多摄像头和惯性传感，建立了稳健的校准和同步管道。发布了SMapper-light数据集，包含同步多模态数据和离线LiDAR SLAM生成的高精度地面真实轨迹。

Result: 构建了完整的开源硬件平台和数据集，提供了亚厘米级精度的地面真实数据，并在最先进的LiDAR和视觉SLAM框架上进行了基准测试。
...
<details>
  <summary>Details</summary>
Motivation: 在边缘平台上部署滑动检测系统面临能量约束挑战，需要开发低功耗的初期滑动检测技术来增强机器人操作安全性

Method: 使用带有突出乳头状皮肤的NeuroTac传感器和脉冲卷积神经网络(SCNN)进行滑动状态分类，通过传感器运动诱导滑动条件，并对最终层脉冲计数进行时间平滑处理

Result: SCNN模型在三类滑动状态(无滑动、初期滑动、明显滑动)分类中达到94.33%准确率；在动态重力诱导滑动验证条件下，系统在所有试验中都能在明显滑动发生前至少360毫秒检测到初期滑动

Conclusion: 该神经形态系统具有稳定且响应迅速的初期滑动检测能力，为机器人安全操作提供了有效的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 传统的图像相对导航方法存在局限性：图像与代理的姿态和体验绑定，而对象作为地图属性提供了不受体验和轨迹影响的世界表征。需要一种能够解耦控制预测与图像匹配问题、支持新路径遍历且具有高跨体验变化性的方法。

Method: 提出了一种基于"对象相对控制"的新范式，使用"相对3D场景图"作为顶度地图表征来获取更信息丰富的对象级全局路径规划成本。训练了一个名为ObjectReact的本地控制器，直接条件化在高级"对象路径成本图"表征上，消除了显式RGB输入的需求。

Result: 在传感器高度变化和多种导航任务中，对象相对控制方法在基础空间理解能力方面显著优于图像相对方法，包括可以逆向导航地图轨迹。仅使用模拟训练的策略能够艾好地沿用到实际室内环境。

Conclusion: 对象相对控制为视觉导航提供了一种更加稳健和可以沿用的方法，充满了图像相对方法的局限性。通过使用对象级表征和相对3D场景图，该方法能够实现更好的跨体验变化性、更强的空间理解能力以及从模拟到实际的良好沿用性。
...
<details>
  <summary>Details</summary>
Motivation: 现有治疗和社交机器人主要模仿外观和关节运动，但忽视了生物体全身膨胀收缩的体积变化运动对生命感感知的影响

Method: 开发MOFU机器人，采用Jitterbug几何变换机制实现单电机驱动的全身膨胀收缩，通过在线视频调查使用Godspeed问卷评估不同运动模式对生命感感知的影响

Result: 膨胀收缩运动显著提高感知生命感；两个机器人相比单个机器人未显示显著差异；膨胀收缩与运动结合比单独运动获得更高生命感评分
...
<details>
  <summary>Details</summary>
Motivation: 现有方法采用重定向、跟踪和残差校正的三阶段工作流程，导致演示数据利用不足和误差累积，限制了运动捕捉数据在灵巧机器人操作中的直接应用

Method: 联合执行重定向和跟踪的统一单循环优化，将演示作为软指导而非绝对真值，从原始轨迹推导自适应空间范围，使用强化学习训练策略保持在范围内同时最小化控制努力完成任务

Result: 统一表述保留了演示意图，使机器人特定策略涌现，提高对噪声的鲁棒性，并能扩展到大规模演示语料库
...
<details>
  <summary>Details</summary>
Motivation: 解决VLA模型两大挑战：大规模人类操作轨迹数据的稀缺和高成本，以及在分布偏移任务上的沿用性限制

Method: 基于veRL构建SimpleVLA-RL框架，包含VLA特定轨迹采样、可扩展并行化、多环境渲染和优化损失计算，并提出探索增强策略

Result: 在LIBERO上达到最先进性能，在RoboTwin 1.0&2.0上超过基线模型，实际任务上显著超过SFT方法，发现了新的"pushcut"现象

Conclusion: RL能够有效提升VLA模型的长时间谜划能力，减少对大规模人类数据的依赖，并实现更好的沿用性，为机器人操作领域提供了新的训练方法
