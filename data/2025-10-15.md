...
Motivation: 尽管毫米/微米机器人在过去二十年取得显著技术进展，但大多数仍局限于实验室概念验证，缺乏实际临床应用可行性。需要弥合技术创新与现实应用之间的差距。

Method: 引入战略性的毫米/微米机器人技术就绪水平框架(mTRL)，该系统开发从初始概念化到临床采用，通过明确定义的里程碑和逐步活动来映射。

Result: mTRL模型提供了技术成熟度的结构化衡量标准，为跨学科合作提供共同语言，并为加速转化发展提供可操作指导。

Conclusion: 该领域的长期影响和可持续性取决于将开发与未满足的医疗需求对齐，确保应用可行性，并无缝集成到现有临床工作流程中，这些是实现有意义的患者结果的基本支柱。
...
<details>
  <summary>Details</summary>
Motivation: 虽然基于地标语义注册的方法在全局定位中表现优于纯几何方法，但地标可能重复且误导对应关系建立，需要解决这一问题

Method: 使用高斯过程学习语义分布的连续函数，构建包含对象层、连续函数层和度量语义层的三层3D场景图作为轻量级后端
...
<details>
  <summary>Details</summary>
Motivation: 越野自主车辆需要在实时规划中同时考虑曲率可行路径、空间变化的土壤强度和坡度危险，以在非结构化环境中实现可靠自主导航。

Method: 使用连续状态-成本度量结合贝克尔压力-沉降模型和坡度惩罚，在格网上评估精确转向基元，采用车辆动力学RRT*进行全局探索，车辆动力学D* Lite进行局部修复。

Result: 硬件试验表明，该框架能够在软土和坡度过渡区域实现实时导航，支持非结构化环境中的可靠自主性。

Conclusion: 通过将地形-车辆模型与规划器分离，该框架为可变形地形中的确定性、采样或学习驱动规划提供了可重用的基础。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶车辆安全性评估需要多样化的安全关键场景，特别是碰撞场景，但在现实中收集既危险又罕见。仿真生成可控的碰撞场景具有挑战性。

Method: 构建COLLIDE大规模碰撞场景数据集，提出预测碰撞模式的框架，该模式捕捉碰撞时车辆空间配置，然后生成完整对抗轨迹。

Result: 方法在碰撞率和可控性上优于基线，生成的场景能诱导更高规划器失败率，揭示现有规划器局限性，并能用于提升规划器鲁棒性。

Conclusion: 提出的可控碰撞场景生成方法能有效评估和改进自动驾驶规划器，为不同碰撞场景下的安全部署做出贡献。
...
Method: 开发了结合数据驱动密度奖励和基于规则目标的框架，使用采样前瞻控制器生成监督动作，然后蒸馏为紧凑的学生策略。

Result: 在合成和电梯共乘仿真中相比基线方法在成功率和时间效率上有显著提升，真实世界实验验证了部署可行性。

Conclusion: 该框架有效平衡了导航的适应性和安全性，为动态人类环境中的机器人导航提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的VLA模型主要基于2D数据预训练，缺乏准确的空间感知能力，限制了在3D物理世界中的操作能力。现有方法使用显式3D传感器输入或深度估计器，但面临传感器噪声、硬件异构性和数据集深度覆盖不完整等问题。

Method: SF是一种简单有效的对齐策略，将VLA模型的中间视觉嵌入与预训练3D基础模型生成的几何表示进行对齐，通过在中间层强制对齐来引导VLA编码更丰富的空间表示。

Result: 在仿真和真实环境中的广泛实验表明，SF达到了最先进的性能，超越了基于2D和3D的VLA模型。SF进一步将训练速度提高了3.8倍，并在各种机器人任务中提高了数据效率。

Conclusion: SF方法通过隐式强制VLA模型发展空间理解能力，无需依赖显式3D输入或深度估计器，有效提升了机器人执行精确动作的能力。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统仅尖端控制在腔内手术（如支气管镜检查）中因复杂解剖结构导致的壁接触、组织创伤或无法到达远端目标的问题，提高手术导航的安全性和精确性。

Method: 结合物理信息骨干模型与增强神经ODE进行残差学习，实现精确形状估计；采用基于采样的MPPI控制器联合优化尖端跟踪、骨干顺应性和避障；任务管理器支持实时调整目标参数。

Result: 仿真研究显示毫米级精度，在轨迹跟踪、动态避障和形状约束到达等场景表现优异；真实机器人实验在支气管镜模型上验证了框架有效性，显示改进的管腔跟随精度、减少的壁接触和增强的适应性。

Conclusion: 该框架在微创腔内手术中具有提高安全性、可靠性和操作效率的潜力，并可广泛应用于其他受限和安全关键环境。
...
<details>
  <summary>Details</summary>
Motivation: 解决劳动力短缺、人口老龄化和生产需求增长等复杂社会挑战，推动工业环境中机器人的应用发展。

Method: 采用跨学科方法，整合学术研究、实用专业知识和体验性知识，在飞机发动机维修和维护操作中探索协作机器人的潜力。

Result: 正在进行中的多方面努力，旨在探索协作机器人在特定工业场景中的应用可行性。

Conclusion: 跨学科方法对于成功将机器人技术整合到工作场所至关重要，能够优先考虑工人福祉和工作吸引力等价值。
...
<details>
  <summary>Details</summary>
Motivation: 为了模拟人类行走，机器人需要在未知空间中准确踩踏看到的台阶位置，现有行走方案在楼梯攀爬方面存在局限。

Method: 通过LiDAR、RGB-D相机和IMU多传感器融合，构建实时多边形楼梯平面语义地图，并基于这些多边形平面段进行脚步规划。

Result: 在NVIDIA Orin上部署，实现20-30Hz全身运动规划输出，室内外真实场景实验表明方法高效且鲁棒。

Conclusion: PolyMap框架为人形机器人楼梯攀爬提供了有效的感知驱动规划解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 在机器人强化学习中，每个技能通常都是从零开始学习，但属于同一机器人体现的任务特定策略之间很可能存在可泛化的共享知识。

Method: 使用任务无关的探索性数据收集算法获取多样化动态转移数据，通过监督学习训练本体感觉逆动力学模型(PIDM)，然后将预训练权重加载到actor和critic网络中热启动策略优化。

Result: 在7个不同的机器人运动控制任务上验证，相比随机初始化，平均提高样本效率40.1%，任务性能7.5%。

Conclusion: 提出的预训练初始化策略在机器人强化学习中具有显著优势，通过共享知识提高了学习效率和性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统轨迹生成方法通常优先考虑效率，但往往无法让人类清晰理解机器人意图；现有可读性运动方法通常只生成单一"最可读"轨迹，忽视了在不同情境下调节意图表达性的需求。

Method: 基于信息势场建模方法为轨迹分配连续可读性分数，并构建两阶段扩散框架：首先生成指定可读性水平的路径，然后将其转换为可执行的机器人动作。

Result: 在2D和3D到达任务中的实验表明，该方法能产生具有不同可读性程度的多样化可控运动，同时实现与最先进方法相当的性能。

Conclusion: 所提出的框架成功实现了机器人运动可读性的连续可控调节，为人类-机器人交互中的意图表达提供了灵活解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决生成行为克隆中扩散策略的随机性错误和开环控制的延迟响应问题，提高机器人学习的稳定性和适应性

Method: 1. 自引导技术：利用过去观察提高动作保真度，隐式促进未来感知行为；2. 自适应分块：在反应性收益超过时间一致性需求时选择性更新动作序列

Result: 在广泛的模拟和真实世界机器人操作任务中，该方法显著提升了生成行为克隆的性能

Conclusion: 所提出的自引导和自适应分块技术有效增强了扩散策略的一致性和反应性，为生成行为克隆提供了更可靠的解决方案
...
Method: 通过强化学习、行为克隆等基础方法，发展到通用语言条件模型，提供概念理解和实用工具，并在lerobot中实现即用示例。

Result: 解锁了自主系统的前所未有的能力，为研究人员和从业者提供了贡献机器人学习发展所需的知识和工具。

Conclusion: 本教程旨在指导研究人员和从业者掌握现代机器人学习的关键概念和实践工具，推动该领域的发展。
...
Conclusion: 通过实验验证了所提出触觉传感器的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 在人机协作环境中，安全机制通常会阻碍任务效率，因为人类干预会导致机器人备份运动和目标失败。频繁的运动重规划会增加计算负担和失败几率。

Method: 采用混合RL规划框架，包含交互式运动规划器和RL任务规划器。RL任务规划器基于运动规划器的反馈选择统计上安全高效的任务序列，运动规划器通过检测人体手臂运动并在路径失效时部署新路径来保持任务执行过程无碰撞。

Result: 在仿真和真实世界的协作机器人上验证，与硬编码任务运动规划方法相比，该框架能够：1）在关节和任务层面响应不确定的人体运动；2）减少重复失败目标命令的次数；3）减少总重规划请求次数。

Conclusion: 提出的混合RL规划框架能够有效平衡人机协作环境中的安全性和效率，通过学习避免危险任务并确保所选任务的安全性。
...
<details>
  <summary>Details</summary>
Motivation: 现有机器人策略在高速操作和资源受限系统中存在效率问题，需要一种能够同时处理多模态动作预测且计算高效的新方法。

Method: 基于两个核心组件：采用能量分数作为学习目标以促进多模态动作建模，并引入能量MLP来实现该目标，同时保持架构简单高效。

Result: 在模拟环境和真实机器人任务中的实验表明，Energy Policy在性能上匹配或超越最先进的操作方法，同时显著降低计算开销。在MimicGen基准测试中，以更快的推理速度实现了更优性能。

Conclusion: Energy Policy框架为高频机器人操作提供了一种高效解决方案，在保持高性能的同时大幅提升了计算效率。
...
<details>
  <summary>Details</summary>
Motivation: 果园修剪是重要的农业实践，但现有研究主要关注感知挑战，而忽略了操纵的复杂性，特别是在关节空间和笛卡尔空间中规划末端执行器穿过复杂树枝的路径规划问题。

Method: 制定了高维机械臂在修剪场景中的规划问题，研究了系统的内在冗余性，提出了一个整合感知、建模和整体规划的完整修剪工作流程。

Result: 实验表明更全面的规划方法能显著提升机器人操纵器的性能，并在真实机器人上实现了所提出的工作流程。
...
<details>
  <summary>Details</summary>
Motivation: 视觉触觉传感器在提取物体物理属性时产生大量冗余信息，单维提取缺乏有效融合，无法充分表征物体属性，限制了识别精度的提升。

Method: 采用双流网络特征提取和融合策略：一个流通过三维重建获取深度图信息，另一个流通过测量接触力数据获取硬度信息；使用CNN提取特征后进行加权融合。

Result: 在标准测试中：力预测误差0.06N（12N范围内），硬度识别准确率98.0%，形状识别准确率93.75%；融合算法在实际抓取场景中物体识别准确率超过98.5%。
...
Method: 结合现有最先进控制范式的优势，开发神经增强反馈控制器，通过非线性反馈结构和学习过程实现控制性能提升。

Result: 控制器能够准确跟踪高度攻击性轨迹，在强干扰环境下仍保持鲁棒性和跟踪性能，计算效率高，无需额外训练即可直接部署到真实平台。

Conclusion: 该神经增强反馈控制器成功解决了现有控制范式的局限性，为高速敏捷飞行提供了具有稳定性保证、高效计算和直接部署能力的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前自主工具设计框架仅依赖性能优化，未考虑代理对工具重复使用的置信度，而史前人类设计石制工具时不仅追求目标完成精度，还注重工具在相似环境下的使用信心，这促使了工具鲁棒性的提升。

Method: 定义了任务条件化的自主手工具设计优化框架，引入神经启发的控制置信度项到优化流程中，使用基于CMAES的进化优化策略进行工具设计。

Result: 通过机器人手臂的严格仿真表明，以控制置信度为目标函数设计的工具在环境不确定性下比纯精度驱动设计的工具更具鲁棒性，且在控制扰动下在鲁棒性和目标精度之间取得平衡。

Conclusion: CMAES进化优化策略在自主工具设计中优于其他最先进优化器，能以最少迭代次数设计出最优工具，控制置信度的引入提升了工具设计的鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 解决机器人在与人类协作时，如何在保证自身任务完成的同时最小化对人类自主性的干扰。

Method: 结合最大适应性和最小可调反馈两种能力：最大适应性让机器人能够在线调整策略以利用人类行为进行合作；最小可调反馈仅在必要时请求人类合作以保证任务进展。

Result: 在真实世界积木操作任务和Overcooked-AI基准测试中验证，该方法产生了超越现有方法的丰富涌现合作行为，同时保持强大的形式化保证。

Conclusion: 该框架能够在最小化人机干扰、保护人类自主性的同时，确保机器人任务的持续满足，即使在目标冲突的情况下也能有效工作。
...
<details>
  <summary>Details</summary>
Motivation: 传统轮式机器人在非结构化和陡峭地形中的局限性促使采用腿式机器人，其在月球恶劣地形中具有更好的移动性和适应性。建立永久月球基地需要能够在月球恶劣地形中导航和操作的机器人平台。

Method: 采用约束强化学习框架，整合全身运动和操控能力，明确处理碰撞避免、动态稳定性和能效等关键安全约束，适应月球特定的低重力和不规则地形条件。

Result: 实现了精确的6D任务空间末端执行器姿态跟踪，平均位置精度为4厘米，方向精度为8.1度。系统始终尊重软硬约束，表现出针对月球重力条件优化的自适应行为。

Conclusion: 该工作有效桥接了自适应学习与关键任务安全要求，为未来月球任务的先进自主机器人探索者铺平了道路。
...
<details>
  <summary>Details</summary>
Motivation: 预训练的视觉-语言-动作模型虽然向通用机器人迈出了一大步，但在现场高效适应新特定任务仍然存在重大障碍。强化学习适应过程效率低下，阻碍快速任务掌握。

Method: 采用双路径架构：1) 失败驱动反思RL路径，利用VLM因果推理从失败分析自动合成密集奖励函数；2) 成功驱动质量引导SFT路径，识别并选择性模仿高质量成功轨迹，防止奖励黑客问题。

Result: 在挑战性操作任务实验中，该框架相比代表性基线实现了更快的收敛速度和更高的最终成功率。
...
<details>
  <summary>Details</summary>
Motivation: MPC提供可解释、可调的控制但受限于模型失配和实时计算约束；RL能产生鲁棒行为但缺乏解释性且需要大量奖励工程。需要结合两者优势。

Method: 开发了运动学全身MPC公式，在100Hz下并行评估数千个智能体进行RL训练。残差策略学习对MPC输出进行针对性修正。

Result: 相比独立MPC或端到端RL，该方法实现了更高的样本效率、更大的渐近奖励、更宽的速度命令跟踪范围，并能零样本适应未见步态和不平地形。

Conclusion: 该方法成功结合了MPC和RL的优势，模型控制先验作为强偏置引导策略学习，实现了高效鲁棒的控制性能。
...
Method: 提出T(R,O)图统一表示方法，结合图扩散模型和高效逆运动学求解器，支持无条件和条件抓取合成。

Result: 在多种灵巧手上平均成功率94.83%，推理速度0.21秒，吞吐量41抓取/秒，显著优于现有基线方法，且内存消耗显著降低。
...
<details>
  <summary>Details</summary>
Motivation: 在复杂城市环境中进行安全可解释的运动规划需要考虑双向多智能体交互，现有方法需要设计复杂的成本函数来编码期望的车辆行为，这在考虑广泛复杂场景时非常困难。

Method: 提出HYPE混合规划器，集成来自学习提案模型的多模态轨迹提案作为MCTS细化的启发式先验，并引入自我条件占用预测模型来建模双向交互。

Result: 在nuPlan和DeepUrban大规模真实世界基准测试中，HYPE实现了最先进的性能，特别是在安全性和适应性方面。

Conclusion: HYPE通过提案驱动的指导显著简化了细化过程中的成本函数设计，仅需要最小化的基于网格的成本项，有效解决了复杂城市环境中的运动规划问题。
