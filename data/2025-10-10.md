...
Method: 使用LLM前端生成任务图（包含持续时间和优先级）和机器人-任务适应度矩阵，形式化后端解决最小化完工时间问题，机器人执行具有自主闭环控制的子任务。

Result: 在多个自由形式语言引导的自主协调基准测试中，FLEET在异构任务的两智能体团队上比最先进的生成规划器成功率更高。消融实验显示MILP主要改善时间结构，而LLM衍生的适应度对能力耦合任务至关重要。

Conclusion: FLEET框架成功将自然语言转化为优化的多机器人调度，在异构机器人团队协调方面表现出色，并通过硬件试验验证了其在实际挑战中的适用性。
...
<details>
  <summary>Details</summary>
Motivation: 自主驾驶应用通常基于现有车辆开发，但缺乏详细的车辆结构信息，导致在信息稀缺条件下难以建立准确的车辆动态模型。

Method: 使用门控循环单元构建轻量级编码器-解码器模型，通过车辆历史状态数据和驾驶员控制动作来预测未来状态，完全数据驱动且不受物理约束。

Result: 模型在极端动态条件下最大平均相对误差低于2.6%，对噪声输入具有良好的鲁棒性，输出信号（纵向/横向加速度、横摆角速度、纵向速度）具有物理一致性。

Conclusion: 该方法为信息稀缺条件下的车辆动态建模提供了有效解决方案，模型性能优异且具有物理合理性。
...
<details>
  <summary>Details</summary>
Motivation: 传统解析求解器受限于低自由度和特定拓扑结构，数值优化方法计算成本高且容易陷入局部最小值，需要开发更高效准确的逆运动学求解器。

Method: 使用GPU加速结合采样和优化，采用方向感知的贪婪坐标下降初始化方案和雅可比矩阵优化程序，实现混合求解策略。

Result: 相比现有技术，在收敛速度和整体精度方面均有提升，在精度-延迟帕累托前沿上持续找到解决方案，通常实现数量级增益，并产生高质量样本的广泛分布。
...
<details>
  <summary>Details</summary>
Motivation: 灵巧操作任务需要在不同接触模式间切换，传统方法将任务分解为独立子任务分别优化，这限制了性能且计算成本高昂。独立优化可能导致系统陷入难以推进后续子任务的状态。

Method: AVO引入学习价值函数预测总未来任务性能，将该价值函数纳入轨迹优化的成本函数中，通过价值函数梯度引导优化器朝向最小化未来子任务成本的状态。

Result: 在螺丝刀抓取和转动任务的仿真和真实实验中验证，即使计算预算减少50%，相比无价值函数的轨迹优化仍能获得改进性能。

Conclusion: AVO有效桥接独立优化的子任务，通过减少在线计算需求加速优化过程，在灵巧操作任务中表现出优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 基础设施老化和复杂性增加使得高效检测规划更加重要。现有基于采样的检测规划器虽然快速但内存消耗大，特别是在处理大型复杂结构时。虽然隐式模型（如神经SDF）能高效表示复杂结构，但现有规划器原语计算主要针对显式模型设计。

Method: 开发了IPIM（基于隐式模型的检测规划原语）计算集，使基于采样的检测规划器能够在整个规划过程中完全使用神经SDF表示，无需在隐式和显式模型间频繁转换。

Result: 在三个场景（包括具有9200万个三角网格面的复杂真实结构）的评估表明，即使使用基本的基于采样规划器配合IPIM，也能生成与最先进规划器质量相当的检测轨迹，同时内存使用减少高达70倍。

Conclusion: IPIM方法成功解决了基于采样检测规划器在大型复杂结构中的内存效率问题，通过完全利用神经SDF表示实现了显著的内存节省，同时保持检测质量。
...
<details>
  <summary>Details</summary>
Motivation: 现有GPU加速方法要么并行化单个求解以满足实时要求，要么以慢于实时的速率扩展到非常大的批量，或者通过限制模型通用性来实现速度。这导致许多需要实时批量求解的最先进MPC应用存在求解器性能差距。

Method: GATO采用算法、软件和计算硬件协同设计的方法，利用块级、warp级和线程级并行性在求解内部和求解之间实现超高性能。

Result: 模拟基准测试显示，随着批量大小增加，相比CPU基线实现18-21倍加速，相比GPU基线实现1.4-16倍加速；案例研究显示改进的扰动抑制和收敛行为；在工业机械臂上进行了硬件验证。

Conclusion: GATO填补了中等批量规模实时轨迹优化求解器的性能空白，通过开源支持可重复性和采用。
...
<details>
  <summary>Details</summary>
Motivation: 解决顺序机器人操作任务中，在高维配置空间中寻找满足多对象交互几何约束的无碰撞轨迹的实时计算挑战，克服现有GPU方法因CPU-GPU数据传输开销和复杂逻辑导致的性能限制。

Method: 采用两阶段粒子优化策略：首先通过大规模并行采样解决放置约束，然后在关节空间中提升为完整轨迹优化。完全GPU并行化，将约束评估、采样和梯度优化编译为优化的CUDA内核，无需CPU协调。

Result: 在具有挑战性的基准测试中，解决方案时间达到毫秒级，成功率为100%，相比现有方法实现了4000倍的加速。

Conclusion: SPaSM通过完全GPU并行化和联合优化对象放置与机器人轨迹，实现了顺序操作任务的实时求解，显著超越了现有方法的性能。
...
<details>
  <summary>Details</summary>
Motivation: 模型扩散中的约束条件会导致灾难性性能下降，即使在简单2D系统中也会因蒙特卡洛近似得分函数的样本效率低下而出现问题

Method: 引入新兴障碍模型扩散(EB-MBD)，使用逐步引入的障碍约束避免性能问题，无需计算昂贵的投影操作，通过分析每轮迭代的采样活性来指导障碍参数调度选择

Result: 在2D碰撞避免和3D水下机械臂系统中，该方法相比模型扩散获得更低成本解，计算时间比基于投影的方法少几个数量级

Conclusion: 新兴障碍函数方法能有效解决模型扩散中的约束问题，显著提升性能同时大幅降低计算成本
...
<details>
  <summary>Details</summary>
Motivation: 解决双足机器人在粗糙地形上安全导航的挑战，需要处理地形高程不确定性问题，同时保证动态可行性和质心稳定性。

Method: 使用高斯过程回归估计高程图，保形预测构建置信区间，结合收缩理论可达管和线性倒立摆模型飞轮扭矩控制，构建高级MPC导航框架。

Result: 在MuJoCo中对Digit双足机器人进行物理仿真验证，证明框架能提供概率安全性和目标可达性保证。

Conclusion: 该规划框架能够有效处理地形不确定性，为双足机器人粗糙地形导航提供概率安全保证和动态稳定性。
...
<details>
  <summary>Details</summary>
Motivation: 现有故障注入研究通常针对单一传感器或感知模块，导致难以推广或集成到统一仿真环境中的孤立框架。需要一种独立于特定传感器或算法的分析方法。

Method: 将感知失败重新定义为幻觉，提出可配置的组件无关幻觉注入框架，在开源模拟器中注入六种合理幻觉类型，执行超过18,350次模拟。
...
Result: 在MetaWorld模拟数据上，TrajSkill相比最先进方法将FVD降低39.6%，KVD降低36.6%，跨形态成功率提高达16.7%。真实机器人厨房操作实验验证了方法的有效性。

Conclusion: TrajSkill实现了实用的人类到机器人跨形态技能迁移，直接从人类演示视频学习操作技能。
...
<details>
  <summary>Details</summary>
Motivation: 当前VLA模型在预训练阶段缺乏与具身场景相关的推理密集型任务，无法进行复杂交互所需的人类意图推理。

Method: 使用精心设计的推理数据（意图推理、空间定位和紧凑具身推理）进行课程训练，在微调阶段将紧凑推理输出作为动作生成的上下文指导。

Result: 在直接指令下比π0高18%成功率，在意图指令下比ECoT高28%成功率，在分布外意图任务上达到基线两倍以上的成功率，零样本人机交互达到40%成功率。
...
<details>
  <summary>Details</summary>
Motivation: 现有的微移动车辆动态建模工具主要依赖运动学自行车模型或其变体，这些模型无法捕捉轮胎滑移、载荷转移和骑手/车辆倾斜等关键物理现象，且缺乏统一的物理基础模型来覆盖各种常见微移动车辆和车轮布局。

Method: 开发了基于轮胎刷表示的轮胎级公式GM3，支持任意车轮配置；构建了交互式模型无关仿真框架，将车辆/布局规范与动态特性解耦，使用固定步长RK4积分、人在回路和脚本控制，以及实时轨迹跟踪和日志记录进行分析；在斯坦福无人机数据集的环形交叉口场景中对骑行者、滑板车和手推车类别进行了实证验证。

Result: GM3模型能够更准确地捕捉微移动车辆的物理动态特性，包括轮胎滑移、载荷转移和倾斜效应，相比传统运动学自行车模型提供了更真实的仿真结果。

Conclusion: GM3提供了一个统一的物理基础框架，能够有效建模各种微移动车辆的动态行为，填补了现有建模工具的空白，为自动驾驶系统训练和城市交通仿真提供了更准确的基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于流的策略存在表示崩溃问题，无法区分相似的视觉表示，导致在精确操作任务中失败。需要一种既能保持一步生成效率又能避免表示崩溃的方法。
...
<details>
  <summary>Details</summary>
Motivation: 水下环境对机器人操作具有独特挑战，包括复杂流体动力学、有限能见度和受限通信。现有数据驱动方法缺乏大规模高质量水下数据集，难以开发能自主执行多任务的水下智能系统

Method: 构建USIM仿真数据集（56.1万帧，1852条轨迹），提出U0视觉-语言-动作模型，集成双目视觉和其他传感器模态，采用卷积注意力感知聚焦增强模块改善空间理解和移动操作

Result: 在检查、避障、扫描和动态跟踪等任务中达到80%成功率，在移动操作任务中比基线方法减少目标距离21.2%

Conclusion: VLA模型可有效应用于水下机器人，为可扩展数据集构建、改进任务自主性和实现智能通用水下机器人提供基础
...
<details>
  <summary>Details</summary>
Motivation: 开发能够在动态人多的室内环境中安全、高效且符合社交规范的自主导航系统，仅使用机载RGB-D传感器和里程计，无需全局地图或特权信息。

Method: 在Falcon模型基础上引入主动风险感知模块，通过学习预测周围人类的基于距离的碰撞风险分数，增强空间感知和主动避碰行为。

Result: 在Social-HM3D基准测试中，该方法提高了代理在拥挤室内场景中保持个人空间合规性的能力，在16个参赛团队中获得第二名。

Conclusion: 主动风险感知模块有效提升了社交导航性能，使自主代理能够在动态人类环境中更好地遵守社交规范并安全导航。
...
<details>
  <summary>Details</summary>
Motivation: 当前MLLMs在双臂人形机器人的长视野任务中效果有限，主要由于缺乏系统评估平台和模型缺乏本体感知能力，无法推理双臂选择和身体位置。

Method: 开发了DualTHOR仿真器，具有连续转换和应急机制；提出Proprio-MLLM模型，通过运动位置嵌入和跨空间编码器整合本体感知信息。

Result: 在现有MLLMs表现不佳的环境中，Proprio-MLLM实现了平均19.75%的规划性能提升。

Conclusion: 该研究为推进人形机器人具身智能提供了必要的仿真平台和有效模型。
...
<details>
  <summary>Details</summary>
Motivation: 解决机器人在非结构化环境中执行精确通用操作的根本挑战，弥合视觉语言模型高级语义理解与机器人实际物理执行之间的"语义-物理"鸿沟。

Method: 引入GRACE框架，通过可执行分析概念（EAC）- 数学定义的蓝图编码对象可供性、几何约束和操作语义，构建结构化策略支架管道，将自然语言指令和视觉信息转化为实例化EAC，从中推导抓取姿态、力方向和物理可行运动轨迹。

Result: 在模拟和真实环境中对多种铰接物体实现了强大的零样本泛化能力，无需任务特定训练。

Conclusion: GRACE提供了一个统一且可解释的接口，有效实现语义-物理接地，支持精确和可泛化的机器人操作。
...
<details>
  <summary>Details</summary>
Motivation: 旋转群SO(3)是黎曼流形，其非欧几何性质导致局部约束难以整合，特别是同时整合多个局部约束时存在困难。
...
Conclusion: 该方法解决了SO(3)流形上的失真问题，使现成的欧几里得学习算法在非欧空间中重新适用，有效整合多个局部约束。
...
Method: 通过FastUMI机器人系统收集数据，该系统采用模块化、硬件解耦的机械设计和集成轻量级跟踪系统，整合了末端执行器状态、多视角腕戴鱼眼图像和文本注释等多模态数据流。

Result: 实验结果表明，FastUMI-100K能够在各种基线算法上实现高策略成功率，证明了其在解决复杂动态操作挑战方面的鲁棒性、适应性和现实世界适用性。

Conclusion: FastUMI-100K提供了一个更可扩展、灵活和适应性强的解决方案，能够满足现实世界机器人演示数据的多样化需求，数据集和源代码将在GitHub上发布。
...
<details>
  <summary>Details</summary>
Motivation: LLM在机器人规划中存在幻觉问题，导致过度自信但可能不安全或不对齐的计划。现有研究未能充分区分认知和内在不确定性，限制了不确定性估计的效果。

Method: 提出CURE方法，将不确定性分解为认知不确定性和内在不确定性，认知不确定性进一步细分为任务清晰度和任务熟悉度。使用随机网络蒸馏和基于LLM特征的多层感知机回归头进行不确定性评估。

Result: 在厨房操作和桌面重排两个实验环境中验证，相比现有方法，该方法的不确定性估计与真实执行结果更加一致。

Conclusion: CURE方法通过分解不同类型的不确定性，显著提高了LLM在机器人规划中的可靠性，不确定性估计更加准确。
...
Method: 系统包含AI智能体（集成多模态感知和记忆注意力机制用于定位不可见目标结构）和588克6自由度线驱动机器人，通过安装在腹部增强运动鲁棒性

Result: 机器人能自主获取专家级标准肝脏超声平面，在患者中检测病理，包括来自高海拔医疗资源有限城市西宁的两名患者，在快速运动个体和野外环境中表现有效

Conclusion: 这是首个在多种挑战性场景下实现自主超声检查的系统，有望改变医疗资源不足地区获得专家级诊断的途径
...
<details>
  <summary>Details</summary>
Motivation: 现有工作大多不直接关注模型发现，只提取常规动作集合，且未在包含噪声（人类执行的自然变异和偶然错误）的场景中评估。

Method: 采用基于聚类的技术从UI日志中提取常规日志，在九个具有不同噪声水平的UI日志上进行实验。

Result: 与现有技术相比，该技术能够提取更准确的常规日志，特别是在存在噪声的情况下，通过标准指标评估显示性能更优。

Conclusion: 所提出的聚类技术能够有效提取常规日志，在噪声环境下表现优于现有方法，为机器人流程自动化提供了更好的支持。
...
Method: 引入NavSpace基准测试，包含六个任务类别和1,228个轨迹-指令对，用于探测导航代理的空间智能。在此基础上提出SNav空间智能导航模型。

Result: 评估了22个导航代理，包括最先进的导航模型和多模态大语言模型。SNav在NavSpace和真实机器人测试中优于现有导航代理。

Conclusion: NavSpace基准揭示了具身导航中的空间智能问题，SNav为未来工作建立了强基线。
...
<details>
  <summary>Details</summary>
Motivation: 评估在有限时间离散化条件下，经典和现代控制方法在真实世界电缆驱动并联机器人中的性能表现，特别是针对欠约束系统。

Method: 对经典PID控制器和现代强化学习算法（包括DDPG、PPO和TRPO）进行对比分析，评估它们在各种轨迹和控制更新间隔下的性能。

Result: TRPO在所有方法中表现最优，实现了最低的均方根误差，并且对更大的控制更新间隔具有鲁棒性，能够在噪声环境中实现稳定控制。

Conclusion: TRPO作为复杂机器人控制任务的鲁棒解决方案具有巨大潜力，特别适用于动态环境和未来在传感器融合或混合控制策略中的应用。
...
<details>
  <summary>Details</summary>
Motivation: 随着工业机器人进入共享人类空间，其不透明的决策过程威胁安全、信任和公共监督。该项目探索复杂多智能体AI是否能变得直观可理解。

Method: 使用两个强化学习训练的机械臂竞争抖动床单，基于三个设计原则：竞争作为清晰指标（谁举得更高）、身体熟悉性（观众识别布料抖动）、感官到感知映射（通过森林和天气投影显示机器人合作或竞争）。

Result: 在五个国际展览中的观察表明，观众能实时解读机器人的策略、冲突和合作，情感反应与系统内部状态一致。
...
<details>
  <summary>Details</summary>
Motivation: 现有的逆最优控制方法要么基于缓慢的双层过程，要么基于快速但对噪声敏感的优化条件违反最小化方法，需要一种既快速又鲁棒的新方法。

Method: 采用等式约束的最优控制模型，通过单级重构方法替代传统的双层逆最优控制方法。

Result: 在仿真实验中，该方法对噪声表现出很强的鲁棒性，计算时间比传统双层实现减少了15倍。
...
<details>
  <summary>Details</summary>
Motivation: 需要验证6-6 Stewart-Gough平台机械臂在给定动平台方向下的最大无碰撞球体尺寸，确保机械臂在工作空间内的运动安全性。

Method: 利用CAD软件API自动化更新动平台位置，在CFS表面包围壳内采样，检测每对腿部之间的相互碰撞。

Result: 该方法能够验证预计算CFS的安全性，并可估计任何空间并联机械臂的无碰撞球体。

Conclusion: 提出的基于CAD API的验证方法能够有效验证机械臂无碰撞球体的安全性，适用于各种空间并联机械臂。
...
<details>
  <summary>Details</summary>
Motivation: VLA模型在资源受限硬件上部署困难，虽然剪枝技术能有效压缩大型语言模型，但在机器人领域研究不足，且直接剪枝VLA模型会导致性能急剧下降和安全违规增加。

Method: GLUESTICK执行一次性权重空间插值，计算密集模型与剪枝模型之间的校正项，在推理时由每个剪枝层使用该校正项恢复丢失的能力，仅引入一个控制效率与精度权衡的超参数。

Result: 在多种VLA架构和操作、导航任务中，GLUESTICK实现了竞争性的内存效率，同时显著恢复成功率并减少安全违规。

Conclusion: GLUESTICK提供了一种无需训练、与剪枝算法无关的有效方法，能够在保持稀疏性优势的同时恢复剪枝VLA模型的功能，解决了机器人领域模型压缩的关键挑战。
...
<details>
  <summary>Details</summary>
Motivation: 解决从第三方人类操作视频中学习灵巧操作技能的问题，消除对复杂传感器和标注数据的依赖，直接控制类人机器人。

Method: 使用基于接触的奖励来改进从野外视频估计的噪声手部物体姿态中进行策略学习，支持真实和合成视频输入。

Result: 在TACO基准测试中实现最先进的物体姿态估计性能（ADD-S和VSD分别提升0.08和0.12），在OakInk-v2上的成功率比先前方法提高19%。

Conclusion: DexMan能够从真实和合成视频生成技能，无需手动数据收集和昂贵的动作捕捉，为训练通用灵巧操作创建大规模多样化数据集。
...
<details>
  <summary>Details</summary>
Motivation: 解决机器人操作中空间泛化问题，传统方法需要大量人工演示覆盖不同空间配置，现有数据生成方法存在显著的仿真到现实差距，且局限于固定基座和预定义相机视角。

Method: 基于单个源演示，通过细粒度场景和轨迹解析、分组增强策略处理多对象组合和任务约束、相机感知处理对齐真实3D传感器分布，直接增强点云观测-动作对。

Result: R2RGen在大量实验中显著提高了数据效率，在移动操作中展现出强大的扩展和应用潜力。

Conclusion: R2RGen框架为机器人操作提供了高效、即插即用的真实世界数据增强方案，有效解决了空间泛化问题。
...
<details>
  <summary>Details</summary>
Motivation: 解决机器人手内物体旋转从仿真到真实世界的泛化难题，克服复杂接触动力学造成的"现实差距"，突破以往工作受限于简单几何、有限物体尺寸和长宽比、约束手腕姿势或定制化手部的局限。

Method: 使用关节级动力学模型学习桥接现实差距，通过关节分解动态、压缩系统影响为低维变量，从各关节自身动态特征学习演化；结合全自动数据收集策略获取多样化真实世界交互数据。

Result: 单一策略成功旋转复杂形状物体（如动物）、高长宽比物体（达5.33）和小尺寸物体，处理多样化手腕方向和旋转轴，在真实世界评估和复杂任务遥操作应用中验证有效性和鲁棒性。

Conclusion: 该框架展示了前所未有的泛化能力，有效解决了手内物体旋转的sim-to-real挑战，为机器人灵巧操作提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常假设任务在分布内或需要特定机器人的微调数据，限制了跨平台的迁移能力。

Method: 通过视频生成模型合成任务视频，使用现成感知模块提取3D对象流，为刚性物体计算相对位姿，为可变形物体提供基于粒子动力学模型的跟踪目标。
...
<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶系统的离线评估难以准确预测在线性能，微小误差可能在测试时累积导致事故，这种离线与在线评估之间的关系在复杂城市场景中研究不足。

Method: 通过大量仿真实验分析离线与在线评估的相关性，提出基于认知不确定性的离线指标来识别可能导致闭环错误的事件。

Result: 发现离线与在线评估的相关性比先前研究更差，基于认知不确定性的新指标相比传统离线指标提升了13%以上的相关性，在真实世界环境中表现更好。
...
Method: 基于LLM规划器的零样本能力，在模拟器中自动生成多样化操作任务的演示，利用成功示例微调LLM以提升其规划能力，无需人工监督。

Result: BLAZER显著提升了模拟和真实环境中的零样本操作能力，在训练池外的任务上也有改进，并能缩小LLM模型规模。

Conclusion: BLAZER框架通过自动生成训练数据有效解决了机器人领域数据稀缺问题，实现了从模拟到真实环境的技能迁移，提升了LLM在机器人操作任务中的性能。
