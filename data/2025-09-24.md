...
Motivation: 机器人操作策略难以泛化是因为需要同时学习注意力位置、动作类型和执行方式。PEEK旨在将高层推理任务卸载给视觉语言模型，让策略专注于动作执行

Method: 1. 微调VLMs预测末端执行器路径和任务相关掩码 2. 开发自动标注管道在20+机器人数据集上生成标注 3. 将中间表示直接叠加到机器人观测上，实现策略无关性

Result: 1. 仅仿真训练的3D策略在真实世界中提升41.4倍 2. 大型VLA和小型操作策略分别获得2-3.5倍性能提升 3. 在9种不同机器人平台上实现零样本泛化

Conclusion: PEEK通过让VLMs吸收语义和视觉复杂性，为操作策略提供最小必要提示，有效解决了机器人操作策略的泛化问题
...
<details>
  <summary>Details</summary>
Motivation: 通用机器人策略需要个性化调整以匹配用户偏好，但个性化过程中会泄露用户的偏好、习惯和风格等隐私数据，需要开发既能个性化又能保护隐私的机器人策略。

Method: 为每个用户分配唯一密钥，使用该密钥对机器人网络的权重进行数学变换。只有使用正确密钥时，机器人策略才会切换到该用户的个性化版本；使用错误密钥时，机器人会恢复到基线行为。

Result: PRoP在模仿学习、强化学习和分类任务中表现出通用适用性，保持了原始策略的架构和行为，在实验性能上优于现有的基于编码器的方法。

Conclusion: PRoP提供了一个模型无关的框架，能够在实现机器人个性化学习的同时保护用户隐私，解决了人机交互中个性化与隐私保护之间的基本挑战。
...
<details>
  <summary>Details</summary>
Motivation: 研究人类在协作操纵物体时如何通过触觉通信传递运动意图，并将这种通信模式应用于改进机器人协作能力。

Method: 通过IMU追踪人-人协作和人-机器人协作时的物体运动轨迹，结合问卷调查收集参与者对协作流畅性的主观评价。

Result: 人-人协作在流畅性上显著优于人-机器人协作，IMU数据也显示出两种条件下运动模式的客观差异。

Conclusion: 研究结果支持未来开发能够发送和接收拟人化触觉信号的机器人助手，以提升物理任务协作效果。
...
Method: 通过自动处理火星2020探测器在每个位置拍摄的图像数据，结合轨道高程和彩色地图，构建上下文网格。

Result: 成功开发出可在ASTTRO工具中使用的交互式3D地形可视化系统，部分成果已部署到公众访问网站。

Conclusion: Landform contextual mesh技术有效支持了火星任务的科学规划和公众参与。
...
<details>
  <summary>Details</summary>
Motivation: 在结构化户外环境中，特别是葡萄园这种具有重复行几何结构的环境下，基于LiDAR的定位方法经常因感知混叠而失败，需要更稳定的定位解决方案。

Method: 使用语义粒子滤波器，将稳定的物体级检测（葡萄树干和支撑杆）投影到鸟瞰图并与LiDAR扫描融合，创新性地使用语义墙连接相邻地标形成伪结构约束，并在语义稀疏区域引入自适应GPS先验。

Result: 在真实葡萄园实验中，该方法能够保持在正确行内定位，从AMCL失败的位置恢复定位，并优于基于视觉的SLAM方法如RTAB-Map。

Conclusion: 所提出的语义粒子滤波器在葡萄园等具有重复结构的户外环境中表现出优越的定位性能，能够有效解决感知混叠问题。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM驱动的规划系统在物理世界部署时经常违反安全和监管约束，传统的数据驱动对齐方法需要昂贵的人工标注，而现有的形式化反馈方法仍依赖资源密集的微调。
...
<details>
  <summary>Details</summary>
Motivation: 无控制交叉口由于通行权规则模糊、视线遮挡和驾驶员行为不可预测，导致大量交通事故。现有研究主要关注自动驾驶车辆，缺乏针对人工驾驶车辆的辅助导航系统。

Method: 使用POMDP框架构建驾驶员辅助系统，在自定义仿真环境中比较四种决策方法：确定性有限状态机（FSM）和三种概率规划器（QMDP、POMCP、DESPOT）。

Result: 概率规划器优于基于规则的基线方法，在部分可观测条件下实现高达97.5%的无碰撞导航，其中POMCP优先考虑安全性，DESPOT平衡效率和运行时可行性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的VLA模型依赖于大规模人工标注的动作数据集，模型规模大且部署困难。需要一种更高效、实用的方法，能够利用无标签视频数据进行预训练。
...
Conclusion: LAWM提供了一种高效实用的自监督预训练框架，能够有效利用无标签视频数据，在保持性能的同时降低模型规模和部署难度。
...
<details>
  <summary>Details</summary>
Motivation: 在物理人机交互中，不同身体部位可能施加不兼容的力要求，特别是在护理任务中，多接触点交互不可避免。由于无法同时满足所有目标，优先级排序变得至关重要

Method: 结合学习排序方法和分层操作空间控制，利用模拟循环展开进行数据高效且安全的探索。通过用户研究获得个性化舒适阈值并整合到框架中

Result: 通过广泛的仿真和真实世界实验验证，PrioriTouch能够适应用户接触偏好，保持任务性能，并提高安全性和舒适度

Conclusion: PrioriTouch框架为多接触点物理人机交互提供了一种有效的优先级排序解决方案，特别适用于护理场景，能够处理个性化偏好和冲突目标
...
<details>
  <summary>Details</summary>
Motivation: 现有的非抓取式操作主要依赖平行夹爪或工具，而多指灵巧手能提供更丰富的接触模式和操作多样性，但非抓取式操作的动力学建模困难。

Method: 通过接触引导采样生成手部姿势，使用物理模拟进行筛选，训练条件扩散模型来预测基于物体几何的可行手部姿势，最后使用标准运动规划器执行推拉动作。

Result: 在Allegro Hand上进行了840次真实世界实验，结果表明GD2P为训练灵巧非抓取操作策略提供了可扩展的途径，并在LEAP Hand上验证了方法的通用性。

Conclusion: GD2P方法有效解决了灵巧手非抓取操作的挑战，提供了包含130万个手部姿势和2300个物体的开源数据集，促进了相关研究的进一步发展。
...
<details>
  <summary>Details</summary>
Motivation: 感知系统为自主系统提供环境理解，其故障会影响下游模块。感知故障具有环境依赖性和跨模块传播的特点，需要专门的方法来检测和隔离。

Method: 采用反事实推理构建感知可靠性测试，将故障检测与隔离问题建模为因果推理问题。被动方法通过信念更新实现，主动方法则建模为因果多臂赌博机问题，使用蒙特卡洛树搜索和上置信界算法来优化控制输入。

Result: 在机器人探索场景中验证了方法的有效性，机器人能够主动调整姿态以增加有效信息，成功隔离了由传感器损坏、动态场景和感知退化引起的故障。

Conclusion: 基于反事实推理的框架能够有效处理感知系统的故障检测与隔离问题，特别是在环境依赖和故障传播的复杂场景下表现出良好性能。
...
<details>
  <summary>Details</summary>
Motivation: 受人类运动控制中成本效益权衡模型的启发，研究如何通过故意变异奖励函数来产生机器人操作任务的多样化技能变化。

Method: 开发了基于高斯噪声的奖励函数变异框架，奖励函数包含准确性、时间和努力三个关键项，在NVIDIA Isaac Sim仿真环境中使用Franka Emika Panda机械臂进行倒水任务，采用PPO强化学习算法。

Result: 不同权重配置的变异奖励函数产生了广泛的行为变化：从原始倒水任务执行方式的变异，到容器边缘清洁、液体混合和浇水等意外任务的新技能。

Conclusion: 该方法为机器人系统执行特定任务的多样化学习提供了有前景的方向，同时可能为未来任务衍生出有意义的技能。
...
<details>
  <summary>Details</summary>
Motivation: MPC在双足行走中有效但难以建模复杂地形交互，RL在多样化地形上训练稳健策略但缺乏约束保证。现有MPC和RL结合方法主要限于平坦地形或四足机器人。

Method: 参数化基于单刚体动力学的MPC的三个关键组件：系统动力学、摆动腿控制器和步态频率，在NVIDIA IsaacLab中进行双足机器人仿真验证。

Result: 在楼梯、踏脚石和低摩擦表面等多种地形上的实验表明，RL增强MPC框架比基线MPC和RL产生显著更自适应和稳健的行为。

Conclusion: RL增强MPC框架成功结合了MPC的约束保证和RL的地形适应性，为双足机器人在挑战性地形上的稳健行走提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有规划控制框架主要基于参考轨迹，在车辆达到动态极限时性能受限，需要能够直接处理动态可行性和安全约束的统一框架

Method: 开发了计算高效的车辆动力学模型和连续可微的数学公式来准确捕捉可行驶包络线，结合强化学习和优化技术解决包络规划问题

Result: 通过仿真和真实实验验证，该框架在赛车、紧急避障和越野导航等多种任务中表现出高性能

Conclusion: 该框架具有可扩展性和广泛适用性，能够处理各种驾驶场景而不依赖预定义参考轨迹
...
<details>
  <summary>Details</summary>
Motivation: 解决多模态语义学习中的异构数据有效融合和计算效率问题，特别是在资源受限的机器人交互环境中。

Method: 提出LCMF框架，将多级跨模态参数共享机制引入Mamba模块，结合交叉注意力和选择性参数共享状态空间模型的优势。

Result: 在VQA任务中达到74.29%准确率，在EQA视频任务中达到LLM Agents分布集群的中等水平，FLOPs相比同类基线平均减少4.35倍，参数量仅为166.51M（图像-文本）和219M（视频-文本）。

Conclusion: LCMF为资源受限场景下的人机交互应用提供了高效解决方案，具有强大的多模态决策泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有视觉语言导航方法在未知环境中适应性差、计算效率低的问题，实现可扩展的实时自主导航。
...
Conclusion: VLN-Zero通过结合快速探索、符号推理和缓存执行，克服了现有方法的计算低效和泛化能力差的问题，实现了在未知环境中的鲁棒和可扩展决策。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的代码生成方法存在噪声大、受限于固定原语、上下文窗口有限等问题，在长时域任务中表现不佳，且闭环反馈中的修正知识存储格式不当导致泛化能力差和灾难性遗忘。

Method: 采用人机协作框架，将人类修正编码为可重用技能，利用外部记忆和带有提示机制的检索增强生成技术实现动态重用。

Result: 在Ravens、Franka Kitchen和MetaWorld等平台以及真实环境中，框架实现了0.93的成功率（比基线高27%）和42%的修正轮次效率提升，能够稳健解决需要规划超过20个原语的"建造房屋"等极端长时域任务。

Conclusion: 该框架通过有效整合人类修正和可重用技能学习，显著提升了LLM在机器人操作任务中的性能，特别是在长时域复杂任务中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 解决农业冠层环境下GNSS信号不可靠、行间环境杂乱以及光照条件变化导致的导航困难问题。

Method: 采用端到端学习方法，使用基于体素的下采样策略将LiDAR输入尺寸减少95.83%，通过深度强化学习策略在模拟环境中训练，直接将3D LiDAR数据映射到控制命令。

Result: 在模拟验证中，在直线行种植园中达到100%的成功率，随着行曲率增加性能逐渐下降，在不同正弦频率和幅度下进行了测试。

Conclusion: 该方法无需标记数据集或手动设计的控制接口，能够有效处理农业冠层环境下的导航问题，特别是在直线行种植场景中表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 端到端运动规划虽然简化了自动驾驶流程，但在场景理解和有效预测决策方面仍面临挑战，阻碍了其大规模部署。

Method: 采用双向Mamba融合处理相机和LiDAR多模态数据，结合推理增强解码器（Mamba+MoE）进行场景兼容锚点选择和自适应轨迹推理，并使用动作-运动交互模块利用周围智能体状态预测来优化自车规划。

Result: 在NAVSIM基准测试中，PIE不使用任何集成和数据增强技术，取得了88.9 PDM分数和85.6 EPDM分数，超越了现有最先进方法。

Conclusion: PIE能够可靠地生成可行且高质量的自车轨迹，为端到端运动规划提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决开放词汇无人机导航的挑战，包括大规模演示数据稀缺、无人机实时控制需求以及缺乏可靠的外部姿态估计模块。

Method: 使用三个核心组件：(i) 基于高斯溅射的光真实语言嵌入飞行模拟器，(ii) RRT启发的多轨迹生成专家用于无碰撞导航演示，(iii) 轻量级端到端视觉运动策略进行实时闭环控制。

Result: 在硬件飞行实验中，SINGER在未见环境和未见语言条件目标物体上表现出优越的零样本模拟到真实迁移性能，比基线方法平均多到达查询目标23.33%，多保持查询在视野中16.67%，且碰撞减少10%。

Conclusion: SINGER成功实现了仅使用机载传感的语言引导无人机自主导航，展示了强大的开放词汇导航能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于学习的自动驾驶系统主要基于无事故数据训练，在安全性能边界附近缺乏指导。真实事故报告包含所需的对比证据，但难以直接使用，因为叙述是非结构化、第三人称且与传感器视图关联性差。

Method: 将事故叙述标准化为自我中心语言，将日志和事故转换为统一的场景-动作表示。在决策时，通过检索统一索引中的相关先例来评估提议动作；还提出了基于代理的反事实扩展，为每个动作提议合理替代方案，检索结果并跨结果推理后再做决定。

Result: 在nuScenes基准测试中，先例检索显著改善了校准效果，上下文偏好动作的召回率从24%提升到53%。反事实变体在保持这些收益的同时，在风险附近做出了更精确的决策。

Conclusion: 该方法成功地将真实事故报告转化为可用的训练数据，通过检索先例和反事实推理显著提升了自动驾驶系统在安全边界附近的决策能力。
...
<details>
  <summary>Details</summary>
Motivation: 行为克隆在机器人操作中表现出潜力，但真实世界演示成本高昂。仿真数据虽可扩展，但存在仿真与现实之间的领域差距，阻碍策略迁移。

Method: 采用最优传输（OT）损失在协同训练框架中，通过对齐观察和动作的联合分布来学习领域不变特征空间，并扩展为不平衡OT框架处理仿真与真实数据不平衡问题。

Result: 在挑战性操作任务上验证，方法能利用丰富仿真数据将真实世界成功率提升高达30%，并能泛化到仅在仿真中见过的场景。

Conclusion: 该方法通过联合分布对齐而非仅观察对齐，有效缩小仿真与现实差距，为机器人操作策略学习提供了高效解决方案。
...
Method: 使用Lloyd算法进行均匀分区和匈牙利算法进行分配(PAAS)，结合基于基元的路径搜索和非线性轨迹优化来规划DVS的时空轨迹，实现自适应变换。

Result: 在模拟环境中支持最多15%的无人机加入或离开，能够快速恢复期望编队形状，相比前沿编队规划方法展现出更强的编队恢复能力和环境适应性。

Conclusion: 该方法通过可变形虚拟结构和分布式轨迹规划，有效解决了动态无人机编队在狭窄环境中的规划挑战，具有实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 传统模仿学习方法同时使用视觉观察和本体感觉状态，导致策略过度依赖本体感觉输入，对训练轨迹过拟合，空间泛化能力差

Method: 构建状态无关策略，移除本体感觉状态输入，仅在相对末端执行器动作空间中基于视觉观察预测动作，使用双广角手腕摄像头提供完整任务相关视觉信息

Result: 在真实世界任务中（如拾放、衬衫折叠、全身操作），状态无关策略在高度泛化上成功率从0%提升到85%，水平泛化从6%提升到64%，同时在数据效率和跨机器人适应方面也有优势

Conclusion: 状态无关策略通过消除对本体感觉的依赖，显著改善了空间泛化能力，增强了实际部署的实用性
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中部署强化学习面临安全性挑战，仿真器虽然提供安全训练环境，但仿真-现实差距会引入新的安全隐患。现有鲁棒安全RL方法难以与标准训练流程兼容，而流行的域随机化方法在实践中往往导致不安全行为。

Method: SPiDR采用悲观域随机化方法，将仿真-现实差距的不确定性纳入安全约束中，使其具有可扩展性且与现有训练流程高度兼容。

Result: 在仿真-仿真基准测试和两个不同的真实机器人平台上进行广泛实验，证明SPiDR能有效确保仿真-现实差距下的安全性，同时保持强大的性能。

Conclusion: SPiDR提供了一种具有可证明保证的安全仿真-现实转换算法，在确保安全性的同时维持良好性能，为现实世界RL部署提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统碰撞避免方法往往忽略上下文信息或缺乏分布鲁棒性，在复杂场景下容易失效。需要一种能够结合环境上下文并处理分布不确定性的鲁棒方法。

Method: 使用条件核均值嵌入在RKHS中表示障碍物轨迹的条件分布，构建基于经验估计的分布模糊集，将分布鲁棒碰撞避免约束集成到模型预测控制框架中。

Result: 仿真结果表明，在多个挑战性场景下，该方法比不包含上下文信息和分布鲁棒性的方法具有更好的碰撞避免性能。

Conclusion: 提出的分布鲁棒方法通过有效利用上下文信息和处理分布不确定性，显著提高了碰撞避免的鲁棒性和成功率。
...
<details>
  <summary>Details</summary>
Motivation: 移动操作中，操作策略对初始位姿有强偏好，但导航模块只关注到达任务区域而不考虑后续操作需求，这种错配影响了任务成功率。

Method: N2M模块仅依赖自我中心观测，无需全局或历史信息，具有实时环境适应能力和高视角鲁棒性。

Result: 在PnPCounterToCab任务中，N2M将平均成功率从基于可达性的基线3%提升到54%；在Toybox Handover任务中，仅用15个数据样本就能在未见环境中提供可靠预测。

Conclusion: N2M具有广泛适用性、卓越的数据效率和泛化能力，能有效解决导航与操作之间的位姿偏好错配问题。
...
Method: 提出3D FDP框架，预测采样查询点的时间轨迹，并在统一扩散架构中基于这些交互感知流生成动作，将操作建立在局部动态基础上，同时考虑动作的广泛场景级后果。

Result: 在MetaWorld基准测试中，3D FDP在50个任务上达到最先进性能，特别是在中等和困难设置中表现优异。在8个真实机器人任务中，在接触丰富和非抓取场景中持续优于现有基线。

Conclusion: 3D流作为学习可泛化视觉运动策略的强大结构化先验，支持开发更鲁棒和通用的机器人操作。
...
Method: QDP框架使用包含物体、接触点和技能信息的查询来识别任务相关组件，并利用点云观测指导底层策略，提高策略鲁棒性

Result: 在FurnitureBench仿真和真实环境实验中，QDP在插入和拧螺丝任务中相比无结构化查询的基线方法，技能成功率提升超过50%

Conclusion: QDP通过查询中心机制有效桥接了高层规划和底层控制，在复杂装配任务中表现出优越性能
...
Method: 提出基于强化学习的方法，让配备深度相机的虚拟四旋翼无人机在模拟城市环境中学习导航

Result: 论文提出了一个强化学习框架，但具体实验结果未在摘要中提及

Conclusion: 该方法旨在提高自主飞行器在城市环境中的避障和导航能力
...
<details>
  <summary>Details</summary>
Motivation: 手持抓取器作为数据收集工具虽然直观且可扩展，但仅依赖第一人称视角手腕相机难以捕捉足够的场景上下文，限制了模仿学习的任务范围。

Method: 提出MV-UMI框架，整合第三人称视角与第一人称视角相机，减轻人类演示与机器人部署之间的领域偏移，同时保持手持设备的跨具身优势。

Result: 实验结果显示，在需要广泛场景理解的子任务中，性能提高了约47%，验证了该方法的有效性。

Conclusion: MV-UMI框架扩展了手持抓取器系统可学习的操作任务范围，同时不损害其固有的跨具身优势。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉模仿学习方法主要关注策略设计，但忽视了视觉编码器的结构和能力，限制了空间理解和泛化。受生物视觉系统启发，需要结合视觉和本体感觉线索来实现鲁棒控制。

Method: 采用视觉几何基础Transformer作为视觉编码器，引入本体感觉引导的视觉学习策略对齐感知与机器人内部状态，设计帧级token重用机制减少推理延迟，并应用随机token剪枝增强策略鲁棒性。

Result: 在MetaWorld挑战任务上的实验表明，VGGT-DP显著优于DP和DP3等强基线方法，特别是在精度要求高和长时程场景中表现突出。

Conclusion: VGGT-DP通过整合几何先验和本体感觉反馈，有效提升了视觉模仿学习的空间定位和闭环控制性能，为机器人操作技能学习提供了更鲁棒的解决方案。
...
Method: 开发了高斯过程概念归因（GP-CA）方法，利用主动学习查询信息丰富的实例来发现新的不确定性来源，并对配准不确定性进行归因分析。

Result: 在三个公开数据集和真实机器人实验中验证了GP-CA的有效性，该方法在运行时间、样本效率和准确性方面均优于现有最先进方法，并能实现有效的故障恢复行为。

Conclusion: GP-CA方法能够显著提高机器人感知的鲁棒性，通过不确定性归因和主动学习机制为点云配准问题提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 随着车辆自动化和互联程度的提高，协同智能交通系统需要处理动态环境下的高效资源利用问题，传统云原生技术在C-ITS中面临独特挑战。

Method: 基于Kubernetes和ROS 2构建应用管理框架，采用需求驱动的方法自动化微服务的部署、重配置、更新和扩展等流程。

Result: 该方法能够按需执行应用管理流程，减少计算资源消耗和网络流量，并在集体环境感知用例中验证了框架的有效性。

Conclusion: 提出的需求驱动应用管理方法能够有效应对C-ITS中的动态环境挑战，框架源代码已公开供研究使用。
...
<details>
  <summary>Details</summary>
Motivation: 人类皮肤提供丰富的触觉感知能力，能够在大的曲面区域定位有意和无意的接触事件。为灵巧机器人操作系统复制这些触觉感知能力仍然是一个长期挑战。

Method: 开发DexSkin电子皮肤，将其安装在平行夹爪手指上，提供几乎覆盖整个手指表面的触觉覆盖。在模仿学习框架下评估其在需要整个手指表面传感覆盖的挑战性操作任务中的能力。

Result: DexSkin在物体重定向和弹性带缠绕等接触丰富的操作任务中表现出色，能够实现传感器实例间的模型迁移，并适用于真实机器人的在线强化学习。

Conclusion: DexSkin展示了其对于学习真实世界、接触丰富的机器人操作的适用性和实用性，为灵巧机器人操作提供了有效的触觉传感解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统双边控制方法虽然利用关节角度、速度、扭矩和视觉信息实现精确操作，但需要针对每个任务训练特定模型，限制了通用性

Method: Bi-VLA结合机器人关节角度、速度、扭矩数据与视觉特征和自然语言指令，通过SigLIP和FiLM-based融合技术处理多模态信息

Result: 真实机器人实验表明，Bi-VLA能成功解释视觉-语言组合，相比传统双边控制模仿学习方法提高了任务成功率

Conclusion: Bi-VLA解决了先前双边方法只能处理单一任务的限制，证明视觉和语言的结合显著增强了系统的通用性
...
<details>
  <summary>Details</summary>
Motivation: 现有的机器人手形态设计方法依赖专家启发式方法和手动调整，自动化优化方法计算密集且依赖仿真，很少针对灵巧手设计。大语言模型具有广泛的人类-物体交互知识和强大生成能力，为零样本设计推理提供了有前景的替代方案。

Method: Lang2Morph流程包括：(i)形态设计：将任务映射为语义标签、结构语法和OPH兼容参数；(ii)选择与精炼：基于语义对齐和尺寸兼容性评估设计候选，并在需要时应用LLM引导的精炼。

Result: 在不同任务上的评估表明，该方法能够生成多样化、与任务相关的形态结构。

Conclusion: 这是首个基于LLM的任务条件化机器人手设计框架，展示了语言驱动设计在机器人形态生成中的潜力。
...
<details>
  <summary>Details</summary>
Motivation: VLA模型在机器人操作中表现出潜力，但其对真实世界物理变化的鲁棒性尚未得到充分探索。现有评估缺乏系统性，难以在保持可重复性的同时全面评估模型对各种物理变化的抵抗力。

Method: 1）将真实世界变化分解为三个关键领域：物体3D变换（影响空间推理）、光照变化（挑战视觉感知）和对抗性补丁（干扰场景理解）；2）引入连续黑盒优化框架，将离散物理变化转化为参数优化问题，系统探索最坏情况场景。

Result: 在多个基准测试中对最先进的OpenVLA模型进行广泛实验显示：所有变化类型都导致超过60%的失败率，其中物体变换在长时程任务中导致高达97.8%的失败率。

Conclusion: 研究揭示了VLA模型在受控实验室环境与真实部署准备度之间的关键差距，Eva-VLA框架为强化VLA基机器人操作模型对抗真实世界部署挑战提供了实用途径。
...
<details>
  <summary>Details</summary>
Motivation: 传统ICP算法在特征稀少环境和动态场景中容易出错，现有不确定性估计方法依赖手工模型或简化假设，而深度学习方法要么需要预建地图，要么只能进行二元分类，无法准确建模不确定性。

Method: 开发数据驱动框架，利用深度学习在ICP匹配前预测配准误差协方差，将每个LiDAR扫描与可靠的6-DoF误差协方差估计关联，实现与卡尔曼滤波的无缝集成。

Result: 在KITTI数据集上的大量实验表明，该方法能准确预测协方差，在基于预建地图的定位或SLAM应用中降低了定位误差并提高了鲁棒性。

Conclusion: 所提出的框架有效解决了ICP不确定性估计的挑战，为LiDAR定位和SLAM提供了更可靠的不确定性建模方法。
...
Method: 使用学习的前端检测稀疏的类别级语义关键点，利用线性主动形状模型表示未知形状，通过最大后验优化问题同时求解位置、方向和形状，采用自洽场迭代高效求解特征值问题。

Result: 求解器每次迭代仅需约100微秒，能快速剔除异常值，在合成数据和真实场景（包括两个公共数据集和无人机跟踪场景）中进行了测试。

Conclusion: 该方法实现了快速且具有全局最优性证明的形状和姿态估计，代码已开源。
...
<details>
  <summary>Details</summary>
Motivation: VLA模型的出现标志着机器人控制范式的转变，需要对这些新兴方法进行系统分类和全面分析，以推动通用机器人技术的发展。

Method: 通过分析300多项最新研究，提出了VLA方法的分类体系：基于自回归、基于扩散、基于强化学习、混合方法和专门方法，并详细考察了它们的动机、核心策略和实现。

Result: 建立了VLA研究的清晰分类框架，介绍了基础数据集、基准测试和仿真平台，为领域研究提供了系统参考。

Conclusion: 提出了VLA模型和通用机器人技术发展的关键挑战和未来方向，强调了构建可扩展通用VLA方法的机遇与挑战。
...
<details>
  <summary>Details</summary>
Motivation: 解决人形机器人行走中需要大量运动捕捉数据或复杂奖励设计的问题，提出一种无需人类演示就能学习自然步态的方法。

Method: 第一阶段：使用PPO训练4自由度降阶模型生成能量高效的步态模板；第二阶段：使用SAC加对抗判别器训练全身策略，确保步态特征分布与ROM演示匹配。

Result: 在1m/s和4m/s速度下，ROM-GRL产生稳定对称的步态，跟踪误差显著低于纯奖励基线方法。

Conclusion: ROM-GRL通过轻量级ROM指导高维策略，填补了纯奖励方法和模仿学习方法之间的空白，无需人类演示即可实现多功能、自然的人形机器人行为。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉触觉传感器在传感机制、结构尺寸等参数上存在差异，导致性能差异显著，缺乏标准化评估指标，难以针对特定任务进行选择和优化。

Method: 设计了一套性能指标和结构化实验流程，涵盖典型应用场景的关键特性，确保评估的一致性和可重复性。

Result: 该框架应用于多种不同传感机制的VBTS，结果表明能够提供全面的设计评估和各个性能维度的量化指标。

Conclusion: TacEva框架使研究人员能够按任务需求预先选择最合适的VBTS，并为VBTS设计优化提供性能指导。
...
<details>
  <summary>Details</summary>
Motivation: 现有模仿学习方法主要依赖纯视觉演示，无法有效处理需要精确力控制的接触丰富操作任务。

Method: 使用ManipForce系统采集RGB和力扭矩数据，通过频率感知多模态Transformer编码异步信号，利用双向交叉注意力在transformer扩散策略中融合多模态信息。

Result: 在齿轮装配、箱子翻转、电池插入等六个任务中，FMT平均成功率达到83%，显著优于纯视觉基线。
...
Method: 重新设计和重写了SlicerROS2模块，提高了模块化程度，增加了对低层级功能的访问，支持3D Slicer的Python API，并改进了数据传输协议。

Result: 开发了四个应用案例，展示了SlicerROS2在真实图像引导机器人场景中的核心功能应用。

Conclusion: 新设计的SlicerROS2模块为医学机器人研究提供了更强大和灵活的集成平台，能够有效支持图像引导的机器人干预应用。
...
<details>
  <summary>Details</summary>
Motivation: 机器人操作策略通常通过模仿学习初始化，但受限于专家数据的稀缺性。强化学习可以优化策略，但真实机器人训练成本高且不安全，而模拟器训练存在仿真到现实的差距。扩散模型在真实世界模拟方面表现出色，因此研究如何结合扩散模型来增强预训练策略。

Method: 提出World4RL框架，基于两个原则：1）在多任务数据集上预训练捕捉多样动态的扩散世界模型；2）在冻结的世界模型中完全优化策略，避免在线真实世界交互。设计了针对机器人操作的双热动作编码方案，并采用扩散骨干网络提高建模保真度。

Result: 广泛的仿真和真实世界实验表明，World4RL提供了高保真的环境建模，实现了持续的策略优化，相比模仿学习和其他基线方法显著提高了成功率。

Conclusion: World4RL框架成功地将扩散模型作为世界模型用于策略优化，为机器人操作提供了一种高效安全的策略精炼方法。
...
TL;DR: FunCanon框架将长时程操作任务分解为动作块序列，通过功能对象规范化实现功能对齐和自动操作轨迹迁移，提升策略的泛化能力和组合性。


<details>
  <summary>Details</summary>
Motivation: 解决端到端演示学习导致的任务特定策略泛化能力差的问题，使策略能够超越训练分布进行泛化。

Method: 使用功能对象规范化方法，基于大视觉语言模型的affordance线索将对象映射到共享功能框架中，训练对象中心和动作中心的扩散策略FuncDiffuser。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法依赖几何或语义标签无法区分视觉相似但材质不同的表面，而光谱传感器虽然能提供丰富的材质信息，但存在硬件集成复杂、成本高和处理计算密集的问题。

Method: 提出RS-Net神经网络，从RGB图像块预测光谱特征，然后将光谱特征映射到地形标签和摩擦系数，并集成到轮式机器人的运动规划和四足机器人的模型预测控制中。

Result: 开发了一个框架，在训练期间学习任务相关的物理属性，测试时仅需RGB感知，降低了硬件要求和计算成本。

Conclusion: RS-Net成功地将RGB感知与光谱数据的丰富信息连接起来，为机器人在复杂户外环境中的导航提供了更实用的解决方案。
...
TL;DR: BiGraspFormer是一个统一的端到端Transformer框架，直接从物体点云生成协调的双臂抓取，解决了现有方法中存在的协调问题。
...
Method: 提出Single-Guided Bimanual (SGB)策略：首先使用Transformer解码器生成多样化的单抓取候选，然后通过专门的注意力机制利用其学习特征来联合预测双臂姿态和质量分数。
...
TL;DR: 提出一种基于采样的多模态策略规划器，使用交叉熵方法优化多模态策略，提高对局部最小值的鲁棒性和解空间探索效率，适用于多机器人无碰撞规划和实时应用。


<details>
  <summary>Details</summary>
Motivation: 传统规划方法容易陷入局部最小值，多机器人规划中集中式优化计算复杂，需要一种能够处理多模态策略分布、提高鲁棒性和探索效率的规划方法。

Method: 采用后退时域采样规划框架，使用交叉熵方法在共同成本函数下优化多模态策略分布，支持多机器人分布式规划。

Result: 数值仿真显示多模态策略在陷阱环境和多机器人避碰中显著提高成功率，硬件实验验证了方法的实时可行性和实际性能。

Conclusion: 多模态策略规划方法有效提高了规划鲁棒性和成功率，适用于复杂环境下的多机器人实时规划任务。
...
TL;DR: MagiClaw是一个多功能双指末端执行器，通过软多面体网络和嵌入式摄像头实现6自由度力估计，结合iPhone提供多模态数据，用于人机操作技能迁移。


<details>
  <summary>Details</summary>
Motivation: 解决人机操作技能迁移中的"领域差距"问题，即人类演示与机器人执行在感知和形态学上的差异。

Method: 设计MagiClaw末端执行器，集成了软多面体网络（SPN）和嵌入式摄像头进行视觉力估计，结合iPhone提供6D位姿、RGB视频和LiDAR深度图，通过iOS应用实现实时遥操作和多模态数据流。

Result: 该系统降低了收集高保真接触丰富数据集的难度，加速了通用操作策略的开发。

Conclusion: MagiClaw通过统一的系统架构有效弥合了人机操作技能迁移的领域差距，为直观数据收集和策略部署提供了硬件一致性。
...
<details>
  <summary>Details</summary>
Motivation: 间歇性故障在机器人群体中频繁出现和消失，对可靠性和协调性构成重大挑战，但现有研究主要关注永久性故障。自组织神经系统（SoNS）方法首次实现了机器人群体自组织持久网络结构，为检测间歇性故障提供了可能。

Method: 采用主动-反应式策略：主动方面，机器人在故障发生前自组织动态备份路径；反应方面，使用一次性似然比检验比较多路网络中不同路径的信息，实现早期故障检测。检测到故障后，通信以自组织方式临时重路由。

Result: 在形成控制中故障位置数据的代表性场景中验证了该方法，证明间歇性故障不会破坏期望形成的收敛，具有高故障检测准确率和低误报率。
...
TL;DR: 提出了一种模仿引导的双臂规划框架，通过稳定抓取流形采样策略和分层双阶段运动架构，实现了在动态环境中无缝的抓取转换和优化的运动性能。


<details>
  <summary>Details</summary>
Motivation: 解决机器人在动态环境中进行抓取转换时的稳定性问题，现有方法难以有效应对变化的外部力和复杂运动约束。

Method: 采用模仿引导的双臂规划框架，包括稳定抓取流形采样策略实现单/双手抓取无缝转换，以及分层双阶段运动架构（基于模仿学习的全局路径生成器和二次规划驱动的局部规划器）。

Result: 在力密集型任务评估中显示出显著的抓取转换效率和运动性能提升。

Conclusion: 该方法有效提高了机器人在动态环境中的操作稳定性和灵巧性，为复杂操作任务提供了实用解决方案。
...
Result: 在仿真和真实世界任务中，SOE始终优于现有方法，获得更高的任务成功率、更平滑安全的探索和更好的样本效率。
...
TL;DR: 提出一种结合行为克隆和强化学习的残差学习框架，利用BC策略作为基础，通过样本高效的离策略RL学习轻量级残差修正，在高自由度系统上实现高效策略改进


<details>
  <summary>Details</summary>
Motivation: 行为克隆受限于人类演示质量和数据收集成本，而强化学习在真实机器人上训练面临样本效率低、安全性问题和稀疏奖励等挑战，需要结合两者优势

Method: 残差学习框架：使用BC策略作为黑盒基础策略，通过样本高效的离策略RL学习每步的轻量级残差修正，仅需稀疏二元奖励信号

Result: 在高自由度系统上成功改进操作策略，在仿真和真实世界中都有效，首次在具有灵巧手的人形机器人上实现成功的真实世界RL训练

Conclusion: 该方法为在真实世界部署RL提供了一条实用路径，在各种视觉任务中展示了最先进的性能
