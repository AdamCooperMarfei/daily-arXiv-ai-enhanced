...
Motivation: 现有的视觉基础模型通常在特定领域表现优异，但缺乏跨任务的通用性。将多个模型蒸馏为统一表示虽然能缓解这一问题，但往往产生不灵活的任务特定特征选择，且需要昂贵的全参数重训练来整合机器人领域知识。

Method: 提出VER框架：1）预训练阶段蒸馏多个视觉基础模型构建视觉专家库；2）仅微调轻量级路由网络（少于0.4%参数）动态选择任务相关专家；3）引入基于课程学习的Patchwise Expert Routing with Curriculum Top-K Annealing来提升专家选择的灵活性和精度；4）支持参数高效微调以扩展专家利用和自适应机器人知识整合。

Result: 在17个多样化机器人任务和多个策略头上，VER实现了最先进的性能。研究发现VER减少了任务无关区域（如背景）的大范数异常值，并聚焦于任务关键区域。

Conclusion: VER通过动态专家选择和参数高效微调，成功解决了多视觉基础模型融合的通用性问题，在机器人学习中表现出色，同时保持了计算效率。
...
<details>
  <summary>Details</summary>
Motivation: 传统分层规划中全局规划不考虑动力学，局部规划强制执行动力学约束，这种动力学不连续性在受限环境中容易导致轨迹跟踪失败。现有方法采用静态的动力学保真度递减方案，无法适应环境复杂度变化。

Method: 使用强化学习动态调整机器人动力学属性，将ADP集成到三种不同规划器中，并设计独立的ADP导航系统进行基准测试。

Result: 在仿真和真实世界测试中，ADP一致提高了导航成功率、安全性和效率。

Conclusion: ADP通过学习增强的范式成功解决了传统规划方法在动力学适应性方面的局限性，为自主机器人导航提供了更有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前触觉传感在机器人手中的应用受限，主要由于传感器成本高、制造集成困难以及从信号中提取可靠信息的挑战。

Method: 使用应变片传感器捕获静态力，接触式麦克风传感器测量高频振动，所有传感器集成在紧凑的指尖设计中，内部安装以避免直接磨损。

Result: 应变片传感器在0-5N范围内提供可重复的2D平面力测量，接触式麦克风能够区分接触材料属性，在三个灵巧操作任务中表现出色，特别是无视觉条件下精确计数和拆叠纸杯达到100%成功率。

Conclusion: 多模态触觉传感可以灵活应用于操作的不同阶段，单独或与视觉观察结合使用，显著提高任务性能，特别是在视觉受限情况下表现出明显优势。
...
TL;DR: 提出了一种自适应人机交互框架，通过整合关节活动度限制模型到分层最优控制器中，使机器人能够在线生成反应性、活动度感知的行为，帮助上肢残疾用户在协作任务中利用残余功能活动度。


<details>
  <summary>Details</summary>
Motivation: 传统人机协作方法假设用户为健全人，而实际工作环境对上肢残疾人士缺乏包容性，需要开发能够适应不同上肢活动度限制的自适应交互系统。

Method: 将特定关节活动度限制模型整合到分层最优控制器中，使机器人能够在线生成反应性、活动度感知的行为，引导用户受损肢体利用残余功能活动度。

Result: 在涉及不同上肢活动度限制（肘部和肩部关节炎、腕部阻塞）的交接任务中，该框架能够个性化交互以适应用户受损活动范围，并根据功能限制严重程度鼓励关节使用。

Conclusion: 该自适应框架能够有效促进上肢残疾用户的积极参与，通过个性化交互适应其活动度限制，为包容性工作环境提供了可行解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决在复杂室内环境中如何利用场景语义进行高效探索的问题，传统方法缺乏对语义信息的充分利用。

Method: 构建紧凑的层次化多层场景图表示环境，使用LLM采样与部分观测一致的未观测区域场景图，基于此计算信息增益指导探索。

Result: 在模拟的复杂3D室内环境中验证，相比基线方法能更快更准确地确定环境语义。

Conclusion: 该方法通过结合场景语义和主动感知，显著提升了复杂环境中的探索效率。
...
<details>
  <summary>Details</summary>
Motivation: 移动机器人在不确定环境中需要能够响应环境变化的动力学模型，特别是在直接环境信息有限的情况下。现有方法难以适应时空变化的环境条件。

Method: 基于神经常微分方程构建自适应动力学模型，使用两阶段训练程序学习潜在环境表示，通过状态-动作历史推断操作环境，无需直接环境知识。

Result: 在三个机器人平台上验证：2D差动轮式机器人（变化轮接触条件）、3D四旋翼（变化风场）和Sphero BOLT机器人（两种接触条件）。实验结果表明方法能有效处理时空变化的环境变化。

Conclusion: 该方法能够在不依赖直接环境知识的情况下适应环境变化，在仿真和真实系统中都表现出良好的环境适应能力，为模型预测控制提供了有效的自适应动力学模型。
...
<details>
  <summary>Details</summary>
Motivation: 精确的姿态和速度估计对于机器人机械臂的空间任务规划至关重要，传统集中式传感器融合方法存在局限性。

Method: 使用两个独立的自适应扩展卡尔曼滤波器，在矩阵李群上制定，通过相关感知融合规则在SE(3)流形上融合姿态和速度。

Result: 在UFactory xArm 850机器人上的实验验证了该方法的有效性和鲁棒性，相比现有方法有持续改进。

Conclusion: 提出的分散式双视角估计框架能够有效提升目标物体姿态和速度的估计精度。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个实用的系统，将自然语言指令连接到安全的本地机器人控制，通过结合RAG技术提高规划的有效性和适应性，同时保持感知和底层控制的本地化。

Method: 使用向量数据库索引机器人知识（运动模式、任务模板和安全启发式），为每个指令检索任务相关上下文，并基于大语言模型生成JSON结构化的动作计划。在UFactory xArm 850机械臂上执行，采用AprilTag检测与深度融合的感知方法，并通过软件安全门确保执行安全。

Result: 实验结果表明，基于RAG的规划方法显著提高了计划的有效性和适应性，同时保持了感知和底层控制的本地化。系统在桌面扫描、接近和抓取放置任务中表现出良好性能。

Conclusion: RAG-based规划可以显著改善计划的有效性和适应性，同时保持感知和低层控制的本地化，为自然语言指令到安全机器人控制的连接提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统人工势场方法在群体控制中容易陷入死锁和局部极小值，特别是在有障碍物的环境中，现有解决方案通常是被动的，导致集体导航效率低下。

Method: GO-Flock包含上游感知模块（处理深度图提取路径点和虚拟代理）和下游集体导航模块（应用新型APF策略），实现混合规划-反应式控制。

Result: 在障碍物密集环境中验证了GO-Flock，并与被动APF方法对比，展示了更好的群体行为和克服局部极小值的能力。硬件在环实验中成功控制了9架无人机（6架物理+3架虚拟）在森林环境中飞行。

Conclusion: GO-Flock框架有效解决了传统APF方法在复杂环境中的局限性，实现了高效的群体导航。
...
<details>
  <summary>Details</summary>
Motivation: 当前透明物体操作研究局限于短时程任务和基本抓取能力，缺乏对新物体的泛化性和精确长时程操作能力。

Method: 整合深度估计、6D姿态估计和视觉语言规划，采用单次演示方法泛化6D轨迹到新透明物体，无需类别先验或额外训练。

Result: 在长时程场景中显著优于现有透明物体操作方法，特别是在需要精确操作能力的任务上。

Conclusion: DeLTa框架有效解决了透明物体长时程精确操作的挑战，具有优秀的泛化性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有VLA模型在需要高精度的任务中存在根本性限制，而基于外部验证器的测试时缩放方法需要额外训练且无法泛化到未见条件

Method: 使用随机掩码状态和语言条件生成参考分布，提出联合训练策略学习条件和无条件分布，通过KL散度作为置信度指标从多个候选动作中选择最优动作

Result: 在真实世界任务中实现28%/35%的分布内/分布外性能提升，在RoboCasa拾放任务中相对增益达168%（仅用30个演示训练）

Conclusion: MG-Select框架显著提升了VLA模型的精度和泛化能力，无需额外训练或外部模块
...
<details>
  <summary>Details</summary>
Motivation: 解决高维视觉输入与敏捷动作输出结合带来的样本效率低和仿真到现实差距大的长期挑战。

Method: 两阶段学习：上游阶段使用掩码Transformer进行时间建模和对比学习提取视觉表示；下游阶段使用特权信息教师策略监督早期训练，逐步减少指导以促进独立探索。

Result: 在模拟和真实环境中的广泛实验表明，OMC-RL实现了优越的样本效率和渐近策略性能，同时提高了在多样化和感知复杂场景中的泛化能力。

Conclusion: OMC-RL框架通过解耦表示学习和策略学习，结合特权信息指导，有效解决了视觉运动策略学习中的关键挑战，具有实际应用价值。
...
Method: 使用神经常微分方程，通过投影神经向量场使其严格满足李雅普诺夫稳定性准则，利用灵活的神经网络参数化基向量场和李雅普诺夫函数

Result: 在单位四元数(S^3)和对称正定矩阵流形上解决了黎曼LASA数据集，以及机器人运动在R^3×S^3上的应用，通过广泛仿真和真实世界实验验证了性能

Conclusion: 该方法能够准确表示复杂轨迹同时尊重流形约束，展示了良好的性能、可扩展性和实际应用性
...
<details>
  <summary>Details</summary>
Motivation: 工业物联网系统特别是智能工厂中，数据隐私、通信效率和设备异构性是关键问题，需要研究适合资源受限机器人的联邦分割学习框架。

Method: 比较了同步、异步、分层和异构FedSL框架的工作流程、可扩展性和适应性；系统分类了输入级、中间级和输出级三种令牌融合策略；提出了模型压缩、分割层选择、计算频率分配和无线资源管理等优化技术。

Result: 仿真结果验证了这些框架在工业检测场景下的性能表现。

Conclusion: 为未来智能制造系统中的联邦分割学习研究指明了开放问题和研究方向。
...
<details>
  <summary>Details</summary>
Motivation: 确定性碰撞检查在不确定驾驶条件下往往不准确或过于保守，因为噪声感知、定位误差和其他交通参与者预测的不确定性给规划过程带来了显著不确定性。

Method: 第一种方法评估自动驾驶车辆与周围障碍物空间重叠的概率，第二种方法基于随机边界穿越估计碰撞概率。两种方法都包含完整状态不确定性（位置、方向、速度）。

Result: 仿真研究验证了所提方法与蒙特卡洛结果高度匹配，同时提供显著的运行时优势，适合实时风险感知轨迹规划。

Conclusion: 提出的碰撞概率估计方法实现了高精度和计算效率的平衡，为自动驾驶的风险感知轨迹规划提供了实用工具，并已开源发布。
...
Method: 采用协方差矩阵自适应进化策略(CMA-ES)持续优化下肢外骨骼的按需辅助控制器，在六名健康个体上进行为期两天的实验。

Result: CMA-ES算法似乎能为每个个体收敛到独特的刚度参数集，但在验证试验中未观察到对受试者表现的可测量影响。

Conclusion: 人机协同适应和人类行为变异性的影响可能大于个性化基于规则的辅助控制器的潜在益处，这揭示了当前外骨骼辅助步态康复个性化方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有语言驱动抓取方法缺乏足够的推理和泛化能力，或依赖复杂模块化流程，且当前抓取基础模型过度强调对话和对象语义，导致性能不佳且仅限于单对象抓取。

Method: 采用多轮处理范式，动态关注视觉输入并提供可解释的推理轨迹，使用包含167K合成图像和400+真实图像的VCoT-GraspSet数据集进行训练。
...
<details>
  <summary>Details</summary>
Motivation: 现有协同设计框架通常只优化最大高度或最小能耗，忽略了二者的权衡关系，且经常省略变速箱参数优化，使用过于简化的执行器质量模型，导致设计难以实际复现。

Method: 引入三阶段协同设计优化框架，明确包含真实的执行器质量模型，在统一框架内优化机械设计（包括变速箱）和控制参数，自动生成参数化CAD模型。

Result: 实验评估显示，相比基线设计机械能耗降低50%，同时跳跃高度达到0.8米。

Conclusion: 该框架能有效平衡跳跃高度与能耗的权衡，生成可直接制造的设计，显著减少手动设计迭代。
...
<details>
  <summary>Details</summary>
Motivation: 软体爬行机器人利用柔顺性和变形能力实现运动，但由于模型不准确、传感器噪声和需要发现运动步态，设计控制策略具有挑战性。

Method: 使用模型强化学习框架，从机载传感器推断潜在动力学作为预测模型，结合演员-评论家算法优化运动策略。在仿真中使用惯性测量单元和飞行时间传感器作为观测数据。

Result: 学习的潜在动力学能够实现短时域运动预测，演员-评论家算法发现了有效的运动策略。
...
<details>
  <summary>Details</summary>
Motivation: 行星探测任务需要能在极端环境中长期运行的鲁棒性移动系统，传统轮毂执行器易受热循环、粉尘和机械磨损影响。

Method: 采用双横臂悬架配置，结合万向节和绞盘驱动转向，将所有电机化组件置于受保护环境中，实现独立车轮牵引、转向控制和悬架管理。

Result: 设计满足50公里穿越要求且无性能下降，集成了粉尘防护机制和热管理解决方案，计划于2026年Q1进行1:3比例样机测试验证。

Conclusion: DISTANT设计通过重新定位执行器位置，为长距离行星探测任务提供了更可靠、耐用的移动系统解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前行星探测车的移动速度仅为约10厘米/秒，严重限制了探测效率，需要开发更先进的自主系统来提高速度和安全性。

Method: 采用三个组件：FASTNAV远距离障碍物检测器（计算机视觉）、CISRU多机器人协调框架（人机协作）、ViBEKO和AIAXR深度学习地形分类系统。

Result: 在火星模拟环境中验证达到技术成熟度4级，实现了1.0米/秒的持续移动速度，显著提高了移动速度、分类精度和操作安全性。

Conclusion: 这些集成AI系统为下一代行星任务提供了可衡量的性能改进，有望大幅提升行星探测的效率和自主性。
...
<details>
  <summary>Details</summary>
Motivation: SLAM方法的坐标系原点每次运行会重置，而UWB定位需要精确的锚点坐标分配。需要结合两者优势实现跨会话的坐标系一致性定位。

Method: 两阶段方法：第一阶段使用连续时间批量优化，结合距离、里程计数据、高度先验和锚点间距离因子恢复锚点3D位置；第二阶段使用滑动窗口优化方案融合UWB和SLAM数据。

Result: 在NTU VIRAL数据集上的六种无人机飞行场景实验表明，仅使用一次运行数据进行校准就足以在其余运行中实现精确定位。

Conclusion: 该方法成功实现了坐标系一致且精确的定位，融合了UWB和SLAM的优势，解决了跨会话定位的坐标系一致性问题。
...
<details>
  <summary>Details</summary>
Motivation: 多指灵巧抓握由于高维关节空间和基于优化的流水线成本而具有挑战性，现有端到端方法需要针对特定手型进行大规模训练，难以跨具身泛化。

Method: 从手部形态描述推导形态嵌入和特征抓握集，结合物体点云和手腕姿态，通过振幅预测器在低维空间中回归关节系数，使用运动学感知关节损失监督学习。

Result: 在三个灵巧手的未见物体上达到91.9%的平均抓握成功率，推理时间小于0.4秒；通过少样本适应到未见手型，在仿真中达到85.6%成功率，真实世界实验达到87%成功率。

Conclusion: 该框架实现了高效的跨具身灵巧抓握生成，在仿真和真实世界中都表现出色，具有很好的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 受蟑螂触角启发，开发能够高效探索未知环境的多机器人系统，应用于搜索救援、工业检测和环境监测等领域。

Method: 使用分布式优化方法，每个机器人配备触觉传感器，采用壁障跟随行为自主探索环境，记录碰撞点而非避障，通过分散控制策略实现任务分配。

Result: 在1.5×1.5米模拟环境中使用e-puck机器人进行实验验证，系统实现了高覆盖率、最小化碰撞并构建了准确的2D地图。

Conclusion: 该仿生多机器人系统在未知环境探索和建图方面表现出色，验证了分布式优化和壁障跟随策略的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 慢性伤口影响650万美国患者，年成本超250亿美元，但伤口护理仍依赖人工操作。机器人自动化可降低成本并改善患者预后。

Method: 使用基于力反馈的模仿学习处理胶带初始分离，开发数值轨迹优化方法确保平滑粘附和无皱褶应用。

Result: 通过大量实验验证了方法的可靠性，在定量评估和集成伤口换药流程中均表现良好。
...
<details>
  <summary>Details</summary>
Motivation: 在受控农业环境中，风力授粉不可用，商业授粉者使用受限，机器人授粉成为替代人工和熊蜂授粉的有前景方案。

Method: 使用末端执行器RGB-D传感器数据，进行3D植物重建和注册，识别无障碍抓取位姿；采用离散弹性杆模型预测驱动参数与花朵动态关系；软抓手抓取茎干并施加受控振动。

Result: 实验显示主茎抓取成功率达92.5%，振动参数优化验证了方法的可行性，确保机器人能安全有效地授粉而不损坏花朵。

Conclusion: 这是首个将视觉抓取和振动建模联合集成的自动化精确授粉机器人系统。
...
Method: 迁移到Unreal Engine 5.3，采用Fossen的先进车辆动力学模型，开发自定义ROS2桥接，并正在开发基于光线追踪的声纳实现、语义传感器等功能。

Result: HoloOcean 2.0成为能够支持多种任务的通用海洋模拟器，具备最先进的模拟功能。

Conclusion: HoloOcean 2.0通过集成最新技术和开发新功能，为海洋机器人系统开发提供了强大的仿真平台。
...
<details>
  <summary>Details</summary>
Motivation: 头发护理是日常必需活动，但对于行动不便者和自主机器人系统来说具有挑战性，因为头发具有细粒度物理结构和复杂动态特性。

Method: 采用基于动作条件的潜在状态编辑机制，结合紧凑的3D潜在空间来学习头发动态；使用新型头发物理模拟器进行大规模预训练；结合模型预测路径积分规划器进行视觉目标条件头发造型。

Result: 在模拟实验中，DYMO-Hair的动态模型在捕捉未见发型的局部变形方面优于基线方法；在闭环头发造型任务中，最终几何误差平均降低22%，成功率提高42%；在真实世界实验中，系统能够零样本迁移到假发上，在具有挑战性的未见发型上取得一致成功。

Conclusion: 该研究为基于模型的机器人头发护理奠定了基础，朝着在无约束物理环境中实现更通用、灵活和可访问的机器人头发造型迈进。
...
<details>
  <summary>Details</summary>
Motivation: 当前机器人控制方法虽然能遵循自然语言指令，但难以扩展到多样化环境，依赖大量标注数据且可解释性有限。需要一种更通用、可解释的方法来连接感知与操作。

Method: 采用基于编码的范式，直接使用编码模型生成可执行的机器人轨迹，无需额外数据收集或微调，支持灵活的对象几何参数化和操作轨迹合成。

Result: 在真实移动机器人上的实验表明，EmbodiedCoder在多样化长期任务中表现稳健，并能有效泛化到新对象和新环境。

Conclusion: 该方法提供了一种可解释的方式连接高级推理与低级控制，超越了固定原语，朝着通用机器人智能迈进。
