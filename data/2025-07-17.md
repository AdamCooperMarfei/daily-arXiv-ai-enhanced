...
Motivation: 高速公路匝道合流区是交通拥堵和事故的高发区，当前基于CAV的协作控制策略尚未普及，因此需要针对混合交通流提出解决方案。

Method: 扩展了基于智能驾驶员模型的纵向跟车模型和基于五次多项式曲线的横向换道模型，提出HCOMC框架，包括分层协作规划、基于博弈论的换道模型和多目标优化。

Result: 仿真结果表明，HCOMC在提升车辆群安全性、稳定合流过程、优化交通效率和节省燃油方面具有显著优势。
...
Result: 明确了机器人技术在能源优化、精准农业、环境监测等领域的应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 随着残疾人口增长，现有电动轮椅的完全自主或手动控制模式灵活性不足，难以在空间受限环境中导航。本文旨在通过共享控制模式提升轮椅的导航效率和用户信任。

Method: 开发了基于ROS的CoNav Chair，结合共享控制和障碍物避障功能。通过21名健康参与者对比手动、共享和完全自主三种导航模式在室内环境中的表现。

Result: 共享控制模式在碰撞次数、任务完成时间、轨迹长度和平滑度上表现优异，且用户主观评价认为其更安全和高效。

Conclusion: CoNav系统展示了良好的安全性和性能，为后续针对残疾用户的测试奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 机器人领域中，环境数据的多样性和格式不兼容性导致数据整合困难，影响机器人认知控制。

Method: 开发统一场景图模型，将MJCF、URDF和SDF等格式标准化为USD格式，并通过语义标注和知识图谱转换实现数据整合。

Result: 成功将3D环境数据转换为USD格式并生成知识图谱，支持实时机器人决策，同时开发了可视化工具辅助语义映射。
...
Method: 通过仿真、人类演示和远程操作收集灵巧操作数据，并采用模仿学习和强化学习框架进行技能学习。

Result: 总结了现有数据收集范式和技能学习框架，并提出了三个限制灵巧机器人操作发展的关键挑战。

Conclusion: 灵巧机器人操作的发展仍面临数据收集和学习框架的挑战，需进一步研究解决。
...
<details>
  <summary>Details</summary>
Motivation: 随着微型交通工具（如电动滑板车和电动自行车）的快速普及，开发可靠的AR技术成为迫切需求，但AR面临独特的挑战，如两轮平台的不稳定性、尺寸和功率限制以及环境不可预测性。

Method: 通过系统分析AR的核心组件（感知、规划和控制），并从AD技术的角度审视AR的现状，识别当前研究的不足。

Result: 发现当前AR研究存在感知系统不完善、行业和政府支持不足以及研究关注度低等问题，并提出了多模态传感器技术和边缘深度学习架构等研究方向。
...
<details>
  <summary>Details</summary>
Motivation: 系留机器人在特殊环境中具有优势，但其运动规划受限于系绳长度和缠绕风险，需要高效的路径规划方法。

Method: 利用CDT编码作为同伦不变量表示路径拓扑状态，结合代数拓扑和几何优化，提出CDT-TCS算法及其三种应用算法。

Result: 仿真和实验表明，所提算法在各自问题领域显著优于现有方法，并验证了其工程实用性。

Conclusion: CDT-TCS及其衍生算法为系留机器人运动规划提供了高效解决方案，具有理论和工程价值。
...
<details>
  <summary>Details</summary>
Motivation: 当前AUV平台通常执行静态任务或依赖高延迟通信，限制了适应性和响应能力。

Method: 提出NemeSys系统，包括设计、控制架构和语义任务编码框架，通过低带宽通信实现交互式探索和任务适应。

Result: 通过建模、实验和开放水域测试验证了系统的可行性，支持在线任务适应和语义更新。
...
<details>
  <summary>Details</summary>
Motivation: 解决密集动态环境中实时路径规划的挑战，避免预测大量障碍物运动的高计算负担。

Method: 结合多种预测模型，利用P-CRI选择性评估障碍物风险，并通过混合共形预测量化不确定性。

Result: 理论分析和仿真表明，HyPRAP在安全性和计算效率上优于单一预测方法，P-CRI优于基于距离的风险评估。

Conclusion: HyPRAP在复杂环境中有效平衡了安全性和实时性，适用于大规模障碍物场景。
...
Method: 1) 利用视觉特征进行相似性匹配；2) 基于候选模型的抓取知识规划模仿抓取；3) 通过局部优化提升抓取质量。

Result: 提出了一种多级相似性匹配框架，结合语义、几何和尺寸特征，并引入新的点云描述符C-FPFH。

Conclusion: 新方法通过相似性匹配和局部优化，显著提升了单视角抓取未知物体的鲁棒性和准确性。
...
Method: 结合神经网络预测周围车辆的反应，并引入基于样条的MPPI采样分布先验，优化车道变换行为。
...
Result: GAI在水下感知、数字孪生建模和自主规划等方面发挥重要作用，但仍面临数据不足、实时性能限制等挑战。

Conclusion: GAI是推动智能、韧性和环保水产养殖系统的关键工具，但需解决技术和监管问题。
...
<details>
  <summary>Details</summary>
Motivation: 高风险交通区域（如交叉路口）是碰撞事故的主要原因，研究旨在提升自动驾驶车辆在此类场景中的安全性。

Method: 训练1000步去噪扩散概率模型生成碰撞相关的传感器噪声序列，并通过生成对抗架构将其蒸馏为单步模型，用于快速推理。

Result: 单步模型在保持采样质量的同时实现快速推理，基于此构建的规划器在仿真实验中显著降低了失败率和延迟率。

Conclusion: 单步去噪扩散模型为自动驾驶车辆在交叉路口的安全决策提供了高效且可靠的解决方案。
...
Method: 集成优化与模拟，采用预算、椭球和支持向量聚类（SVC）方法及分布鲁棒方法解决最短路径问题。

Result: 鲁棒路径规划显著提升操作可靠性，椭球和DRSP方法表现最佳。

Conclusion: 鲁棒方法在恶劣天气和高行人密度下效果更显著，尤其适合宽型、低速机器人。
...
<details>
  <summary>Details</summary>
Motivation: 果园中GPS信号不可靠且标准SLAM方法因树木重复外观易出错，需一种精准的单棵树定位方法以支持自主机器人任务。

Method: 利用RGB-D图像检测树干，通过级联图数据关联算法重识别树干，结合因子图框架整合GPS、里程计和树干观测。

Result: 在苹果和梨园数据集上验证，定位误差低至18厘米，低于种植距离的20%，且对不可靠GPS信号具有鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法收集步行性相关数据成本高且难以扩展，而人行道配送机器人提供了自动化、可扩展的解决方案。

Method: 在斯德哥尔摩KTH部署传感器机器人，完成101次行程，收集人行道特征数据并分析其与步行性的关系。

Result: 行人行为受人行道特征（如宽度、密度、表面不平）显著影响，机器人速度可作为行人动态的代理指标。

Conclusion: 该框架支持持续监测人行道条件和行人行为，有助于建设更适宜步行、包容性强的城市环境。
...
<details>
  <summary>Details</summary>
Motivation: 工业自动驾驶车辆在安全关键环境中的部署增加，需确保其在不同条件下的安全运行。
...
Method: 将意图不确定性下的交互建模为贝叶斯博弈，并证明其在一定条件下可转化为潜在博弈；提出基于ADMM的分布式算法并行求解。

Result: 方法在多种意图不确定性场景下有效，可扩展性优于现有集中式和分散式基线，支持实时规划。

Conclusion: 新方法解决了意图不确定性带来的挑战，显著提升了多智能体轨迹规划的效率和可扩展性。
...
<details>
  <summary>Details</summary>
Motivation: 现有LGL方法通常仅考虑部分信息或针对单一传感器，忽略了均匀性。UniLGL旨在解决这一问题。

Method: 将点云编码为空间和强度BEV图像，设计多BEV融合网络提取均匀特征，引入视角不变性假设确保传感器类型均匀性，并推导全局姿态估计器。

Result: 实验表明UniLGL优于现有方法，并在多种平台上成功部署，适用于工业和野外场景。

Conclusion: UniLGL通过统一设计显著提升了LGL的性能和适用性。
...
<details>
  <summary>Details</summary>
Motivation: 探索AI驱动机器人在文化空间中如何提升用户体验和知识获取，同时揭示实际部署中的挑战。

Method: 设计并实现基于LLMs和SLAM技术的机器人，通过34名参与者的实地测试，结合定性和定量分析。

Result: 机器人受到普遍欢迎，提升了博物馆体验，但在理解和响应方面存在不足。

Conclusion: AI机器人有潜力支持文化空间的可访问性和学习，但需克服实际部署中的技术限制。
...
Result: 提出了一种鲁棒的搜索方法，能够高效处理复杂的多次抓取操纵问题。

Conclusion: 该方法通过结合regrasp map和TAMP求解器，显著提升了在复杂环境中的操纵效率。
...
Conclusion: ACAT的设计和实施展示了其在自动化表面测试中的潜力，为未来集成提供了基础。
...
<details>
  <summary>Details</summary>
Motivation: 机器人数据收集受硬件限制，难以规模化；人类视频数据丰富且场景多样，可弥补这一不足。

Method: 训练VLA模型预测人类手腕和手部动作，通过逆运动学和重定向转换为机器人动作，并用少量机器人演示微调模型。

Result: 在Isaac Humanoid Manipulation Benchmark上，EgoVLA表现显著优于基线，验证了人类数据的重要性。

Conclusion: 人类视频数据可用于高效训练机器人策略，为机器人模仿学习提供了新思路。
