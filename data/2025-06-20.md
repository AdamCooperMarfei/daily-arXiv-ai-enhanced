...
<details>
  <summary>Details</summary>
Motivation: MPPI控制虽然灵活，但在实时高频场景中计算需求高，限制了其应用。

Method: F-MPPI通过灵敏度分析计算局部线性反馈增益，实现快速闭环修正，无需每步重新优化。
...
Conclusion: 系统在无人机与视障人士协同导航中表现出实用性和有效性。
...
<details>
  <summary>Details</summary>
Motivation: 机器人运动生成涉及复杂几何约束和多目标行为，现有方法在特定问题域或几何约束利用上存在不足。

Method: 采用增强拉格朗日谱投影梯度下降（ALSPG），结合欧几里得投影、Minkowski和及基函数，有效利用几何约束。
...
<details>
  <summary>Details</summary>
Motivation: 解决家庭用餐辅助中活动多样性、情境复杂性和用户偏好差异的挑战。

Method: 采用模块化硬件、多样化交互方法和参数化行为树，结合大型语言模型实现安全透明的个性化。

Result: FEAST在透明性和安全性上优于固定定制基线，并通过用户研究验证了其实际适用性。

Conclusion: FEAST成功实现了个性化需求，适用于真实世界场景。
...
<details>
  <summary>Details</summary>
Motivation: 四旋翼无人机在复杂未知环境中实现自主导航面临传感器限制和实时决策的挑战。

Method: 结合立体和单目学习深度估计构建密集3D地图，开发快速全局轨迹规划框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有无人机遥操作未能充分利用全向旋转自由度，且未充分利用人类手指的灵活性。

Method: 系统包括肩部和手部运动追踪标记及数据手套，设计了四种交互模式（球形、笛卡尔、操作和锁定模式）。

Result: 在真实阀门旋转任务中验证了系统的有效性。
...
Result: 实验表明，该方法样本效率显著提高，仅需基线1/10的偏好查询即可达到相同奖励准确度，低数据量下性能提升15%。用户研究验证了其有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有任务分配方法假设机器人是单任务的，这在实际场景中可能效率低下或不可行。

Method: 提出了一种任务分配框架，考虑了多任务引入的物理约束，并通过加权MAX-SAT编译和贪心启发式方法求解。

Result: 在合成领域和模拟场景中验证了多任务的优势，并通过物理实验展示了任务效率的提升。

Conclusion: 多任务分配框架在实际应用中显著提高了任务效率。
...
Method: 使用轻量级Matcha-TTS作为语音生成框架，引入表情符号提示实现细粒度表达控制，并通过三个案例研究验证效果。

Result: 在讲故事任务中，变化的表情符号提示提升了语音的感知和表达性，但在助手场景中表达性语音不受欢迎。

Conclusion: EmojiVoice为社交机器人提供了灵活的表达性语音生成方案，但需根据应用场景调整表达性策略。
...
<details>
  <summary>Details</summary>
Motivation: 为了实现类人触觉感知，需要将传统平面VTS扩展到仿生曲面结构，但现有技术面临光照不足、重建模糊和复杂边界条件等挑战。

Method: 1. 开发GelSplitter3D，通过棱镜和近红外相机扩展成像通道；2. 提出光测立体神经网络，结合CAD生成法向量真值校准触觉几何；3. 设计边界约束法，利用深度先验信息修正表面积分累积误差。

Result: 实验显示触觉感知性能显著提升，法向量估计精度提高40%，传感器形状在抓取和操作任务中表现出优势。

Conclusion: 该方法成功解决了曲面触觉感知的技术难题，为类人指尖设计提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有方法因固定角度或距离间隔限制而无法自由选择导航目标的问题，提出动态动作空间和协作图记忆的创新方案。

Method: 采用自优化图记忆存储对象位置为可执行拓扑关系，支持分布式图更新和检索增强，无需任务特定训练或微调。

Result: 在GOAT和ObjectNav基准测试中表现优异，实际测试验证了其鲁棒性和泛化能力。

Conclusion: DyNaVLM通过动态动作空间、协作图记忆和无训练部署的创新，为可扩展的机器人导航建立了新范式。
...
<details>
  <summary>Details</summary>
Motivation: 现有语言教学机器人缺乏针对任务优化的合成语音，可能影响教学效果。

Method: 1. 使用微调的Matcha-TTS和表情符号提示创建表达力强的语音；2. 研究语音如何适应物理和社交环境；3. 开发针对L2学习者的英语TTS系统，优化元音清晰度。

Result: 1. 语音更具表达力且适合长时间使用；2. 环境适应性调整使语音更合适；3. L2清晰模式减少了转录错误并提升了理解。

Conclusion: 提出的语音系统显著提升了语言教学机器人的表达力和教学效果，尤其在L2学习者中表现优异。
...
Method: 利用单波束声纳改进尺度估计，结合磁力计和磁场线圈进行位置识别，并开发分层视觉-磁性位置识别方案。
...
TL;DR: 该论文提出了一个全面的代码框架，用于简化人形机器人运动策略从仿真到现实的迁移，并通过实验验证了其有效性。
...
Method: 开发了一个涵盖训练到部署全流程的代码框架，包括强化学习方法、域随机化、奖励函数设计和并行结构处理。
...
Method: 开发了TACT策略，通过模仿学习人类遥操作数据，整合关节位置、视觉和触觉输入，并结合双足模型实现平衡与行走。

Result: 实验验证表明，TACT策略能有效提升涉及广泛和精细接触的操作鲁棒性，并使机器人RHP7 Kaleido实现全身接触操作。

Conclusion: TACT策略通过多模态输入和模仿学习，成功解决了全身接触操作的挑战，提升了人形机器人的操作能力。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有步态相位估计方法在地形变化时精度和鲁棒性不足的问题，以提升外骨骼系统的适应性和人机交互效率。

Method: 开发了一种基于隐式建模的神经网络，结合时间卷积和Transformer层，并采用通道掩码重建预训练策略。

Result: 在稳定地形下，步态相位RMSE为2.729±1.071%，相位率MAE为0.037±0.016%；地形变化时RMSE为3.215±1.303%，MAE为0.050±0.023%。硬件验证证实算法可靠。

Conclusion: 该方法为更智能、自适应的外骨骼系统奠定了基础，提升了多样现实环境中的安全性和效率。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有上下文模仿学习（In-Context IL）中LLM生成的轨迹存在幻觉问题，导致机器人任务可靠性下降的挑战。

Method: 提出RIP算法，利用Student's t分布聚合LLM生成的多个候选轨迹，忽略异常值（幻觉），生成鲁棒轨迹。

Result: 实验表明，RIP在模拟和真实环境中均优于现有方法，任务成功率至少提高26%，尤其在低数据场景下表现突出。

Conclusion: RIP通过鲁棒性设计有效解决了LLM轨迹幻觉问题，提升了机器人模仿学习的可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 当前雷达研究多局限于同质传感器，忽视了异构雷达的集成和跨模态挑战，导致难以泛化到多样化雷达数据类型。

Method: 采用RCS极坐标匹配对齐多模态雷达数据，基于分层最优传输的特征聚合方法生成旋转鲁棒的多尺度描述符，结合FFT相似性数据挖掘和自适应边界三元组损失实现FOV感知度量学习。
...
Method: 扩展DeLaN为上下文感知模型，结合递归网络进行在线系统识别，并与MPC集成；同时引入残差动力学模型利用机器人名义模型。
...
<details>
  <summary>Details</summary>
Motivation: 机器人安全性日益重要，传统防御方法不足以应对复杂攻击，需结合进攻性策略。

Method: 结合机器学习与博弈论，优化漏洞识别与利用流程，提出新型网络安全认知引擎架构。

Result: 开发出能自主执行进攻性安全策略的机器人系统，显著提升其自我防护能力。

Conclusion: 进攻性安全策略与机器人架构设计紧密结合，为未来自主防御机器人系统奠定基础。
...
### [51] [Efficient Navigation Among Movable Obstacles using a Mobile Manipulator via Hierarchical Policy Learning](https://arxiv.org/abs/2506.15380)
*Taegeun Yang,Jiwoo Hwang,Jeil Jeong,Minsung Yoon,Sung-Eui Yoon*
...
<details>
  <summary>Details</summary>
Motivation: 解决在动态障碍物环境中导航时，移动机械臂如何高效、稳定地完成任务的问题。

Method: 采用分层强化学习框架，高层策略生成推动命令，低层策略执行协调全身动作。

Result: 实验表明，该方法在成功率、路径长度和到达时间上优于基线方法。
...
### [52] [MCOO-SLAM: A Multi-Camera Omnidirectional Object SLAM System](https://arxiv.org/abs/2506.15402)
*Miaoxin Pan,Jinnan Li,Yaowen Zhang,Yi Yang,Yufeng Yue*
...
### [53] [SurfAAV: Design and Implementation of a Novel Multimodal Surfing Aquatic-Aerial Vehicle](https://arxiv.org/abs/2506.15450)
*Kun Liu,Junhao Xiao,Hao Lin,Yue Cao,Hui Peng,Kaihong Huang,Huimin Lu*
...
### [54] [Real-Time Initialization of Unknown Anchors for UWB-aided Navigation](https://arxiv.org/abs/2506.15518)
*Giulio Delama,Igor Borowski,Roland Jung,Stephan Weiss*
...
<details>
  <summary>Details</summary>
Motivation: 解决UWB辅助导航系统中未知锚点的手动设置问题，提升自动化和实时性。

Method: 结合在线PDOP估计、轻量级异常检测和自适应鲁棒核优化，实现自动检测和校准。

Result: 在自主叉车和四旋翼机器人上验证，显示鲁棒初始化和低定位误差。

Conclusion: 方法优于现有技术，适用于实际应用，并开源了C++库和ROS封装。
...
### [55] [Aerial Grasping via Maximizing Delta-Arm Workspace Utilization](https://arxiv.org/abs/2506.15539)
*Haoran Chen,Weiliang Deng,Biyu Ye,Yifan Xiong,Ximin Lyu*

Main category: cs.RO

TL;DR: 提出了一种新型空中抓取规划框架，通过优化机械臂轨迹和利用MLP与RevNet技术，最大化工作空间利用率，提升操作效率。


<details>
  <summary>Details</summary>
Motivation: 工作空间限制了机械臂系统的操作能力和运动范围，最大化工作空间利用率可提升空中抓取任务的灵活性和效率。

Method: 提出优化问题以规划机械臂轨迹，利用MLP映射位置点到可行性概率，并采用RevNet近似Delta臂的正向运动学，消除工作空间约束。
...
### [56] [GRIM: Task-Oriented Grasping with Conditioning on Generative Examples](https://arxiv.org/abs/2506.15607)
*Shailesh,Alok Raj,Nayan Kumar,Priya Shukla,Andrew Melnik,Micheal Beetz,Gora Chand Nandi*
...
<details>
  <summary>Details</summary>
Motivation: 任务导向抓取（TOG）需要理解任务语义、对象功能和抓取约束，现有学习方法泛化能力有限，GRIM旨在解决这一问题。

Method: GRIM采用粗对齐策略（几何线索和PCA降维的DINO特征）和细化步骤（任务无关的几何稳定抓取）来实现任务导向抓取。

Result: GRIM在少量示例条件下表现出强大的泛化能力和稳健性能。

Conclusion: GRIM为任务导向抓取提供了一种高效且无需训练的新方法，具有广泛的应用潜力。
...
### [57] [Vision in Action: Learning Active Perception from Human Demonstrations](https://arxiv.org/abs/2506.15666)
*Haoyu Xiong,Xiaomeng Xu,Jimmy Wu,Yifan Hou,Jeannette Bohg,Shuran Song*

Main category: cs.RO

TL;DR: ViA是一个用于双手机器人操作的活动感知系统，通过学习人类演示的任务相关感知策略，结合6自由度机械颈和VR远程操作界面，显著优于基线系统。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够学习人类活动感知策略的机器人系统，以提升复杂双手操作任务的性能。

Method: 使用6自由度机械颈实现灵活头部运动，设计VR远程操作界面共享观察空间，并通过3D场景表示减少VR运动眩晕。

Result: 在涉及视觉遮挡的多阶段双手操作任务中，ViA显著优于基线系统。

Conclusion: ViA通过结合硬件和界面设计，成功学习了人类活动感知策略，提升了机器人操作的鲁棒性。
...
### [58] [Particle-Grid Neural Dynamics for Learning Deformable Object Models from RGB-D Videos](https://arxiv.org/abs/2506.15680)
*Kaifeng Zhang,Baoyu Li,Kris Hauser,Yunzhu Li*
