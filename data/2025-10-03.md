...
Motivation: 解决无人机在GNSS拒止环境中长距离飞行时的航迹推算漂移问题，特别是在未知区域缺乏闭环检测且计算资源有限的情况下。

Method: 系统集成感知、建图、规划和控制，采用轻量级漂移校正方法：通过梯度模板匹配将激光雷达获取的局部高度图与先验地理高度图匹配，并在聚类粒子滤波中融合证据与航迹推算数据。

Result: 在竞赛中成功执行了千米级飞行，覆盖城市、森林和开阔地带，相对于原始航迹推算显著减少了漂移，并在仅使用CPU的硬件上实时运行。

Conclusion: 该系统为GNSS拒止环境下的无人机自主飞行提供了实用的设计思路，通过高度图匹配和粒子滤波有效解决了长距离导航中的漂移问题。
...
<details>
  <summary>Details</summary>
Motivation: 传统运动规划方法在狭窄内河水道等挑战性空间中往往计算密集或过于保守，需要更高效且安全的方法。

Method: 使用MPC提供近似运动规划，结合高阶CBFs确保安全性，采用时变膨胀椭圆障碍物表示，根据船舶与障碍物的相对位置和姿态调整膨胀半径。

Result: 仿真和真实世界实验表明，该策略能使全驱动自主机器人船舶实时通过狭窄空间，解决潜在死锁，同时确保安全。

Conclusion: 提出的自适应膨胀方法相比传统固定椭圆障碍物表述减少了控制器的保守性，实现了实时安全导航。
...
Result: 能够连续估计机器人的位姿、速度和应变的均值和协方差，支持时间和空间上的连续插值，计算复杂度与时间呈线性关系。
...
Method: 使用自监督跟踪模型生成路径掩码，结合VLM增强的标注进行训练，避免手动像素级标注；通过图像扩散模型生成视觉路径规划，再由行为克隆策略执行轨迹。

Result: 在真实世界评估中，VENTURA在物体到达、障碍物避让和地形偏好任务上优于最先进的基础模型基线，成功率提高33%，碰撞率降低54%，并能泛化到未见过的任务组合。

Conclusion: VENTURA展示了在机器人导航中利用视觉语言模型的潜力，通过视觉路径规划方法实现了更好的泛化能力和组合能力，为开放世界机器人导航提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的VLA模型缺乏自省机制来预测失败并请求人类监督，需要建立有效的帮助触发机制。

Method: 使用π₀-FAST作为基础模型，提取token级的熵、对数概率以及Dirichlet不确定性估计，训练紧凑的transformer分类器将这些序列映射到帮助触发器。比较了强监督和弱监督两种监督机制。

Result: 强监督标签能捕捉细粒度不确定性动态以实现可靠的帮助检测，而弱监督标签虽然噪声较大，但在训练和评估对齐时仍能提供有竞争力的自省能力。建模token级不确定性信号的时间演化比静态序列级评分具有更强的预测能力。

Conclusion: 这是对VLA中基于不确定性的自省机制的首个系统评估，为主动学习和通过选择性人类干预实现实时错误缓解开辟了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于碰撞锥或速度障碍的CBF方法过于保守，在密集场景中容易导致二次规划不可行，限制了非完整机器人在复杂动态环境中的安全导航能力。

Method: 设计动态抛物线控制屏障函数，抛物线的顶点和曲率根据与障碍物的距离和相对速度大小动态调整，形成更宽松的安全约束。

Result: 相比基线方法，DPCBF控制器显著提高了导航成功率和QP可行性，能在多达100个动态障碍物的密集环境中成功导航，而碰撞锥方法因不可行而失败。

Conclusion: DPCBF为具有输入约束的自行车模型提供了有效的安全保证，在密集动态环境中表现出优越的性能和可行性。
...
Method: 通过双手机器人拾取放置任务的案例研究，分析三个因素对训练策略的影响：数据集大小、数据集质量和流形曲率。

Result: 实验表明扩散策略能学习约束流形的粗略近似，数据集大小和质量的降低都会对学习产生负面影响，而约束流形曲率与约束满足度和任务成功率的关联不明确。

Conclusion: 硬件评估验证了研究结果在现实世界中的适用性，扩散策略在适当的数据条件下能够学习运动学约束，但学习精度受数据质量影响。
...
<details>
  <summary>Details</summary>
Motivation: 解决基于视觉的机器人学习中密集图像或点云输入计算量大、包含无关背景特征的问题，以及现有基于关键点的方法依赖手动启发式或任务耦合选择，限制可扩展性和语义理解。

Method: 采用三阶段流程：1) 可操作性过滤；2) 类别级关键点构建；3) 基于transformer的策略学习，包含嵌入门控机制来推理最相关的关键点。

Result: 构建了紧凑的38维状态策略，可在15分钟内训练完成，在未见过的物体、新类别、背景和干扰物上达到82%的成功率，无需本体感知或密集表示。

Conclusion: AFFORD2ACT通过可操作性引导的语义关键点提取，显著提高了数据效率，在多样化真实世界操作任务中表现优异，证明了其可扩展性和语义理解能力。
...
Method: 使用可微分物理模拟精确建模颗粒动力学，采用低维技能空间参数化降低优化复杂度，并通过课程学习策略逐步提高长期任务能力。
...
<details>
  <summary>Details</summary>
Motivation: 保乳手术中肿瘤边界界定困难，因为肿瘤通常高度移动、无法触诊且边界不规则，需要更好的定位方法。

Method: 将小型触觉机器人改装为协作控制的手术工具，使用超声和电磁导航识别肿瘤边界，当手术工具碰撞肿瘤边界时施加禁止区域虚拟夹具。

Result: 虚拟夹具引导改善了切除边缘，用户在使用触觉反馈时感觉任务更轻松、挫败感更少、所需努力更少。

Conclusion: 虚拟夹具能在模拟保乳手术中帮助定位肿瘤边界，未来将进行更广泛的用户研究来验证结果并优化系统。
...
<details>
  <summary>Details</summary>
Motivation: 解决视觉语言模型在机器人导航中的三个基本限制：缺乏持久场景记忆、空间推理能力有限、视频时长扩展性差无法实时应用。

Method: 使用现代VLM分块处理视频序列，构建持久知识图谱保持对象身份一致性，通过可查询图结构实现可解释的空间推理。

Result: 在真实差分驱动机器人上部署，达到77.27%成功率和76.92%答案准确率，性能匹配Gemini 2.5 Pro，同时提供可解释推理和计算效率。

Conclusion: VL-KnG系统在机器人导航任务中表现出色，提供可解释推理、计算效率，适用于定位、导航和规划等不同任务。
...
<details>
  <summary>Details</summary>
Motivation: 解决自由漂浮、生物启发的多连杆机器人在欠驱动和最小感知条件下的姿态估计问题，这类平台由于关节未驱动且传感器有限，状态估计具有挑战性。

Method: 使用无迹卡尔曼滤波，并增强高斯过程残差学习来补偿非零均值和非高斯噪声。通过概念验证硬件实验和离线卡尔曼滤波分析进行验证。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：传统采样方法难以控制分辨率和扩展性，模仿学习方法需要大量演示数据，而ANTFields方法受谱偏差和灾难性遗忘影响，在复杂场景中效果不佳

Method: 分层规划结构：高层使用稀疏图捕捉环境全局连通性，低层使用基于神经场的规划器通过求解Eikonal PDE来导航局部障碍，采用物理信息策略克服谱偏差和神经场拟合困难

Result: 在大规模环境中验证了框架的有效性，相比先前方法展现出更强的适应性和精度，能够实现平滑精确的成本景观表示

Conclusion: 该方法在在线探索、建图和真实世界导航方面具有潜力，通过分层结构成功解决了复杂室内环境中的机器人导航挑战
...
<details>
  <summary>Details</summary>
Motivation: 现有在线平面映射方法难以平衡精度和计算效率：直接深度图像分割缺乏时间整合，高度图方法无法表示复杂3D结构，体素基平面分割在实时应用中尚未探索。

Method: 开发了一个新颖框架，将基于顶点的连通分量标记与随机采样一致性平面检测和凸包相结合，利用GPU并行计算从高分辨率3D体素地图中快速提取平面区域。

Result: 实验结果表明，即使在0.01米分辨率下，该方法也能以超过30Hz的更新速率实现快速准确的3D多平面分割，使检测到的平面能够实时用于运动任务。
...
<details>
  <summary>Details</summary>
Motivation: 传统双手机器人系统复杂且工作空间利用率低，需要开发更紧凑高效的双手操作方案。

Method: 设计耦合两个低自由度机械臂的MiniBEE系统，制定运动灵巧度指标优化设计，支持穿戴式数据收集和标准机器人部署。

Result: 实现了紧凑的双手操作系统，能够进行穿戴式演示数据收集，并训练出稳健的实时双手操作策略。

Conclusion: MiniBEE系统提供了一种轻量级、可穿戴的双手操作解决方案，扩展了机器人的灵巧工作空间。
...
<details>
  <summary>Details</summary>
Motivation: 解决机器人复杂双手操作的数据收集问题，通过便携式系统捕获人类演示，并学习视觉注意力与操作之间的关键联系。

Method: 结合便携式VR遥操作套件和传感器控制器，通过精确姿态对齐桥接人机运动学，引入沉浸式3D模型渲染、可穿戴计算机和高效校准方法。

Result: 在六个挑战性双手任务中，仅使用ActiveUMI数据训练的策略在分布内任务上达到70%平均成功率，在新物体和新环境中保持56%成功率。
...
<details>
  <summary>Details</summary>
Motivation: 现有机器人操作数据集主要提供地面真实轨迹，一旦发生失败机器人无法恢复，而少数处理失败检测的数据集通常只提供难以直接用于VLA模型的文本解释。

Method: 提出FailSafe系统，可在任何模拟器中无缝应用于任何操作任务，实现失败-动作数据的可扩展创建，并微调LLaVa-OneVision-7B构建FailSafe-VLM。

Result: FailSafe-VLM成功帮助机器人手臂检测和恢复潜在失败，将三个最先进VLA模型(pi0-FAST、OpenVLA、OpenVLA-OFT)在Maniskill多个任务上的平均性能提升高达22.6%，并能泛化到不同空间配置、相机视角和机器人体现。

Conclusion: FailSafe系统有效解决了机器人操作中的失败恢复问题，显著提升了VLA模型的鲁棒性，并具有良好的泛化能力。
...
TL;DR: 提出一种在线学习测量可靠性的统计框架，通过多视角几何一致性作为自监督，动态评估视觉惯性里程计中传感器测量的可靠性，在EuRoC数据集上相比固定不确定性参数的方法平均减少24%平移误差和42%旋转误差。
...
Result: 在EuRoC数据集上测试，相比固定不确定性参数的基线方法，平均减少24%平移误差和42%旋转误差，同时保持实时运行。

Conclusion: 该框架能够实时运行，在准确性和鲁棒性方面均有提升，源代码将公开以促进可复现性和进一步研究。
...
<details>
  <summary>Details</summary>
Motivation: 解决多步操作任务中的挑战：模仿学习缺乏组合泛化能力，而任务运动规划存在规划延迟问题，无法实时处理故障。

Method: 从无标签、无分割的演示数据中联合学习谓词、操作符和技能；运行时使用符号规划器组合和重排序技能；在运动和符号层面进行实时故障恢复。

Result: 在RoboCasa模拟中，12个单步任务成功率85%，可组合成最多6次技能重组的复杂任务；真实机器人上，从5分钟无标签演示数据中学习，能通过目标指定执行多种任务。

Conclusion: SymSkill框架成功结合了模仿学习的反应性和任务运动规划的组合性，实现了实时故障恢复和组合泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有执行器感知控制方法依赖于执行器输入与力矩之间的线性关系，无法捕捉可变倾斜角度引起的非线性效应，这限制了在激进飞行或扰动恢复中的有效性和可靠性。

Method: 利用多旋翼刚体动力学与非线性执行器动力学之间的级联结构，设计几何反步控制器，并建立整个系统的指数稳定性。

Result: 在三个实验场景中，所提控制器始终获得更好的跟踪性能。在最快的平移轨迹跟踪和恢复实验中，基线方法发散并坠毁，而所提控制器保持稳定并成功完成任务。

Conclusion: 所提出的控制器能够有效处理执行器非线性动力学，对参数不确定性具有鲁棒性，在激进飞行和扰动恢复场景中表现出优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决人形机器人全身控制在仿真训练中存在的仿真到真实差距问题，这种差距主要源于单个仿真器的归纳偏差和假设限制。

Method: 开发PolySim平台，在单个训练运行中同时启动来自不同引擎的并行环境，实现动态层面的域随机化。

Result: 在仿真间评估中显著降低运动跟踪误差，在MuJoCo上比IsaacSim基线提高52.8%的执行成功率，并实现零样本部署到真实Unitree G1机器人。

Conclusion: 多仿真器联合训练能有效减少仿真器归纳偏差，实现更好的仿真到真实世界迁移，无需额外微调。
...
<details>
  <summary>Details</summary>
Motivation: 现有VLA模型在机器人操作中表现良好，但其表示对机器人信号（如控制动作和本体感知状态）不够敏感，存在优化不足的问题。

Method: 引入RS-CL损失函数，使用机器人本体感知状态的相对距离作为软监督，与原始动作预测目标互补，增强控制相关表示学习。

Result: 在RoboCasa-Kitchen的拾放任务中，将成功率从30.8%提升至41.5%；在真实机器人操作任务中，成功率从45.0%提升至58.3%。

Conclusion: RS-CL能有效提升VLA模型的机器人操作性能，方法轻量且与标准VLA训练流程完全兼容。
...
<details>
  <summary>Details</summary>
Motivation: 传统轴向磁化设计的磁性连续体机器人主要限于弯曲运动，限制了变形能力。为了扩展变形能力并实现精确的靶向药物递送。

Method: 在导管壁内径向嵌入永磁体，采用物理基础建模和有限元分析建立驱动原理，开发基于扭转剪切的双层阻塞机制（外槽和内板）。

Result: 实验验证了在实用磁场下的解耦模式控制，在体模干预实验中展示了从管腔跟随、弯曲接近目标到扭转激活释放的端到端操作。

Conclusion: 该紧凑、无缆平台结合了多功能变形与精确载荷递送，显示出在下一代靶向治疗中的强大潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现代智能工厂可能包含数百台机器，但现有的SFE求解器只能扩展到几十台机器，存在可扩展性不足的问题。

Method: 提出了TS-ACES（基于交通系统的任意时间循环嵌入求解器），这是一种完整的求解方法。

Result: TS-ACES能够扩展到基于真实工业场景的SFE实例，处理超过一百台机器的情况。

Conclusion: TS-ACES填补了SFE问题中可扩展性解决方案的空白，为大规模智能工厂的制造流程嵌入提供了有效工具。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在自动驾驶中应用广泛，但高推理延迟阻碍实时部署。早期退出方法可降低延迟，但其任务依赖性限制了在多样化场景中的泛化能力。

Method: 提出Nav-EE框架，利用导航系统预测即将到来的上下文（如交叉口、交通灯），离线预计算任务特定退出层，在线基于导航先验动态应用。

Result: 在CODA、Waymo和BOSCH数据集上，Nav-EE达到与完整推理相当的精度，同时降低延迟高达63.9%。实车集成显示推理延迟从600ms降至300ms。
...
<details>
  <summary>Details</summary>
Motivation: 物体目标导航是移动机器人在日常环境中部署的关键能力，但现有方法缺乏对哪些组件真正驱动性能的统一分析。本文旨在通过大规模实证研究确定影响ObjectNav性能的关键因素。

Method: 将模块化RL-based ObjectNav系统分解为感知、策略和测试时增强三个关键组件，通过大量受控实验隔离每个组件的贡献，并基于发现提出设计指南。

Result: 感知质量和测试时策略是性能的决定性驱动因素，而策略改进仅带来边际增益。提出的增强模块化系统在SPL上超越现有最佳方法6.6%，成功率提升2.7%。人类专家在相同条件下达到98%的平均成功率。

Conclusion: 研究不仅设定了新的最佳性能标准，还为未来ObjectNav开发和评估提供了原则性指导，揭示了RL智能体与人类水平导航之间的差距。
...
<details>
  <summary>Details</summary>
Motivation: 解决人形机器人足球中在激烈踢球动作时保持系统稳定性并实现精确球轨迹控制的挑战。现有方法如模型预测控制(MPC)在腿摆动过程中过度简化，限制了脚的环境交互能力，特别是在踢球任务中。

Method: 将无人机应用中成功的时空轨迹规划方法创新性地适配到双足机器人系统，自主生成满足目标踢球位置、速度和加速度约束的脚部轨迹，同时优化摆动阶段持续时间。

Result: 优化的轨迹紧密模仿人类踢球行为，具有后摆动作。轨迹规划时间低于1毫秒，当足球目标在-90度到90度范围内时，任务完成准确率接近100%。
...
<details>
  <summary>Details</summary>
Motivation: 现有温室仿真方法依赖简单或合成资产，限制了仿真到现实的迁移能力。辐射场方法虽能实现逼真重建，但仅限于单个植物或实验室环境。

Method: 使用高斯溅射辐射场方法从RGB图像重建温室资产，集成到支持相机和LiDAR渲染的ROS仿真中。

Result: 提供了包含82株黄瓜植物的数据集，支持多种行配置，展示了在机器人定位等任务中的实用性。
...
<details>
  <summary>Details</summary>
Motivation: 单个飞行员管理多无人机系统需要不同级别的自主性，从直接控制到群体协调再到完全自主的集群行为。需要支持多种共享自主模式的框架来减轻飞行员工作负担。

Method: TACOS框架集成三个关键能力：一对多自然语言交互界面、智能协调器（将用户意图转化为结构化任务计划）、自主代理（与现实世界交互执行计划），让LLM与可执行API库交互。

Result: 在真实多无人机系统中进行了演示，并通过消融研究评估了各模块的贡献。

Conclusion: TACOS提供了一个统一的框架，通过大语言模型实现多无人机系统的高级自然语言控制，弥合语义推理与实时多机器人协调之间的差距。
...
<details>
  <summary>Details</summary>
Motivation: 驾驶模拟器在研发中应用日益广泛，但常因运动缩放和视觉不匹配导致晕动症，需要开发能同时优化保真度和舒适度的运动提示算法

Method: 使用模型预测控制开发运动提示算法，在成本函数中同时惩罚感官冲突和特定力误差；进行人机闭环实验比较四种运动设置：两种MPC变体、自适应冲刷和无运动

Result: 实验结果显示无运动条件晕动症最低但保真度评分最差；折衷方案相比自适应冲刷和纯特定力跟踪算法，晕动症降低50%以上（平均MISC水平从3降至1.5），且保真度无明显下降

Conclusion: 提出的考虑模拟器动力学和晕动症时间演化的运动提示算法开发方法，在实现晕动症控制和特定力重现的最优平衡方面取得重要进展，支持模拟器的更广泛应用
...
<details>
  <summary>Details</summary>
Motivation: 解决传统单目稠密SLAM存在的高延迟、大GPU内存消耗和依赖相机标定的问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有的语言驱动抓取方法难以处理包含隐含意图的模糊指令，需要开发能够理解语言指令中未明确表述但必要的操作和目标信息的能力。

Method: 集成微调的大语言模型，利用其常识理解和环境感知能力推断隐含意图；设计基于2D部件分割引导的点云定位模块，实现部件级点云定位。

Result: 实验表明LangGrasp能准确解析模糊指令中的隐含意图，识别关键操作和目标信息，动态选择最优抓取姿态，实现高精度抓取。

Conclusion: 该框架显著提升了机器人在非结构化环境中的适应性和任务执行效率，支持从物体级到部件级的精细操作。
...
<details>
  <summary>Details</summary>
Motivation: 在类人机器人足球比赛中，机器人无法自主站立会被暂时移除比赛。站立动作是机器人足球不可或缺的部分，需要确保高成功率。

Method: 通过执行特殊动作释放卡住的肢体（如手臂），或通过其他关节补偿大误差来解决关节位置执行误差问题。该方法自2019年开始开发并持续改进。

Result: 显著提高了站立动作的整体成功率。多个标准平台联盟团队采用该方法后也获得了相似的成功率，这在多个锦标赛视频分析中得到验证。

Conclusion: 解决关节位置执行误差是提高机器人站立成功率的关键。经过6年评估和改进的方法被证明有效，并被多个团队成功采用。
...
<details>
  <summary>Details</summary>
Motivation: 开发一种多功能的光学传感软体机器人平台，扩展软体机器人的光谱感知能力，实现物体的可解释性分类识别。

Method: 进行材料分析以寻找适合光谱传感的软基底材料，评估预接触和手持两种模式下的性能，使用线性判别分析识别关键光谱特征。

Result: 实验证明该系统能够对不同类别和尺寸的物体（金属、木材、塑料、有机物、纸张、泡沫）进行可解释的统计分离，近红外波长的敏感性对区分视觉相似物体至关重要。

Conclusion: SCANS系统推进了光学作为软体机器人多功能传感模式的潜力，所有零件清单、组装指南和处理代码均已公开。
...
<details>
  <summary>Details</summary>
Motivation: 在循环经济背景下，制造商往往不共享相关数据，导致产品生命周期结束阶段的再制造和回收过程支持不足。需要解决电动汽车电池拆解过程中的挑战，减少生态影响。

Method: 采用数字孪生技术，提出双向产品-过程-资源资产网络(Bi-PAN)表示法，扩展传统PAN范式以同时建模制造和再制造/回收阶段的关系。

Result: 通过电动汽车电池拆解用例证明，产品数字孪生能够灵活高效地解决不同类型电池的拆解挑战。

Conclusion: 数字孪生技术和Bi-PAN表示法为优化产品拆解过程提供了有效解决方案，有助于提高可持续性和减少生态影响。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法由于固定数据集而难以泛化，而基于LLM和VLM的方法虽然语义丰富但缺乏鲁棒和灵活的精炼能力，导致布局效果不理想。

Method: 开发DisCo-Layout框架，包含语义精炼工具(SRT)修正抽象对象关系，物理精炼工具(PRT)通过网格匹配算法解决具体空间问题，并采用多智能体框架（规划器、设计师、评估器）进行协作精炼。

Result: 实验证明DisCo-Layout实现了最先进的性能，能够生成真实、连贯且可泛化的3D室内布局。

Conclusion: DisCo-Layout通过解耦和协调物理与语义精炼，有效解决了3D室内布局合成中的泛化和精炼问题，代码将公开可用。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉策略在航空导航中容易对单一赛道过拟合，当赛道几何形状变化时性能会下降，需要开发能够生成多样化赛道并提升策略泛化能力的方法。

Method: 基于高斯泼溅技术构建FalconGym 2.0仿真框架，提供编辑API快速生成多样化静态和动态赛道；提出性能引导精炼算法，在具有挑战性的赛道上集中训练视觉策略并迭代提升性能。

Result: 在固定翼无人机和四旋翼无人机两个案例研究中，使用PGR训练的单一视觉策略在泛化性和鲁棒性上优于现有基线：无需针对每个赛道重新训练即可100%成功泛化到三个未见赛道，在门姿态扰动下保持更高的成功率；零样本仿真到真实世界迁移实验中，在四旋翼硬件上达到98.6%的成功率。

Conclusion: FalconGym 2.0结合PGR算法能够有效训练出具有强大泛化能力和鲁棒性的视觉策略，并能成功实现仿真到真实世界的零样本迁移，为航空导航提供了可靠的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决人形机器人运动跟踪中的人体-机器人体现差距问题，现有重定向方法引入的伪影（如脚部滑动、自穿透等）会降低策略性能，需要大量奖励工程来弥补。

Method: 提出通用运动重定向方法(GMR)，并与现有开源重定向器PHC、ProtoMotions以及闭源数据集Unitree进行比较，使用BeyondMimic进行策略训练以隔离重定向效果。

Result: GMR在跟踪性能和运动保真度方面一致优于现有开源方法，达到接近闭源基线的感知保真度和策略成功率，特别是在动态或长序列运动中表现更鲁棒。

Conclusion: 重定向质量对策略性能有显著影响，GMR方法能有效减少重定向伪影，在减少奖励工程的情况下实现更好的运动跟踪效果。
...
<details>
  <summary>Details</summary>
Motivation: 解决模仿学习策略在面对视角变化时的鲁棒性问题，特别是当策略依赖静态背景线索推断相机姿态时，在真实场景中容易失效的问题。

Method: 使用Plucker嵌入表示像素射线，将相机外参作为策略的显式条件输入，在RoboSuite和ManiSkill中构建了6个操作任务来评估视角变化下的策略鲁棒性。

Result: 实验表明，未使用外参条件的策略在固定场景中会利用静态背景线索推断相机姿态，但在工作空间几何或相机位置变化时性能崩溃；而使用外参条件的方法恢复了性能，实现了仅使用RGB输入的鲁棒控制。
...
<details>
  <summary>Details</summary>
Motivation: 模仿学习在大规模真实世界数据上表现良好，但预训练策略在缺乏领域特定数据时性能较差。人工收集演示数据成本高且质量参差不齐，现有的人机协作系统需要全程人工监督。

Method: 开发ARMADA系统，包含FLOAT自主在线故障检测方法，实现并行策略部署，仅在检测到故障时请求人工干预，从而高效获取领域特定数据。

Result: FLOAT故障检测准确率平均达到近95%，比现有最佳方法提升20%以上。ARMADA在多次策略部署和后训练中，成功率提高4倍以上，人工干预率降低2倍以上。

Conclusion: ARMADA系统通过FLOAT故障检测实现了更高效的领域数据获取，显著提升了部署可扩展性和对新场景的适应速度，减少了人类监督需求。
