...
Motivation: 解决无人机在GNSS拒止环境中长距离飞行的挑战，包括航迹推算漂移、缺乏闭环校正以及嵌入式平台计算能力有限的问题。

Method: 集成感知、建图、规划和控制，采用轻量级漂移校正方法，通过梯度模板匹配将激光雷达局部高度图与先验地理数据高度图匹配，并在聚类粒子滤波中融合证据与航迹推算数据。

Result: 在竞赛中执行了公里级飞行，覆盖城市、森林和开阔地带，相对于原始航迹推算显著减少了漂移，并在仅使用CPU的硬件上实时运行。
...
<details>
  <summary>Details</summary>
Motivation: 传统运动规划方法在狭窄内陆水道等挑战性空间中往往计算密集或过于保守，需要一种既能保证安全又不过度保守的实时规划方案。

Method: 采用时变膨胀椭圆障碍物表示，根据船舶与障碍物的相对位置和姿态调整膨胀半径，结合MPC提供近似运动规划，高阶CBFs使用变化的膨胀半径确保船舶安全。

Result: 仿真和真实世界实验表明，该策略使全驱动自主机器人船舶能够实时导航狭窄空间并解决潜在死锁，同时确保安全性。
...
Method: 基于连续时间运动学的因子图优化框架，使用简单的机器人模型配合高频率传感，通过高斯过程处理噪声。

Result: 能够估计机器人位姿、速度和应变的均值和协方差，支持时间和空间的连续插值，计算复杂度与时间呈线性关系。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有视觉语言模型在机器人导航任务中难以迁移的问题，因为预训练目标与机器人动作空间存在差异，需要开发能够适应多样化人类指令并在非结构化环境中安全操作的导航系统。

Method: 使用互联网预训练的图像扩散模型进行路径规划，生成图像空间中的路径掩码作为视觉计划，然后通过轻量级行为克隆策略将这些视觉计划转换为可执行轨迹。训练时使用自监督跟踪模型生成的路径掩码和VLM增强的标注，避免手动像素级标注。

Result: 在真实世界评估中，VENTURA在物体到达、障碍物避让和地形偏好任务上优于最先进的基线模型，成功率提高33%，碰撞率降低54%，并且在未见过的任务组合中展现出涌现的组合能力。

Conclusion: VENTURA通过视觉路径规划和轻量级策略的组合，有效解决了视觉语言模型在机器人导航中的迁移问题，展示了在开放世界环境中适应多样化指令的强大能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有VLA模型缺乏自省机制来预测失败并请求人工监督，需要开发能够主动识别不确定性并寻求帮助的框架。

Method: 使用π₀-FAST作为基础模型，提取token级熵、对数概率、偶然性和认知不确定性，训练transformer分类器将这些序列映射到帮助触发信号。

Result: 强监督标签能捕捉细粒度不确定性动态实现可靠帮助检测，弱监督标签在训练和评估对齐时仍具竞争力，为密集标注不可行时提供可扩展路径。

Conclusion: 建模token级不确定性信号的时间演化比静态序列级评分具有更强预测能力，为VLA中的主动学习和实时错误缓解开辟了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 现有CBF方法在非完整机器人密集动态环境中过于保守，基于碰撞锥或速度障碍的约束仅考虑相对速度角度，导致QP经常不可行。

Method: 设计动态抛物线控制屏障函数，抛物线的顶点和曲率根据障碍物距离和相对速度大小动态调整，创建限制更少的安全约束。

Result: 在密集环境中成功导航多达100个动态障碍物，导航成功率和QP可行性显著优于基线方法。

Conclusion: DPCBF方法有效解决了传统CBF在密集动态环境中的保守性问题，提高了导航性能和QP可行性。
...
Result: 扩散策略学习约束流形的粗略近似，数据集大小和质量下降会负面影响学习效果，而流形曲率与约束满足和任务成功的相关性不确定。硬件评估验证了结果的现实适用性。

Conclusion: 扩散策略能够学习运动学约束的近似表示，但学习效果受数据集特征影响，需要进一步研究约束学习机制。
...
<details>
  <summary>Details</summary>
Motivation: 解决基于视觉的机器人学习依赖密集图像或点云输入导致计算量大、包含无关背景特征的问题，以及现有关键点方法依赖手动启发式或任务耦合选择限制可扩展性和语义理解的问题。

Method: 三阶段流程：affordance过滤、类别级关键点构建、基于transformer的策略学习（包含嵌入门控机制来推理最相关关键点），生成紧凑的38维状态策略。

Result: 在15分钟内完成训练，在未见过的物体、新类别、背景和干扰物上达到82%的成功率，显著提高数据效率。

Conclusion: AFFORD2ACT框架通过语义关键点提取和轻量级策略学习，实现了高效、可扩展的机器人操作，在多样化真实世界操作任务中表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 机器人自动化加速科学发现，但粉末的精确操作（特别是运输任务）仍具挑战性，需要高精度和稳定性。

Method: 集成可微分物理模拟准确建模颗粒动力学，使用低维技能空间参数化降低优化复杂度，采用课程学习策略在长时域内逐步提升任务能力。
...
<details>
  <summary>Details</summary>
Motivation: 保乳手术中肿瘤边界定位困难，因为肿瘤通常高度移动、不可触及且边界不规则，需要更精确的定位方法。

Method: 将小型触觉机器人改装为协作控制的手术工具，结合超声和电磁导航识别肿瘤边界，当手术工具碰撞肿瘤边界时施加禁止区域虚拟夹具。

Result: 虚拟夹具引导改善了切除边缘，用户在使用触觉反馈时感觉任务精神需求、挫败感和努力程度降低，但也发现了一些对手术流程的意外影响。

Conclusion: 虚拟夹具可在模拟保乳手术中帮助定位肿瘤边界，未来将进行更广泛的用户研究以进一步验证结果并优化引导系统。
...
Method: 采用分块处理视频序列的方法，利用现代视觉语言模型构建持久知识图谱，通过可查询的图结构实现可解释的空间推理。

Result: 在真实世界差分驱动机器人上部署，成功率达到77.27%，答案准确率为76.92%，性能与Gemini 2.5 Pro相当，同时提供可解释的推理支持。

Conclusion: VL-KnG系统在保持计算效率的同时，为定位、导航和规划等任务提供了可扩展且可解释的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 针对具有非驱动关节、链接安装推进器和单陀螺仪的欠驱动、最小感知机器人平台，需要解决姿态估计问题，特别是在存在非零均值、非高斯噪声的情况下。

Method: 使用无迹卡尔曼滤波增强高斯过程残差学习来补偿非零均值、非高斯噪声，通过多步态数据集进行训练和验证。

Result: 实验表明，在多步态数据集上训练的滤波器与在更大前进步态数据集上训练的滤波器在前进步态测试轨迹上表现相当，揭示了步态输入空间的重叠性。

Conclusion: 可以利用步态输入空间的重叠性来减少训练数据需求，同时增强滤波器在多种步态间的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在复杂室内环境中存在局限性：传统采样方法难以控制分辨率和扩展性，模仿学习方法需要大量演示数据，而ANTFields方法受到光谱偏差和灾难性遗忘的限制。

Method: 采用分层结构：高层使用稀疏图表示环境全局连通性，低层使用基于神经场的规划器通过求解Eikonal PDE来导航局部障碍物，这种物理信息策略避免了常见问题。

Result: 在大规模环境中验证了该框架，相比先前方法展现出更强的适应性和精度，能够实现平滑精确的成本地形表示。

Conclusion: 该方法在复杂室内导航中表现出色，具有在线探索、建图和实际导航的潜力，解决了现有神经场方法的关键限制。
...
Result: 实验结果表明该方法实现了快速准确的3D多平面分割，在模拟环境和物理腿式机器人平台上验证了方法的有效性

Conclusion: 该方法能够实时检测平面并用于运动任务，在考虑3D平面结构时表现出鲁棒的运动性能
...
<details>
  <summary>Details</summary>
Motivation: 传统双手机器人系统复杂且仅利用部分工作空间进行灵巧操作，需要更紧凑、轻量化的解决方案来扩大灵巧工作空间。

Method: 制定运动灵巧度指标指导设计，采用两个低自由度手臂（3+自由度）耦合到运动链中，支持可穿戴运动数据收集和标准机器人手臂部署两种模式。

Result: 开发出轻量可穿戴系统，能够进行穿戴式演示并训练模仿学习策略，实现鲁棒的真实世界双手机器人操作。

Conclusion: MiniBEE系统通过紧凑设计扩大了灵巧工作空间，为双手机器人操作提供了更高效、轻量化的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决机器人从野外人类演示中学习复杂双手操作的问题，通过便携式系统收集高质量数据，并建立视觉注意与操作之间的关键联系。

Method: 结合便携VR遥操作套件和传感器化控制器，通过精确姿态对齐桥接人机运动学，采用沉浸式3D模型渲染、自包含可穿戴计算机和高效校准方法。

Result: 在六个挑战性双手任务中，仅使用ActiveUMI数据训练的策略在分布内任务上平均成功率70%，在新物体和新环境中保持56%的成功率。

Conclusion: 便携数据收集系统结合学习到的主动感知，为创建通用且高能力的真实世界机器人策略提供了有效且可扩展的途径。
...
<details>
  <summary>Details</summary>
Motivation: 现有机器人操作数据集主要提供地面真实轨迹，无法处理执行过程中不可避免的故障，而少数涉及故障检测的数据集通常只提供难以在VLA模型中直接使用的文本解释，因此需要开发能够自动生成故障-恢复数据的系统。

Method: 提出FailSafe系统，可在任何模拟器中无缝应用于任何操作任务，实现故障-动作数据的可扩展创建，并通过微调LLaVa-OneVision-7B构建FailSafe-VLM模型。

Result: FailSafe-VLM成功帮助机器人手臂检测和恢复潜在故障，在Maniskill多个任务中将三个最先进VLA模型(pi0-FAST、OpenVLA、OpenVLA-OFT)的平均性能提升高达22.6%，并能泛化到不同的空间配置、相机视角和机器人实体。

Conclusion: FailSafe系统有效解决了机器人故障恢复的关键挑战，显著提升了VLA模型的故障处理能力，并具有良好的泛化性能，计划向社区发布FailSafe代码。
...
TL;DR: 提出了一种在线学习测量可靠性的统计框架，通过多视图几何一致性作为自监督，动态评估视觉惯性里程计中传感器测量的可靠性，在EuRoC数据集上相比固定不确定性参数的方法平均减少24%平移误差和42%旋转误差。


<details>
  <summary>Details</summary>
Motivation: 传统方法假设所有测量具有静态均匀的不确定性，无法捕捉真实世界数据中动态变化的误差特性，限制了鲁棒视觉惯性里程计的性能。

Method: 利用多视图几何一致性作为自监督，从传感器数据和优化结果中在线学习测量可靠性评估，推断地标不确定性并在优化过程中自适应加权视觉测量。

Result: 在EuRoC数据集上评估，相比固定不确定性参数的基线方法，平均减少24%平移误差和42%旋转误差，框架能够实时运行并展现更高的精度和鲁棒性。

Conclusion: 提出的统计框架能够有效在线学习测量可靠性，显著提升视觉惯性里程计的跟踪精度和鲁棒性，源代码将公开以促进可复现性和进一步研究。
...
<details>
  <summary>Details</summary>
Motivation: 解决动态环境中多步操作的挑战：模仿学习缺乏组合泛化能力，而任务运动规划存在规划延迟问题，无法实时恢复故障。

Method: 从无标签、无分割的演示中联合学习谓词、操作符和技能；执行时使用符号规划器组合和重新排序技能，在运动和符号层面进行实时恢复。

Result: 在RoboCasa模拟中，执行12个单步任务成功率85%；无需额外数据即可组合成最多6次技能重组的复杂计划；在真实机器人上，仅用5分钟无标签数据就能完成多任务。

Conclusion: SymSkill框架成功结合了IL和TAMP的优势，实现了组合泛化、实时故障恢复和安全的连续执行。
...
<details>
  <summary>Details</summary>
Motivation: 现有执行器感知控制方法依赖于执行器输入与力/力矩之间的线性关系，无法捕捉可变倾斜角引起的非线性效应，这对于激进的飞行机动或突然干扰恢复至关重要。

Method: 利用多旋翼刚体动力学与其非线性执行器动力学之间的级联结构，设计几何反步控制器，并建立整个系统的指数稳定性。

Result: 在快速平移跟踪、快速旋转跟踪和突然干扰恢复三个实验场景中，所提方法始终获得更好的跟踪性能，而基线方法在最快速平移轨迹跟踪和恢复实验中发散并坠毁。

Conclusion: 所提出的控制器对执行器模型中的参数不确定性具有鲁棒性，能够保持稳定性并成功完成任务，证明了其有效性。
...
<details>
  <summary>Details</summary>
Motivation: 解决人形机器人全身控制策略在仿真训练中面临的仿真器归纳偏差问题，这种偏差导致跨仿真器和仿真到真实世界的显著差异。

Method: 开发PolySim平台，在单个训练运行中同时启动来自不同引擎的并行环境，实现动态层面的领域随机化。

Result: 在仿真间评估中显著降低运动跟踪误差，在MuJoCo上比IsaacSim基线提高执行成功率52.8%，并能零样本部署到真实Unitree G1机器人。

Conclusion: 多仿真器联合训练能有效减少仿真器归纳偏差，实现从仿真到真实世界的有效迁移，无需额外微调。
...
Method: 引入RS-CL作为表示正则化方法，使用机器人状态间的相对距离作为软监督，与原始动作预测目标互补

Result: 在RoboCasa-Kitchen的拾放任务中，成功率从30.8%提升至41.5%；在真实机器人操作任务中，成功率从45.0%提升至58.3%

Conclusion: RS-CL能有效增强VLA模型的控制相关表示学习，显著提升机器人操作性能，且方法轻量且与标准VLA训练流程兼容
...
<details>
  <summary>Details</summary>
Motivation: 传统轴向磁化设计的磁性连续体机器人主要局限于纯弯曲运动，限制了其变形能力。为了扩展变形能力，需要开发能够独立控制弯曲和扭转的新型设计。

Method: 采用径向嵌入永磁体的简单装配结构，结合物理基础公式和有限元分析建立驱动原理，通过实验验证解耦模式控制，并开发了利用扭转剪切实现按需药物释放的双层阻塞机制。

Result: 实验验证了在实际场强下的解耦模式控制，并在体模干预实验中展示了完整的端到端操作：通过弯曲进行目标接近，随后在目标位置通过扭转激活释放机制。
...
Method: 开发了基于交通系统的随时循环嵌入求解器TS-ACES，能够处理机器分配和部件运输路径规划

Result: TS-ACES具有完备性，能够扩展到基于真实工业场景的超过100台机器的SFE实例

Conclusion: TS-ACES填补了现有SFE求解器无法处理大规模智能工厂的空白，是第一个高度可扩展的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在自动驾驶中应用日益广泛，但高推理延迟阻碍了实时部署。早期退出方法虽然能减少延迟，但其任务依赖性限制了在不同场景下的泛化能力。

Method: 提出Nav-EE框架，利用自动驾驶导航系统可以预测即将到来的上下文（如交叉口、交通灯）的特点，离线预计算任务特定的退出层，在线根据导航先验动态应用这些退出层。

Result: 在CODA、Waymo和BOSCH数据集上的实验显示，Nav-EE在保持与完整推理相当的准确性的同时，将延迟降低了高达63.9%。真实车辆集成测试中，推理延迟从600ms降至300ms。

Conclusion: 将导航预见性与早期退出相结合，为在自动驾驶系统中高效部署大型模型提供了一条可行路径。
...
<details>
  <summary>Details</summary>
Motivation: 物体目标导航是移动机器人在日常环境中部署的关键能力，但现有方法缺乏统一分析来确定哪些组件真正驱动性能。作者旨在通过分解系统组件来识别性能的关键因素。

Method: 将模块化RL-based物体目标导航系统分解为三个关键组件：感知、策略和测试时增强，通过大量受控实验隔离每个组件的贡献。

Result: 感知质量和测试时策略是性能的决定性驱动因素，而当前方法的策略改进仅带来边际增益。提出的增强模块化系统在SPL上超过现有最佳方法6.6%，成功率提高2.7%。

Conclusion: 研究不仅设定了新的最佳性能标准，还为未来物体目标导航的发展和评估提供了原则性指导，同时揭示了RL代理与人类专家导航（98%成功率）之间的差距。
...
<details>
  <summary>Details</summary>
Motivation: 当前人形机器人足球面临的主要挑战是在进行激烈踢球动作时保持系统稳定性，同时实现精确的球轨迹控制。现有的位置控制方法和强化学习方法存在显著局限性，而模型预测控制方法在腿部摆动过程中过度简化，限制了脚部环境交互能力。

Method: 创新性地将无人机应用中成功的时空轨迹规划方法适配到双足机器人系统，自主生成满足目标踢球位置、速度和加速度约束的足部轨迹，同时优化摆动阶段持续时间。

Result: 实验结果表明，优化后的轨迹能够紧密模拟人类踢球行为，具有后摆动作特征。轨迹规划时间低于1毫秒，当足球目标在-90度到90度范围内时，任务完成准确率接近100%。

Conclusion: 该方法在人形机器人足球踢球任务中表现出高效性和可靠性，为双足机器人的复杂交互任务提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有温室环境模拟方法依赖简化或合成资产，限制了仿真到现实的迁移能力，需要更逼真的模拟环境来开发和评估农业机器人系统

Method: 利用辐射场方法（如高斯溅射）从RGB图像重建逼真温室资产，集成到ROS仿真中，支持相机和LiDAR渲染
...
<details>
  <summary>Details</summary>
Motivation: 单飞行员管理多无人机系统需要不同自主级别，从直接控制到群体协调再到完全自主。需要支持多种共享自主模式的框架，利用语言模型减少飞行员工作负荷。

Method: TACOS框架集成三大能力：一对多自然语言接口、智能协调器将用户意图转为结构化任务计划、自主代理执行计划与现实世界交互。LLM与可执行API库交互，连接语义推理与实时多机器人协调。

Result: 在真实多无人机系统上验证，并通过消融研究评估各模块贡献。

Conclusion: TACOS通过统一架构实现了基于自然语言的多无人机系统高层控制，有效结合了语言模型的推理能力与实时机器人协调。
...
Method: 集成三个扭矩控制执行器、定制1kHz控制板和防护电源单元，开发基于RNEA的计算加速度控制器，带有平滑Stribeck摩擦补偿。

Result: 准静态推拉测试显示线性力-位移特性，水平刚度300-700N/m，相对误差≤1.5%；动态位移释放试验确认质量-弹簧-阻尼器响应。
...
<details>
  <summary>Details</summary>
Motivation: 驾驶模拟器在研发中应用日益广泛，但由于运动缩放和视觉不匹配常导致晕动症，需要开发能同时保证运动保真度和减少不适感的算法。

Method: 使用模型预测控制(MPC)开发运动提示算法，在成本函数中同时惩罚感官冲突和特定力误差，通过人机闭环实验比较四种运动设置。

Result: 实验结果显示，折衷方案相比自适应洗出和纯特定力跟踪算法，晕动症减少超过50%(平均MISC水平从3降至1.5)，且保真度评分无显著下降。

Conclusion: 提出的考虑模拟器动力学和晕动症时间演化的MCA开发方法，在实现晕动症控制和特定力重建的最优平衡方面取得重要进展，支持模拟器的更广泛应用。
...
Conclusion: EC3R-SLAM展示了在现实世界机器人应用中实现高效、鲁棒SLAM的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有的语言驱动抓取方法难以处理包含隐含意图的模糊指令，需要开发能够理解语言指令中未明确表达但完成任务必需的关键操作和目标信息的系统。

Method: 集成微调的大语言模型利用其常识理解和环境感知能力推断隐含意图，设计基于2D部件分割引导的点云定位模块，实现场景中部分点云定位。

Result: 实验结果显示LangGrasp框架能准确解析模糊指令中的隐含意图，识别完成任务必需的关键操作和目标信息，并动态选择最优抓取姿态，实现从物体级到部件级的高精度抓取。

Conclusion: LangGrasp显著提升了机器人在非结构化环境中的适应性和任务执行效率，实现了从粗粒度物体级到细粒度部件级的抓取操作扩展。
...
<details>
  <summary>Details</summary>
Motivation: 站起动作是人形机器人足球比赛中不可或缺的部分，无法自主站起的机器人会被暂时移出比赛。

Method: 通过执行特殊动作释放卡住的肢体（如手臂），或使用其他关节补偿大误差，来应对关节位置执行误差问题。

Result: 显著提高了站起动作的整体成功率，该方法已被标准平台联盟的多个团队采用并取得相似的成功率。

Conclusion: 解决关节位置执行误差是提高机器人站起成功率的关键，该方法经过6年验证具有可靠性和有效性。
...
<details>
  <summary>Details</summary>
Motivation: 开发一种多功能光学感知系统，推进光学作为软体机器人多功能感知模式的应用潜力。

Method: 进行材料分析以探索光谱感知的最佳软基底，评估预接触和手持性能，使用线性判别分析确定关键波长。

Result: 实验证明能够对不同类别和尺寸的物体（金属、木材、塑料、有机材料、纸张、泡沫）进行可解释的统计分离，近红外波长对区分视觉相似物体至关重要。

Conclusion: SCANS系统推进了光学作为软体机器人多功能感知模式的潜力，所有部件清单、组装指南和处理代码均已开源。
...
<details>
  <summary>Details</summary>
Motivation: 在循环经济背景下，制造商往往不共享相关数据，导致产品生命周期末端的再制造和回收过程支持不足。

Method: 采用数字孪生技术，基于产品-过程-资源资产网络（PAN）范式，提出双向PAN（Bi-PAN）表示方法，扩展至再制造/回收阶段。

Result: 通过电动汽车电池拆解用例证明，该方法能灵活高效地解决不同类型电池的拆解挑战。

Conclusion: 数字孪生技术结合Bi-PAN表示能够优化拆解过程，减少生态影响并增强可持续性。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉策略在无人机空中导航中容易过拟合单一赛道，当赛道几何形状变化时性能下降。需要开发能够生成多样化赛道的仿真环境和训练方法，提升视觉策略的泛化能力。

Method: 1) 基于高斯溅射(GSplat)构建FalconGym 2.0逼真仿真框架，提供Edit API在毫秒级生成静态和动态多样化赛道；2) 提出性能引导精炼(PGR)算法，在训练过程中聚焦于挑战性赛道迭代提升策略性能。

Result: 在固定翼无人机和四旋翼两个案例中，单一视觉策略在PGR训练下：1) 在三个未见赛道上实现100%成功率，无需逐赛道重训练；2) 在门姿态扰动下保持更高成功率；3) 零样本sim-to-real迁移到真实四旋翼硬件，在30次试验中达到98.6%成功率(69/70门)。

Conclusion: FalconGym 2.0结合PGR算法能够有效训练出具有强泛化能力和鲁棒性的视觉策略，成功实现从仿真到真实环境的零样本迁移，为无人机视觉导航提供了可靠解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决人形机器人运动跟踪中存在的体现差距问题，现有重定向方法会产生足部滑动、自穿透和物理不可行运动等伪影，这些伪影会降低策略的鲁棒性。

Method: 提出通用运动重定向(GMR)方法，并与PHC、ProtoMotions等开源重定向器以及Unitree的高质量闭源数据集进行比较，使用BeyondMimic进行策略训练以隔离重定向效果。

Result: 实验表明，虽然大多数运动可以被跟踪，但重定向数据中的伪影显著降低了策略鲁棒性，特别是在动态或长序列中。GMR在跟踪性能和运动保真度方面一致优于现有开源方法。

Conclusion: GMR方法在感知保真度和策略成功率方面接近闭源基准，为解决运动重定向质量问题提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有模仿学习策略在固定场景中可能通过静态背景线索推断相机姿态，但当工作空间几何或相机位置变化时这种捷径会失效，需要开发对相机外参显式建模的鲁棒方法。

Method: 使用Plucker嵌入表示像素光线，将相机外参作为条件输入到策略中，在RoboSuite和ManiSkill中构建了6个操作任务，包含固定和随机化场景变体以分离背景线索和相机姿态的影响。

Result: 实验表明，未使用相机外参的策略在固定场景中依赖背景线索推断相机姿态，但当场景变化时性能崩溃；而使用相机外参条件化的策略恢复了性能，实现了仅RGB输入的鲁棒控制。

Conclusion: 显式地将策略与相机外参条件化能够显著提升视角不变模仿学习的鲁棒性，无需深度信息即可实现跨视角的泛化控制。
...
<details>
  <summary>Details</summary>
Motivation: 模仿学习需要大量领域内数据，但人工收集演示数据成本高且质量不一。现有的人机协作系统需要全程人工监控，效率低下。

Method: 开发ARMADA系统，包含FLOAT自主在线故障检测方法，实现并行策略部署，仅在必要时请求人工干预。

Result: FLOAT故障检测准确率接近95%，比现有方法提升20%以上；ARMADA在多次策略部署和后训练中，成功率提升4倍以上，人工干预率降低2倍以上。

Conclusion: ARMADA系统通过减少对人类监督的依赖，实现了更可扩展的部署和更快的场景适应能力。
