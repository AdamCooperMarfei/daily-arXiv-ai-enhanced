...
Motivation: 现有的新视角合成方法在LiDAR仿真和大规模3D场景重建中存在问题，特别是LiDAR特定效应处理不足，需要更准确地模拟LiDAR传感器的真实特性以提高仿真质量。

Method: 通过显式建模LiDAR传感器的关键特性，包括滚动快门效应、激光功率变化和强度衰减等，来改进新视角合成方法在LiDAR仿真中的表现。

Result: 与现有最先进方法相比，该方法在定量和定性评估中都表现出更高的LiDAR仿真准确性。消融研究证明了每个传感器模型组件的重要性。该方法还能在相机视角下生成高分辨率LiDAR扫描。

Conclusion: 该方法通过明确建模LiDAR传感器特性，显著提高了LiDAR仿真的准确性，并具备先进的重新仿真能力，为LiDAR仿真和3D场景重建提供了更有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 结合FMs的通用知识和WMs的动态建模能力，解决复杂观察或领域差距场景中的任务泛化问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖固定稀疏的轨迹监督，无法模拟人类驾驶的分层推理过程。

Method: 提出ReAL-AD框架，包含战略推理注入器、战术推理整合器和分层轨迹解码器，结合视觉语言模型增强推理能力。

Result: 规划准确性和安全性提升超过30%。
...
<details>
  <summary>Details</summary>
Motivation: 探索智能模型是否能为工具设计和操作提供有用先验，以补充机器人智能的研究。

Method: 结合视觉语言模型的代码生成能力和进化搜索，迭代设计工具及操作策略。

Result: 在多样化任务中表现优异，优于人类指定设计和现有手工工具。

Conclusion: VLMgineer为自动化工具发明提供了新方向，并公开了基准和代码以促进研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有土壤湿度传感方法在大规模应用中成本过高，无法满足高分辨率需求，如变量灌溉。

Method: 使用配备TDR传感器和直接推进钻探机制的机器人，结合高斯过程建模的自适应采样策略。

Result: 自适应采样策略优于贪婪基准方法，减少30%旅行距离和5%方差。

Conclusion: MoistureMapper及其自适应采样策略为大规模土壤湿度测绘提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于物理的稳定性指标（如SSM或ZMP）需要难以实时准确测量的接触力、地形几何和机器人质心信息，限制了在动态崎岖地形中的应用。
...
Method: 采用MobileNetV2作为主干网络，设计ASCSPP模块提取DLO特征，结合Atrous Strip Convolution和滑动窗口后处理模块提升识别精度。

Result: 在自建数据集上Miou达75.3%，边缘设备推理速度9.3 FPS，优于现有DLO检测模型。
...
<details>
  <summary>Details</summary>
Motivation: 以往研究多关注机械设计和驱动改进，而步态参数对能量消耗的影响较少被探索。
...
TL;DR: 该论文提出了一种针对动态环境中物体导航的映射与导航系统，结合LLM的语义先验，优化了物体检索的成功率和路径效率。


<details>
  <summary>Details</summary>
Motivation: 解决高细节物体映射因物体移动而快速过时的问题，提升动态或未映射物体查询的导航性能。

Method: 开发了一种从零开始考虑物体可能移动或未映射的映射与导航系统，结合环境背景与LLM语义先验，采用主动在线导航策略。

Result: 在静态和动态物体查询中，系统表现出更高的检索成功率和更短的路径长度，显著优于现有方法。

Conclusion: 通过结合环境背景与LLM语义先验，系统在动态环境中实现了更高效的物体导航，为机器人导航提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 随着机器人操作时间延长，需要积累环境知识并有效利用过去经验来辅助人类。传统EQA方法因上下文窗口有限、缺乏持久记忆等问题，难以应对这一挑战。

Method: 提出了一种受认知科学启发的结构化记忆系统，将经验编码为场景图实例，结合推理和规划算法，实现定向记忆检索和导航。引入基于信息价值的停止标准以平衡探索与记忆检索。

Result: 在真实世界实验和新基准测试中，该方法显著优于现有基线，在答案准确性和探索效率上均有显著提升。

Conclusion: 结构化记忆系统和信息价值停止标准的结合，为长期主动具身问答任务提供了有效解决方案，推动了机器人在复杂环境中的应用。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLMs在控制系统中依赖上下文学习，但需要精心设计的任务示例，且缺乏评估幻觉的方法。

Method: 利用逆最优控制工具和多任务学习，用任务演示替代上下文提示示例，减少对工程设计的依赖。

Result: 方法在仿真和硬件实验中有效，减少了工程需求并能在任务执行前评估幻觉。

Conclusion: DEMONSTRATE方法显著降低了对工程设计的依赖，并通过多任务学习提高了任务执行前的评估能力。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶中的分布外（OOD）场景可能导致危险行为，现有VLM虽能识别场景但决策与动作不对齐。

Method: 采用强化学习与可验证奖励（RLVR）优化VLM，以规划导向指标为目标。

Result: 实验表明LaViPlan提升了OOD条件下的情境意识和决策能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于MoCap的地面真值轨迹因时空校准误差和固有抖动问题，难以准确评估SLAM算法的旋转和帧间误差。

Method: 提出MoCap2GT，包括鲁棒状态初始化器、高阶B样条姿态参数化和退化感知测量拒绝策略。

Result: 实验表明，MoCap2GT优于现有方法，显著提升了SLAM评估的精确性。

Conclusion: MoCap2GT为SLAM算法的精确评估提供了高精度地面真值轨迹，代码已开源。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在运动规划中表现优异，但其训练目标仅近似数据分布，无法直接优化非可微分目标（如安全性和有效性）。

Method: 采用强化学习训练扩散模型，引入奖励加权动态阈值算法，优化非可微分目标。

Result: 在CrowdNav和ETH-UCY等行人数据集上，模型表现优于基线方法。

Conclusion: 该方法为安全有效的运动规划提供了新思路，尤其在复杂动态环境中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: AI感知系统在传感器性能退化时可能失效，现有方法难以全面覆盖所有退化场景，需系统性方法确保安全。

Method: 整合相机噪声因素识别与情境覆盖分析，结合领域、传感器和安全专家意见，扩展退化模型以涵盖影响AI性能的噪声因素。

Result: 提出了一种框架，支持相机退化场景下AI感知系统鲁棒性需求的系统化提取与完整性评估。

Conclusion: 该工作为相机退化场景下AI感知系统的安全需求提供了初步解决方案，为未来研究奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 当前VLN研究过于理想化，未考虑物理机器人部署的实际挑战，如机器人运动限制和环境变化。

Method: 引入VLN-PE平台，评估多种VLN方法（分类模型、扩散模型、基于LLM的无训练方法）在不同机器人类型下的表现。

Result: 发现性能显著下降，原因包括观测空间受限、光照变化及物理碰撞等问题，同时暴露了腿式机器人在复杂环境中的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 随着机器人日益融入日常生活，信任问题变得至关重要，但现有研究缺乏社会学与机器人学的对话。

Method: 结合社会学与社交机器人的见解，分析信任的形成、测试与表现。

Result: 提出跨学科对话的必要性，为理解人机信任提供更扎实的框架。

Conclusion: 通过学科交叉，推动人机交互中信任研究的深入发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有RIG方法通常假设环境已知，但实际环境可能未知或时变，且GP在实时性能上难以扩展到大数据集。

Method: 采用流式稀疏GP技术，结合自适应规划和增量在线映射，优化信息收集效率。

Result: 仿真和真实数据集验证表明，该方法在保持精度的同时显著降低了计算复杂度。

Conclusion: 该方法为未知或时变环境中的高效信息收集提供了可行解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决MPC在实时系统（如机器人）中因计算负担而只能使用短预测时域的问题，避免影响控制性能和长时域目标设计。

Method: 利用可微分MPC和神经网络，通过梯度传播学习模仿长时域MPC行为的压缩成本函数。

Result: 在仿真和真实自动驾驶赛车实验中，ZipMPC表现优于基线方法，接近长时域MPC性能，且能泛化到未训练场景。

Conclusion: ZipMPC在保持计算效率的同时，显著提升了短时域MPC的性能和泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 尽管抓取是机器人的基本技能，但基于学习的6-DOF抓取方法仍难以泛化到不同设备和实际场景。

Method: 提出GraspGen框架，结合扩散变换器架构和高效判别器，引入新的训练方法，并发布包含5300万次抓取的新模拟数据集。

Result: GraspGen在模拟和真实机器人任务中优于现有方法，在FetchBench基准测试中达到最佳性能。

Conclusion: GraspGen通过扩散变换器和判别器的结合，显著提升了抓取生成的性能和泛化能力。
...
Method: 使用预训练的EEG解码器将原始信号转化为概率奖励，结合稀疏外部奖励进行策略学习。

Result: 在模拟环境中，基于EEG反馈的智能体表现与密集人工奖励训练的效果相当。
...
TL;DR: 提出了一种基于多模态感知的少样本工具使用技能迁移框架，通过仿真预训练和现实演示微调，解决了机器人工具操作中的复杂接触问题。


<details>
  <summary>Details</summary>
Motivation: 人类擅长工具操作，但机器人学习这些技能面临复杂接触和现实数据不足的挑战。

Method: 框架包括仿真预训练基础策略以捕捉工具使用中的常见接触状态，并通过现实演示微调以缩小领域差距。

Result: 验证表明，该框架能以少量演示教会机器人使用不同工具完成任务，并通过传感器增强接触状态识别。

Conclusion: 该框架有效提升了机器人工具操作能力，结合传感器进一步优化了接触状态和环境几何的识别。
...
Method: 分两阶段：学习时空运动基元以封装机器人约束，构建STL合规运动计划。使用强化学习生成控制策略库，并通过采样策略实现STL合规规划。
...
<details>
  <summary>Details</summary>
Motivation: 探索四足机器人在医疗领域的潜力，特别是作为导盲犬的替代方案，推动AI驱动的辅助机器人发展。

Method: 使用三种强化学习算法（PPO、DQN、Q-learning）在定制环境中训练模拟四足机器人，评估传感器输入、碰撞频率等指标。

Result: PPO算法在所有指标中表现最优，尤其在平均和每集目标步数上显著优于DQN和Q-learning。
...
Method: 1. 使用光流作为体现无关的动作表示训练世界模型（WM），并在目标体现的小数据上微调；2. 提出潜在策略搜索（LPS）方法，优化行为克隆策略的输出。

Result: 在真实实验中，结合预训练WM和小数据量（30或50次演示）的策略性能显著提升（相对改进50%和20%）。

Conclusion: 通过多体现数据预训练WM和LPS方法，显著减少了对目标体现数据的需求，提升了小数据量下的策略性能。
