...
TL;DR: 提出了农业ODD框架，用于描述和验证自主农业系统的操作边界，解决现有ODD概念在农业应用中的不足。


<details>
  <summary>Details</summary>
Motivation: 农业自动化系统在复杂多变环境中运行，需要处理驾驶和工作过程的双重约束，现有ODD概念无法满足农业应用的特殊需求。

Method: 开发了农业ODD框架，包含三个核心元素：结构化描述概念、扩展的7层模型（包含过程层）和迭代验证流程。

Result: 该框架能够创建明确且可验证的农业ODD，支持自主农业系统环境描述的标准化和可扩展性。

Conclusion: 农业ODD框架为自主农业系统提供了统一的环境描述和验证方法，有助于提升开发效率和系统可靠性。
...
TL;DR: 本文探索了用于比较真实世界和模拟LiDAR扫描的评估指标，发现密度感知Chamfer距离（DCD）在所有测试场景中表现最佳，并验证了虚拟测试环境在自动驾驶系统安全测试中的有效性。


<details>
  <summary>Details</summary>
Motivation: 由于传统物理测试成本高且存在安全风险，需要开发虚拟测试环境（VTE）作为替代方案，通过比较VTE生成的传感器输出与真实世界数据来验证VTE的准确性。

Method: 采用综合实验方法评估不同指标对噪声、密度、失真、传感器方向和通道设置的敏感性和准确性，并使用真实LiDAR扫描数据生成虚拟测试环境，比较模拟和真实LiDAR扫描的模型感知和几何相似性。

Result: 密度感知Chamfer距离（DCD）在所有测试案例中表现最佳，模拟和真实LiDAR扫描在语义分割输出上相似（mIoU为21%），几何特性略有差异（平均DCD为0.63）。

Conclusion: 密度感知Chamfer距离是与感知方法最相关的指标，虚拟测试环境可以有效地用于自动驾驶系统的安全测试验证。
...
TL;DR: AURA是一个用于机器人异常和故障诊断的协作框架，结合了LLM、数字孪生和人机交互，通过双层代理架构实现实时异常检测和响应，并通过人类验证的诊断结果持续改进AI模型。


<details>
  <summary>Details</summary>
Motivation: 在安全关键环境中部署自主系统需要结合人类专业知识和AI分析，特别是在处理不可预见的异常情况时，需要建立可信赖且持续改进的人机协作模式。

Method: 采用双层代理架构：低级状态异常特征化代理监控遥测数据并生成结构化问题描述；高级诊断推理代理与操作员进行基于知识的对话以识别根本原因，利用外部知识源。人类验证的诊断结果转化为训练数据来改进感知模型。

Result: 建立了一个可信赖且持续改进的人机团队模式，通过反馈循环将专家知识逐步提炼到AI中，使其从静态工具转变为自适应合作伙伴。

Conclusion: AURA框架为可信赖、持续改进的人机团队建立了操作原则和具体实现模式，解决了安全关键环境中自主系统部署的挑战。
...
TL;DR: 该论文提出了一种基于模型的机器人学习方法，使用非结构化游戏数据学习视觉世界模型、扩散动作采样器和奖励模型，然后通过MCTS规划器优化动作序列，在真实机器人任务中显著优于行为克隆基线。


<details>
  <summary>Details</summary>
Motivation: 行为克隆方法需要特定任务的演示数据且难以迁移到新任务，而基于模型的方法可以使用易于收集的非结构化数据，实现更好的泛化能力。

Method: 收集少量非结构化游戏数据，学习动作条件视觉世界模型、扩散动作采样器和奖励模型；使用MCTS规划器结合世界模型和动作采样器优化长序列动作；通过零阶模型预测控制器执行计划。

Result: 在3个真实世界机器人任务上的实验表明，该方法相比行为克隆基线有显著改进，动作采样器有效缓解了世界模型在规划过程中的幻觉问题。

Conclusion: 基于模型的规划方法比行为克隆更有效，能够利用非结构化数据实现更好的任务性能，扩散动作采样器对规划过程至关重要。
...
TL;DR: 开发了一个开源库，可将低成本3D打印机转换为自动探测设备，用于生成大量标记训练数据来校准触觉传感器。


<details>
  <summary>Details</summary>
Motivation: 触觉传感器校准过程目前仍然是非标准化且劳动密集型的，需要大量校准工作将原始传感器读数转换为物理上有意义的量。

Method: 使用3D打印机作为自动探测设备收集大量标记训练数据，然后使用定制卷积神经网络重建高质量深度图。

Result: 成功校准了两种商用视觉触觉传感器（DIGIT和GelSight Mini），并进行了数据消融研究以确定准确校准所需的数据量。

Conclusion: 通过自动化触觉传感器校准，该库可以加速触觉传感研究，简化传感器部署，并促进触觉传感在机器人平台中的实际集成。
...
TL;DR: SENT-Map是一种语义增强的拓扑地图，通过JSON文本格式表示室内环境，利用基础模型支持自主导航和操作。


<details>
  <summary>Details</summary>
Motivation: 利用基础模型的进展，为室内环境提供语义信息表示，支持自主导航和操作，避免部署时出现不可行状态。

Method: 采用两阶段方法：首先使用视觉基础模型与操作员一起映射环境，然后使用SENT-Map表示和自然语言查询在基础模型中进行规划。

Result: 实验结果表明，语义增强使得即使是小型本地可部署的基础模型也能成功在室内环境中进行规划。

Conclusion: SENT-Map通过语义增强的拓扑地图表示，有效支持了基于基础模型的室内环境自主导航和操作规划。
...
TL;DR: 提出了一种基于运动先验的方法，成功将深度强化学习应用于真实六足机器人，使其在复杂地形中表现出自然的步态模式和显著的鲁棒性。
...
Result: 学习到的策略成功迁移到真实六足机器人上，在复杂地形中无需视觉信息即表现出自然步态模式和显著鲁棒性。
...
TL;DR: 提出了一个基于学习的框架，将LLM高级任务规划器与混合模仿学习和强化学习的低级策略相结合，用于解决礼品包装等长时程变形物体操作任务。


<details>
  <summary>Details</summary>
Motivation: 在仓库和零售店等环境中，人机协作处理变形物体（如纸张、袋子、布料）时，由于变形材料不可预测的动态特性和自适应力控制的需求，协调机器人动作与人类协助仍然很困难。

Method: 提出Sub-task Aware Robotic Transformer (START)，从人类演示中学习统一策略，通过引入子任务ID提供明确的时间基础，捕获整个包装序列中的长程时间依赖性。

Result: 在真实世界包装任务中实现了97%的成功率，统一基于Transformer的策略减少了对专用模型的需求，允许受控的人类监督。

Conclusion: 该框架有效地将高级意图与变形物体操作所需的细粒度力控制连接起来，支持灵活执行，学习子目标而非仅仅复制运动序列。
...
<details>
  <summary>Details</summary>
Motivation: 传统导纳控制在物理人机协作中难以准确估计人类意图，而纯强化学习方法因安全约束和稀疏奖励不适合板插入任务。

Method: 使用人类设计的导纳控制器辅助强化学习，促进更积极的机器人行为并减少人类努力。

Result: 仿真和真实实验显示，该方法在成功率和任务完成时间上优于导纳控制，且测量到的力/扭矩显著减少。

Conclusion: 提出的RL方法能有效解决盲机器人-人类协作任务，提高协作效率和安全性。
...
TL;DR: GUIDES是一个轻量级框架，通过将基础模型的语义指导注入预训练机器人策略中，无需架构重新设计即可增强策略的语义感知能力。


<details>
  <summary>Details</summary>
Motivation: 预训练机器人策略缺乏基础模型的语义感知能力，但完全替换这些策略成本高昂且会丢失积累的知识。

Method: 使用微调的视觉语言模型生成上下文指令，通过辅助模块编码为指导嵌入，注入策略的潜在空间，并通过简短微调使传统模型适应新输入。推理时使用基于大语言模型的反射器监控置信度并在低置信度时启动推理循环。

Result: 在RoboCasa仿真环境中验证显示任务成功率显著提升，UR5机器人的实际部署证明GUIDES提高了关键子任务（如抓取）的运动精度。
...
<details>
  <summary>Details</summary>
Motivation: 社交焦虑是普遍的心理健康问题，但现有支持不足。社交机器人技术为补充传统心理健康治疗提供了新机会，需要了解用户价值观来设计有意义的解决方案。

Method: 采用参与式设计工作坊，让心理健康学术研究人员通过创意、反思和设想活动，探索场景和设计可能性，系统获取价值观、期望、需求和偏好。

Result: 研究发现适应性、接受度和有效性是社交焦虑支持机器人设计的核心价值观，提供了丰富的设计相关见解。

Conclusion: 研究强调了以研究为主导的价值观获取方法的重要性，在开发社交辅助机器人时需注重以用户为中心和情境感知的设计考量。
...
TL;DR: Dex-Hand 021是一款高性能的绳驱五指机器人手，具有12个主动和7个被动自由度，总自由度19，重量仅1kg。采用基于本体感觉力感知的导纳控制方法，在抓取能力和精度方面表现优异。


<details>
  <summary>Details</summary>
Motivation: 人类手在日常生活和工业应用中具有重要作用，但复制其多功能能力（包括运动、感知和协调操作）仍然是一个巨大挑战。需要平衡人类般的灵活性与工程约束，如复杂性、尺寸重量比、耐久性和力感知性能。

Method: 开发了绳驱五指机器人手Dex-Hand 021，采用基于本体感觉力感知的导纳控制方法来增强操作能力。该手具有12个主动和7个被动自由度。
...
TL;DR: 该论文提出了一个用于分析ROS和ROS 2数据包的MCP服务器，通过LLM和VLM实现机器人数据的自然语言分析、可视化和处理，并评估了不同AI模型在工具调用能力上的表现。


<details>
  <summary>Details</summary>
Motivation: 目前代理AI系统与物理/具身AI系统的交叉研究稀缺，需要开发能够连接这两个领域的技术，特别是利用MCP协议实现机器人数据的自然语言处理。

Method: 开发了专门针对机器人领域知识的MCP服务器工具，支持移动机器人轨迹、激光扫描数据、变换和时间序列数据的分析，同时提供ROS 2 CLI工具接口和时间/主题过滤功能。

Result: 实验评估了8种最先进的LLM/VLM模型，发现Kimi K2和Claude Sonnet 4在工具调用能力上表现明显优于其他模型，工具描述模式、参数数量等因素显著影响成功率。

Conclusion: 该研究填补了代理具身AI领域的空白，证明了MCP在机器人数据分析中的可行性，并为选择合适的AI模型提供了实证依据。
...
TL;DR: 本文研究了人类对机器人视野的错误认知问题，提出了四种增强现实视野指示器来改善人机协作中的视野对齐，并通过用户实验验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 人类往往会错误地认为机器人和人类具有相同的视野范围，这种不准确的心理模型可能导致人机协作失败，特别是在机器人被要求完成视野外物体的不可能任务时。

Method: 提出了四种从自我中心（机器人眼睛和头部空间）到异我中心（任务空间）的增强现实视野指示器，并通过41名参与者的用户实验评估其准确性、信心、任务效率和认知负荷。

Result: 结果显示，任务空间的异我中心块状指示器具有最高的准确性，但解读机器人视野时存在延迟；自我中心的深眼窝指示器也能提高准确性；所有指示器下参与者信心都很高，认知负荷保持较低水平。

Conclusion: 研究为从业者提供了六条指导原则，帮助他们应用增强现实指示器或物理改造来使人类心理模型与机器人视觉能力保持一致。
...
<details>
  <summary>Details</summary>
Motivation: 现有的语义场景补全系统主要针对轮式平台和前向传感器，无法满足腿式/人形机器人在步态引起的身体抖动和360度连续感知方面的需求。

Method: 结合双投影融合(DP-ER)利用环形全景和等距柱面展开，双网格体素化(BGV)在笛卡尔和柱极坐标系中推理，轻量级解码器进行动态多尺度融合，以及无需额外传感器的步态位移补偿(GDC)。

Result: 在QuadOcc和Human360Occ基准测试中达到新的SOTA水平，在QuadOcc上超越强视觉基线和流行LiDAR方法，在H3O上分别提升+3.83 mIoU(同城)和+8.08(跨城)。

Conclusion: OneOcc的模块轻量高效，为腿式/人形机器人提供了可部署的全方位感知能力，填补了全景语义占据感知的空白。
...
TL;DR: 提出了MUP-QBAF框架，用于解决多用户人机交互中的偏好冲突问题，结合用户论点和机器人环境观察，实现动态适应和冲突解决。
...
Method: 基于定量双极论证框架(QBAF)，将用户偏好表示为可迭代更新的论点，结合机器人动态环境观察，构建多用户偏好定量双极论证框架(MUP-QBAF)。

Result: 通过辅助机器人在衰弱评估任务中调解护理人员和被护理者偏好冲突的案例研究，验证了框架的有效性，并进行了参数敏感性分析。
...
TL;DR: 提出了一个流形约束的Hamilton-Jacobi可达性学习框架，用于解决多智能体运动规划中的任务诱导约束问题。


<details>
  <summary>Details</summary>
Motivation: 现实世界中机器人需要在动态环境中导航，同时遵守任务施加的流形约束，如服务机器人必须保持杯子直立并避免碰撞。现有去中心化MAMP方法难以有效整合流形约束。

Method: 通过求解流形约束下的HJR问题来捕获任务感知的安全条件，然后将其集成到去中心化轨迹优化规划器中。

Result: 该方法能够生成既安全又任务可行的运动规划，无需假设其他智能体的策略，在多样化流形约束任务中具有良好泛化能力，并能扩展到高维多智能体操作问题。

Conclusion: 所提出的方法优于现有约束运动规划器，运行速度适合实际应用，为复杂环境下的多智能体运动规划提供了有效解决方案。
...
TL;DR: 该论文研究了在多机器人系统中搜索几何区域内的入侵者问题，针对静态和移动入侵者，证明了该问题是NP难的，并提出了基于空间填充曲线、随机搜索和协同随机搜索的高效算法。


<details>
  <summary>Details</summary>
Motivation: 研究多机器人在几何区域内搜索入侵者的问题，旨在开发高效且鲁棒的搜索算法，以应对NP难的搜索挑战。

Method: 提出了三种算法：基于空间填充曲线的搜索、随机搜索和协同随机搜索，并分析了搜索机器人数量与搜索时间之间的权衡关系。

Result: 证明了即使在静态入侵者情况下，搜索问题也是NP难的，但提出的算法能够在实际应用中提供有效的解决方案。

Conclusion: 通过分析几何特性和机器人数量与搜索时间的权衡，为多机器人搜索系统提供了实用的算法框架。
...
TL;DR: 本文介绍了一种创新的自主无人机系统，成功完成了世界上最大的无人机绘制壁画。该系统结合红外运动捕捉摄像头和LiDAR技术，能够在恶劣户外条件下实现精确导航和绘画。


<details>
  <summary>Details</summary>
Motivation: 解决在户外恶劣条件（如风和阳光直射）下保持艺术精度和操作可靠性的双重挑战，扩展机器人在创意领域的应用。

Method: 采用结合红外运动捕捉摄像头和LiDAR的新型导航系统，使用切向和法向不同调节的控制架构，开发了轨迹规划和路径优化算法，并设计了专门防止螺旋桨气流干扰的喷漆机制。

Result: 实验结果表明系统在各种条件下都具有鲁棒性和精确性，成功完成了大型壁画创作。

Conclusion: 该系统展示了在自主大规模艺术创作方面的潜力，扩展了机器人在创意领域的应用功能。
...
TL;DR: 提出了一种在不确定环境中实现时空逻辑任务的运动规划方法，使用特殊乘积自动机处理语义标签的不确定性，并通过值迭代进行在线重规划。


<details>
  <summary>Details</summary>
Motivation: 解决在环境语义标签存在概率性不确定性的情况下，如何实现scLTL规范的运动规划问题。例如任务要求"先到区域1再到区域2"，但区域位置未知，只有概率信念可用。

Method: 构建特殊乘积自动机来捕捉语义标签的不确定性，为自动机的每条边设计奖励函数，并利用值迭代进行在线重规划。
...
TL;DR: 研究探索如何利用人类运动线索设计富有表现力的机器人手臂动作，通过Geister游戏分析人类自然游戏动作和指令表达动作，并基于此创建特定阶段的机器人动作。


<details>
  <summary>Details</summary>
Motivation: 研究旨在理解人类运动线索如何转化为机器人动作表达，特别是在非完全信息游戏情境下，探索如何让机器人动作更具表现力和可理解性。

Method: 分析人类在Geister游戏中的两种动作：自然游戏动作（无意识倾向）和指令表达动作（有意线索），基于发现创建不同速度和停止时长的阶段特定机器人动作，并在物理机器人和视频两种呈现方式下评估观察者印象。

Result: 结果表明，后期动作时机（特别是撤回阶段）在印象形成中起重要作用，物理实体化增强了运动线索的可解释性。

Conclusion: 基于人类时机行为的发现为设计富有表现力的机器人动作提供了见解，强调了动作时机和物理体现的重要性。
...
TL;DR: 提出了一种自包含的软体抓取器，通过内部液体在三联双稳态腔室间重新分配实现无外部能源的抓取，具有尺寸选择性和刚度适应性。


<details>
  <summary>Details</summary>
Motivation: 传统流体驱动软体抓取器依赖外部能源，限制了便携性和长期自主性，需要开发自包含的无源驱动方案。

Method: 设计三联双稳态腔室结构，利用顶部传感腔变形触发液体重新分配，驱动抓取腔的突跳扩张，实现无持续能量输入的稳定抓取。

Result: 实现了无需外部能源的尺寸选择性抓取，并能根据物体刚度被动调节抓取压力，适用于水下和野外环境。

Conclusion: 这种无源紧凑设计为轻量化、刚度自适应的流体驱动操作开辟了新可能，为特定尺寸采样和操作提供了可行方案。
