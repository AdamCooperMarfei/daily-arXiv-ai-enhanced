...
Method: 使用闭环视觉运动控制器，通过模拟训练，仅依赖腕部摄像头的光学流图像。
...
<details>
  <summary>Details</summary>
Motivation: 多传感器平台的自动外参标定需要高效、通用且少人工干预的方法，现有方法在传感器数量增加时工程成本高。

Method: 引入了一种全局最优算法，支持多传感器和目标姿态的联合估计，适用于单目相机，并提供了局部求解器和开源实现。

Result: 算法性能优于现有方法，推导了新的可识别性准则，并提供了全局最优性的先验保证。

Conclusion: 提出的方法在效率和通用性上优于现有方案，适用于多传感器平台，并提供了开源工具。
...
<details>
  <summary>Details</summary>
Motivation: 四足机器人由于参考运动数据多样性有限，难以实现多样化运动。

Method: 1. 使用CVAE生成多风格动态可行的运动序列；2. 采用对抗运动先验算法。

Result: 生成的运动数据提升了控制器稳定性和速度跟踪性能，实验验证了框架在真实机器人上的有效性。

Conclusion: 该框架显著改善了速度跟踪和部署稳定性，能够生成并执行复杂运动模式。
...
TL;DR: 提出了一种新型感知代理，结合语音提示工程、大语言模型和基础模型，实现更自然的手术中人机交互。
...
Method: 利用语音集成提示工程的大语言模型、SAM和任意点跟踪基础模型，结合记忆库和两种新机制，实现灵活分割。
...
<details>
  <summary>Details</summary>
Motivation: 现有MTRL研究主要局限于低并行化环境下的离策略方法，而MTRL可以利用在策略算法的高渐近性能，并通过GPU加速模拟实现大规模并行化。

Method: 提出MTBench基准，包含广泛的任务分布和多种RL算法与MTRL架构，提供统一评估框架。

Result: 实验表明MTBench能显著加速MTRL方法的评估，同时揭示了大规模并行化与MTRL结合的新挑战。
...
<details>
  <summary>Details</summary>
Motivation: 提升四足机器人的稳定性和运动可塑性，特别是在狭窄路径行走和动态稳定性方面。

Method: 基于质心动力学模型设计控制器，利用QP求解器调节推进器和足部地面接触力，采用模型预测控制框架。

Result: 成功模拟了四足机器人在狭窄路径上的稳定行走，并验证了推进器在侧向推力恢复中的有效性。

Conclusion: 推进器辅助行走能显著提升机器人在复杂环境中的动态稳定性，为未来机器人设计提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 可重构的多机器人单元能够适应波动的装配需求，但其配置的频繁规划带来了新的挑战，尤其是如何生成优化的、协调的多机器人运动序列。

Method: 将装配步骤分为任务相关的核心操作和连接性的遍历操作，核心操作固定，遍历操作可优化。通过分解运动规划策略整合可行的遍历操作，探索了多种求解技术。

Result: 提出的方法生成了高效且无碰撞的多机器人装配流程，优于基于分散式机器人个体运动规划的基线方法。

Conclusion: 仿真实验验证了该方法的有效性，能够显著减少装配时间。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于相机传感器（包括RGB-D）的3DGS方法存在计算负载高、在低纹理或光照差环境中失效以及操作范围短的问题。LiDAR虽为替代方案，但其与3DGS结合时面临全局对齐和稀疏数据导致的优化时间长等新挑战。

Method: GSFusion通过全局位姿图优化中的surfel-to-surfel约束确保高精度地图一致性，采用像素感知的高斯初始化策略和边界Sigmoid约束处理稀疏数据。

Result: 在公开和自有数据集上的实验表明，GSFusion在渲染质量和建图效率上优于现有3DGS SLAM系统。

Conclusion: GSFusion成功解决了LiDAR与3DGS结合的挑战，为高精度实时映射提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决触须传感器在机器人感知中缺乏主动控制的问题，特别是在尖锐表面上定位时容易滑动的挑战。

Method: 设计了一种由三个柔性螺旋臂组成的磁传导触须传感器，利用梯度下降提取接触点位置，并通过贝叶斯滤波减少波动。提出主动运动控制策略，使用B样条曲线预测局部曲率以确定传感器方向。

Result: 算法能有效跟踪物体并实现亚毫米精度的轮廓重建，仿真和实际实验验证了其有效性。

Conclusion: 该方法为触须传感器在机器人感知中的应用提供了高精度和鲁棒性的解决方案。
...
Method: 提出了一种基于原因的轨迹评估框架，量化人类代理的原因（如法规遵从性），并通过可调整的权重和平衡函数评估候选轨迹。

Result: 通过真实世界启发的超车场景，展示了框架如何揭示法规遵从性、效率和舒适性之间的冲突。

Conclusion: 该框架为评估日常场景中的伦理对齐提供了透明工具，是实现有意义人类控制（MHC）的实用步骤。
...
<details>
  <summary>Details</summary>
Motivation: 解决强化学习在车辆漂移控制中仿真与现实差距大的问题。

Method: 采用GPU加速的并行仿真和系统域随机化，设计了一个强化学习框架。

Result: 在仿真和真实RC车平台上实现了复杂操作中的精确轨迹跟踪和可控侧滑角。
...
<details>
  <summary>Details</summary>
Motivation: 农业环境中自主移动机器人的需求增长，需要高效路径规划以减少行驶距离并避免土壤和植被损害。

Method: 采用DTSP进行全局路径规划，生成最短且曲率受限的路径；结合NMPC进行局部路径规划与控制。

Result: 仿真验证显示，该方法路径更平滑、更短，比解耦方法减少约16%的行驶距离。
...
Method: 重新解释PID增益作为复杂未建模动态的替代，引入物理引导增益正则化方案，并结合参数条件化的RNN控制器。

Result: 在带有110:1变速箱的两轮平衡机器人上，增益正则化方法在硬件中的角稳定时间与仿真接近，而纯域随机化策略则表现不佳。

Conclusion: 该方法为缩小仿真到现实差距提供了一种轻量级、可重复的框架，适用于经济型机器人硬件。
...
<details>
  <summary>Details</summary>
Motivation: 解决机器人模仿学习中高质量演示数据稀缺的问题，利用人类操作视频的丰富行为先验。

Method: 两阶段训练：预训练于大规模人类操作数据，微调于机器人数据；基于扩散变换器架构和流匹配。
...
<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型在生成结构化且可执行机器人任务规划中的潜力。

Method: 通过直接使用PDDL文件和Fast Downward规划器对比评估多种语言模型的规划性能。

Result: 模型在简单任务中表现良好，但在复杂场景中（如资源管理、状态跟踪和约束遵循）表现不佳。
...
<details>
  <summary>Details</summary>
Motivation: 由于用户解剖学差异和穿戴不一致性，手部外骨骼的精确跟踪面临挑战，导致运动性能下降。

Method: 采用冗余关节感知和残差加权优化策略，结合运动捕捉数据调整成本函数权重。
...
<details>
  <summary>Details</summary>
Motivation: 改进原有DRACo-SLAM框架，解决水下机器人团队在缺乏先验几何信息时高效检测闭环的问题。

Method: 引入对象图表示声纳地图，利用对象图匹配实现高效闭环检测；提出增量GCM方法优化扫描匹配。
...
<details>
  <summary>Details</summary>
Motivation: 现有LiDAR定位方法在长期环境变化下鲁棒性不足，导致定位漂移和可靠性下降，亟需一种更稳健的解决方案。

Method: DuLoc方法通过紧密耦合LiDAR-惯性里程计与离线地图定位，并引入恒定速度运动模型以减少噪声，实现了动态与静态地图的无缝集成。

Result: 在超大规模港口环境中进行了2856小时的实验，结果表明DuLoc在大规模动态户外环境中优于其他先进LiDAR定位系统。

Conclusion: DuLoc通过结合动态与静态地图，显著提升了LiDAR定位的鲁棒性和准确性，适用于复杂变化环境。
...
<details>
  <summary>Details</summary>
Motivation: 现有3DGS-SLAM系统主要针对室内环境且依赖主动深度传感器，缺乏适用于户外大规模场景的解决方案。

Method: 使用双目RGB图像对，结合预训练深度立体网络的深度估计，通过多损失策略优化3D高斯溅射。

Result: 在多个数据集上，BGS-SLAM在复杂户外环境中表现出优于其他3DGS解决方案的跟踪精度和地图性能。

Conclusion: BGS-SLAM填补了户外3DGS-SLAM的空白，展示了高效和高质量的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 探索如何通过潜在动作（视觉变化的抽象表示）提升视觉-语言-动作模型在机器人操作任务中的表现。

Method: 提出ViLLA框架，改进潜在动作的学习和整合方式，结合视觉-语言-动作预训练。
...
Method: 提出跨视图目标规范作为统一多任务目标空间，并利用Minecraft环境进行自动化任务合成和大规模多任务RL训练。

Result: 实验结果显示，RL将交互成功率提高了4倍，并在多样环境中实现了零样本空间推理泛化。

Conclusion: 研究表明，RL在3D模拟环境中的训练具有巨大潜力，尤其是支持大规模任务生成的环境，能显著提升视觉运动代理的空间推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 昆虫级机器人在复杂环境中需要可靠的触觉感知，但现有传感器技术受限于尺寸、重量和功耗。

Method: 设计了一种多段式柔性层压传感器，嵌入电容角度传感器，实现被动弯曲和精确角度测量。

Result: CITRAS在静态和动态弯曲中误差分别仅为0.79度和3.58度，并能预测距离、估计间隙宽度和区分表面纹理。
...
Method: 采用分布式AI代理，结合多模态感知、自适应推理、动态任务规划和实时决策，利用检索增强生成、强化学习和ROS 2节点动态生成等技术。

Result: 实验验证显示UROSA在模拟和实际水下任务中表现出优异的适应性和可靠性。
