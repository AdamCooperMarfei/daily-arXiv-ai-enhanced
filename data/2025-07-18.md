...
Motivation: 现有的新视角合成（NVS）方法在LiDAR仿真和大规模3D场景重建中得到应用，但对LiDAR特定效应的处理仍不充分。虽然已有针对更快渲染或处理动态场景的解决方案，但缺乏对LiDAR传感器特性的精确建模。

Method: 通过显式建模LiDAR传感器的关键特性，包括滚动快门效应、激光功率变化和强度衰减规律，来改进新视角合成方法，从而实现更准确的LiDAR仿真效果。

Result: 通过与现有最先进方法的定量和定性比较，以及消融研究验证了方法的有效性，证明了每个传感器模型组件的重要性。此外，该方法还展现了先进的重仿真能力，能够从相机视角生成高分辨率LiDAR扫描。

Conclusion: 所提出的方法通过精确建模LiDAR传感器特性，显著提升了LiDAR仿真的准确性，为LiDAR仿真和3D场景重建提供了更有效的解决方案，并具备良好的实用性和扩展性。
...
Conclusion: FOUNDER通过结合FMs和WMs，实现了任务语义的深度捕捉和奖励函数的一致性验证。
...
<details>
  <summary>Details</summary>
Motivation: 现有端到端自动驾驶方法依赖稀疏轨迹监督，难以模拟人类驾驶的层次推理过程，限制了适应性和性能。

Method: 提出ReAL-AD框架，包括战略推理注入器、战术推理整合器和分层轨迹解码器，结合视觉语言模型增强情境感知和推理。

Result: 实验表明，该框架将规划准确性和安全性提升30%以上，使自动驾驶更接近人类推理。

Conclusion: ReAL-AD通过层次化推理和视觉语言模型，显著提升了自动驾驶的决策质量和可解释性。
...
<details>
  <summary>Details</summary>
Motivation: 探索基础模型是否能通过智能工具设计提升物理世界的任务解决能力。
...
Method: 使用轻量级神经网络IMUnet和基于ArUco标记的视觉跟踪方法（C3分数）来估计和量化机器人稳定性。

Result: 在多种地形和速度下验证了方法的有效性，并展示了其对未见条件的泛化能力。

Conclusion: 该方法为农业和太空应用中的移动操作任务提供了一种有效的稳定性估计和监督机制。
...
Method: 提出ASCnet轻量级分割模型，结合MobileNetV2和ASCSPP模块，高效检测可变形线性物体（DLOs），并通过滑动窗口（SW）后处理模块提升识别精度。

Result: 在自建数据集上达到75.3%的Miou分数，在Jetson Orin Nano设备上实现9.3 FPS的推理速度。

Conclusion: 该方法在推理速度和分割性能间取得平衡，优于现有DLO检测模型，并在实际机器人平台上验证成功。
...
Method: 建模Unitree A1机器人并开发控制器独立调整步态参数，在Gazebo中模拟不同速度下的步态，并进行实验验证。
...
<details>
  <summary>Details</summary>
Motivation: 现有高细节对象地图因物体频繁移动而快速过时，需开发一种能适应动态或未映射对象的导航系统。

Method: 构建一个结合环境地图与LLM语义先验的系统，采用主动在线方法导航至目标对象。

Result: 在静态和动态对象查询中，新方法在检索成功率和路径长度上优于现有方法。

Conclusion: 通过结合环境地图与LLM语义先验，系统显著提升了动态或未映射对象导航的效率。
...
<details>
  <summary>Details</summary>
Motivation: 随着机器人运行时间延长，需要积累环境知识并有效利用以辅助人类。传统EQA方法在长期记忆和主动探索方面存在不足，因此需要新的解决方案。

Method: 提出基于场景图的结构化记忆系统，结合认知科学的记忆宫殿方法，设计推理与规划算法，并引入信息价值停止准则以平衡探索与记忆检索。

Result: 在真实实验和新基准测试中，该方法显著优于现有基线，提高了答案准确性和探索效率。

Conclusion: 结构化记忆系统有效解决了长期主动具身问答的挑战，为机器人长期运行中的知识利用提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖LLMs生成优化问题，但需要工程师设计任务示例且无法评估幻觉，DEMONSTRATE旨在解决这些问题。

Method: 利用任务描述的嵌入表示和逆最优控制工具，结合多任务学习，避免使用LLMs生成优化问题。

Result: 通过仿真和硬件实验验证了方法的有效性，减少了工程设计的依赖并提前评估幻觉。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶中，视觉语言模型（VLM）在分布外（OOD）场景中表现出泛化能力，但其高层决策与低层轨迹预测存在不匹配问题，导致决策缺乏上下文感知。

Method: 提出LaViPlan框架，结合强化学习与可验证奖励（RLVR），通过规划导向的指标优化VLM。

Result: 实验表明，该方法在OOD条件下提升了情境感知和决策能力，缓解了不匹配问题。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在运动规划中表现优异，但无法直接优化不可微分目标（如碰撞避免和目标达成）。

Method: 采用强化学习训练扩散模型，引入奖励加权动态阈值算法以优化安全性和有效性。

Result: 在行人数据集（CrowdNav, ETH-UCY）上实现了最先进的性能。

Conclusion: 该方法能有效学习不可微分目标，提升运动规划的安全性和有效性。
...
Method: 结合相机噪声因素识别与情境覆盖分析，扩展退化模型，并引入领域、传感器和安全专家意见。

Result: 提出了一种支持鲁棒性需求原则性制定和完整性评估的框架。
...
<details>
  <summary>Details</summary>
Motivation: 弥补现有视觉与语言导航（VLN）方法在物理机器人部署中的不足，提供更真实的评估平台。

Method: 引入VLN-PE平台，支持人形、四足和轮式机器人，评估多种技术方案，包括分类模型、扩散模型和基于LLM的无训练路径规划。

Result: 发现性能显著下降，原因包括机器人观察空间有限、环境光照变化及物理挑战（如碰撞和跌倒）。
...
Result: 提出跨学科对话的必要性，为人类-机器人互动中的信任提供更扎实的框架。

Conclusion: 需通过跨学科合作，构建更适应发展的信任理解框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有RIG方法假设环境已知，但实际环境可能未知或时变，且GP在实时性能上难以扩展到大数据集。
...
<details>
  <summary>Details</summary>
Motivation: 解决模型预测控制（MPC）在实时系统（如机器人）中因计算负担而难以应用长时域预测的问题。

Method: 利用可微分MPC和神经网络，通过MPC优化传播模仿损失的梯度，学习压缩成本函数。

Result: 在仿真和真实自动驾驶赛车实验中，ZipMPC表现优于基线方法，接近长时域MPC性能，并在未见过的赛道上保持良好表现。

Conclusion: ZipMPC有效平衡了长时域目标优化与计算效率，适用于复杂实时系统。
...
Method: 提出GraspGen框架，结合扩散变换器架构和高效判别器，并引入新的训练方法。发布包含5300万次抓取的新模拟数据集。

Result: GraspGen在模拟和真实机器人任务中优于现有方法，在FetchBench基准测试中达到最优性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统强化学习在稀疏奖励条件下效果不佳，而现有的基于人类反馈的方法（RLHF）依赖显式反馈，增加了用户认知负担。

Method: 利用错误相关电位（ErrPs）解码脑电信号，将其转化为概率奖励，结合稀疏外部奖励进行策略学习。

Result: 在MuJoCo仿真环境中，使用Kinova Gen2机械臂完成复杂任务，性能与密集人工奖励相当。

Conclusion: 隐式神经反馈为交互式机器人提供了一种可扩展且符合人类期望的强化学习方法。
...
Method: 框架包括仿真预训练基础策略以捕捉工具使用中的常见接触状态，并通过少量真实世界人类演示微调以弥合领域差距。

Result: 验证了该框架能在Franka Emika机械臂上通过少量演示教授表面跟随任务，并提升工具-环境接触关系的识别能力。

Conclusion: 多模态传感器结合增强了接触状态和环境几何的识别，机器人能通过迁移学习掌握新工具使用技能。
...
<details>
  <summary>Details</summary>
Motivation: 解决自主机器人在复杂环境中执行STL任务时面临的轨迹规划与控制问题。

Method: 分两阶段：1) 学习时空运动基元以封装机器人约束；2) 基于这些基元构建符合STL的运动计划。采用强化学习生成控制策略库，并通过采样方法实现STL合规运动规划。

Result: 在差速驱动和四足机器人上验证了该方法的有效性，能够生成符合STL的运动计划。

Conclusion: 提出的无模型方法能够适应多种环境和STL任务，展示了其通用性和实用性。
...
Method: 通过自定义环境和统一数据收集，比较三种强化学习算法的性能，重点关注传感器输入、碰撞频率和奖励信号。

Result: PPO在导航和避障任务中表现最佳，优于DQN和Q-learning。

Conclusion: PPO算法在复杂环境中表现优异，为AI驱动的四足机器人辅助应用提供了可行性支持。
...
<details>
  <summary>Details</summary>
Motivation: 减少学习视觉运动机器人策略时的数据收集成本，利用现有或低成本的多体现数据（如公开机器人数据集和人类玩耍数据）提升策略性能。

Method: 1. 使用光流作为体现无关的动作表示，在多体现数据集上训练世界模型（WM），并在目标体现的小量机器人数据上微调。2. 开发潜在策略引导（LPS）方法，通过在世界模型的潜在空间中搜索更好的动作序列来改进行为克隆策略的输出。

Result: 在真实世界实验中，结合预训练世界模型和潜在策略引导，显著提升了小数据量下的策略性能（30次演示相对提升50%，50次演示相对提升20%）。

Conclusion: 通过利用多体现数据和潜在策略引导，能够显著减少数据收集需求并提升视觉运动策略的性能。
