...
Motivation: 现实世界中人类很少直接发出指令，而是通过对话、环境声音和视觉线索表达意图，需要机器人能够主动推断用户意图，而现有方法主要依赖显式指令。

Method: 提出Perceiver-Thinker-Talker-Executor框架，基于端到端全模态LLM，统一意图识别、交互确认和动作执行，时空融合听觉和视觉信号，并构建了OmniAction数据集（14万条数据）。

Result: 在仿真和真实环境实验中，RoboOmni在成功率、推理速度、意图识别和主动协助方面均优于基于文本和ASR的基线方法。

Conclusion: RoboOmni框架能够有效处理跨模态上下文指令，实现主动意图识别，为机器人操作任务提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 虽然社交机器人在学习任务支持方面已被广泛研究，但在辅助学生自主学习方面的潜力尚未充分探索。

Method: 采用探索性Wizard-of-Oz研究，比较目标提醒、情感支持和仅物理存在（控制组）三种条件对大学生自习的影响。

Result: 目标提醒和情感支持条件下的参与者报告了更高的易用性，目标提醒组还表现出更强的未来使用意愿。参与者对机器人的满意度与其将机器人视为社会存在的感知相关，这种感知是自习任务目标达成水平的预测因子。

Conclusion: 社交辅助机器人通过功能和情感参与，在支持自主学习方面具有巨大潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有轮腿式机器人依赖昂贵的执行器和传感器，且很少集成跌倒恢复功能，特别是针对轮腿形态的恢复。
...
<details>
  <summary>Details</summary>
Motivation: 解决动态环境中实时感知的数据瓶颈问题，从压缩数据流中创建高质量的3D世界表示，为复杂动态环境中的机器人学习和部署提供关键支持。

Method: 集成两个互补模块：基于光度误差和结构相似性误差的选择模块，以及基于动量的更新模块，根据场景运动动态调整关键帧选择阈值。

Result: 在Spann3r和CUT3R两个先进的3D重建网络上验证，相比传统静态关键帧选择策略（如固定时间间隔或均匀跳帧）有显著改进，消融研究证实了各模块的有效性。

Conclusion: 该方法代表了自适应感知系统的重要进展，能够动态响应复杂和变化的视觉场景，为动态环境中的3D重建提供了有效的解决方案。
...
Method: 使用Cosserat杆理论建立模型，允许每个管扭转和伸长，同时强制弯曲共享中心线。

Result: 实验验证显示尖端预测误差小于机器人总长度的4%，应用于现有机器人时最大偏差约为总长度的5%。
...
<details>
  <summary>Details</summary>
Motivation: 为了解决软体手在密集堆放的多个物体中抓取单个物体的问题，需要软体手指具备在平面内和平面外方向的自适应扭转变形功能，以便深入物体间的有限间隙进行抓取操作。

Method: 提出可变刚度机制，通过压力变化自适应调整刚度；进行有限元分析确定设计参数；开发软体手指并进行基础实验和多种物体抓取演示。

Result: 成功开发出具有自适应扭转变形能力的软体手指，能够实现包裹式抓取，在密集物体环境中有效工作。

Conclusion: 所提出的可变刚度软体手指通过单一驱动源实现了自适应扭转变形和包裹抓取功能，为密集环境下的物体抓取提供了有效解决方案。
...
Method: 通过系统分类方法，按架构（集中式、分布式）对方法进行归类，并分析多智能体一致性对齐、通信效率、高斯表示、语义蒸馏、融合与位姿优化、实时可扩展性等核心组件。

Result: 提供了关键数据集和评估指标的总结，为性能评估提供背景。识别了该领域的主要挑战和局限性。

Conclusion: 确定了未来的关键研究方向，包括终身建图、语义关联与建图、多模型鲁棒性以及弥合Sim2Real差距等开放挑战。
...
Method: 基于感知输入评估预训练策略的可行性和能耗，通过预测执行安全性和运输成本来选择最优策略。

Result: 在楼梯爬升和下降任务中，相比传统端到端DRL策略，VOCALoco在模拟和真实场景中都表现出更好的鲁棒性和安全性。

Conclusion: VOCALoco框架通过模块化技能选择有效解决了腿式机器人在复杂地形运动中的安全性和能效问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有的BVC计算模型大多局限于二维环境，在存在水平对称性的环境中容易产生空间模糊性。为了解决这一局限性，需要在BVC框架中融入垂直角度敏感性。

Method: 提出的模型处理LiDAR数据以捕获垂直轮廓，从而区分在纯二维表示下无法区分的位置。通过将垂直维度纳入BVC框架，实现三维边界检测。

Result: 实验结果显示，在垂直变化最小的环境中，提出的3D模型与2D基线性能相当；但随着3D复杂度的增加，它产生了显著更多的独特位置场，并明显减少了空间混叠。

Conclusion: 在BVC定位中添加垂直维度可以显著增强真实世界3D空间中的导航和映射能力，同时在更简单的近平面场景中保持性能相当。
...
<details>
  <summary>Details</summary>
Motivation: 真实世界驾驶数据限制了训练场景的多样性，而合成场景生成虽然能丰富数据，但在端到端自动驾驶模型中的应用尚未充分探索，主要因为缺乏指定的ego车辆和传感器输入。

Method: 在多智能体合成场景中指定信息最全面的智能体作为ego车辆；将路径级场景投影到地图上；开发Map-to-BEV网络从地图生成鸟瞰图特征；设计训练策略整合地图基合成数据与真实驾驶数据。

Result: 实验结果表明SynAD有效整合了所有组件，显著提升了安全性能。

Conclusion: SynAD通过桥接合成场景生成与端到端自动驾驶，为更全面和鲁棒的自动驾驶模型开辟了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 感知模糊性和任务冲突限制了基于模仿学习的多任务机器人操作性能，需要解决视觉相似任务的区分问题和多任务间的干扰问题。

Method: 结合语言条件化视觉表征模块(LCVR)解决感知模糊性，通过语言指令对视觉特征进行语义基础；使用语言条件化混合专家密度策略(LMoE-DP)缓解任务冲突，采用稀疏专家架构专门处理不同的多模态动作分布，并通过梯度调制进行稳定。

Result: 在真实机器人基准测试中，LCVR将ACT和DP的成功率分别提升33.75%和25%；完整框架达到79%的平均成功率，比先进基线提升21%。

Conclusion: 结合语义基础和专家专业化能够实现鲁棒、高效的多任务操作，证明了该框架在解决多任务机器人操作挑战方面的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 解决多机器人在障碍密集非凸环境中的协作探索问题，特别关注动态平衡的区域划分和任务分配，以提高整体探索效率和团队工作负载均衡。

Method: 使用增量更新的拓扑图结构表征空间连通性和全局探索完整性；提出分布式加权拓扑图Voronoi算法实现平衡图空间划分；局部规划器优化探索目标访问序列并生成安全平滑的运动轨迹。

Result: 与最先进方法相比，在探索效率、完整性和机器人团队工作负载平衡方面表现出显著改进，理论保证了分布式共识收敛和恒定边界的公平图空间划分。

Conclusion: 该方法为多机器人在复杂环境中的协作探索提供了一种有效的分布式解决方案，通过拓扑图结构和平衡分区策略实现了高效的探索性能和良好的负载均衡。
...
Method: 利用线性微分方程的叠加特性解耦每个接触点的平移动力学，使用贝塞尔多项式的微分矩阵推导位置与力的解析关系，利用贝塞尔多项式的凸包特性确保摩擦锥约束。

Result: 所提出的轨迹优化框架能够为足式机器人生成具有各种步态序列的动态可靠运动。

Conclusion: 该方法在四足机器人模型上验证了动力学和运动生成的可行性。
...
<details>
  <summary>Details</summary>
Motivation: 现有端到端自动驾驶框架主要依赖模仿学习，但受限于次优专家演示和部署时的协变量偏移；强化学习虽能利用模拟环境扩展，但通常局限于低维符号输入，无法实现真正的端到端学习。

Method: 提出ZTRS框架，结合离线强化学习和Exhaustive Policy Optimization (EPO)算法，专门针对可枚举动作和奖励的变体策略梯度方法，直接从高维传感器数据学习而不依赖模仿学习。

Result: 在Navtest、Navhard和HUGSIM三个基准测试中表现优异，在Navhard上达到最先进水平，在HUGSIM上优于基于模仿学习的基线方法。

Conclusion: ZTRS证明了完全消除模仿学习、仅通过奖励学习处理高维传感器数据的可行性，为端到端自动驾驶提供了新的研究方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的具身智能体缺乏在线规划和执行复杂自然语言控制任务的能力，无法满足以人为中心的人工智能对机器人智能水平的高要求

Method: 提出包含人机语音交互模块、视觉语言智能体模块和动作执行模块的机器人具身智能体框架。视觉语言智能体包括基于视觉的任务规划器、自然语言指令转换器和任务执行反馈评估器

Result: 在模拟和真实环境中，该智能体的平均任务成功率比仅使用LLM+CLIP的方法提高了28%，显著提升了高级自然语言指令任务的执行成功率

Conclusion: 基于视觉语言模型的智能体框架能够有效实现复杂自然语言控制任务的在线规划和执行，为以人为中心的人工智能提供了新的实现路径
...
<details>
  <summary>Details</summary>
Motivation: 解决传统视觉导航方法局限于单目标、单模态和封闭集目标设置的问题，满足多模态、开放词汇查询和多目标视觉导航的实际需求。

Method: 在探索阶段构建统一3D语言记忆，根据任务目标查询记忆预测候选目标位置，并集成基于局部感知的验证机制在导航过程中动态匹配和验证目标。

Result: LagMemo的记忆模块实现了有效的多模态开放词汇目标定位，在多目标视觉导航任务中优于最先进方法。

Conclusion: LagMemo系统通过语言3D高斯泼溅记忆和动态验证机制，为多模态开放词汇多目标视觉导航提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统行为克隆依赖完全知情的专家演示，需要大量任务演示才能实现泛化。本文旨在通过限制专家信息来减少所需演示数量并提高泛化能力。

Method: 让专家在部分信息被隐藏的情况下（"蒙眼"）执行任务，迫使其进行探索性行为，然后克隆这种蒙眼专家的行为。

Result: 在真实机器人插孔任务和Procgen基准测试中，克隆蒙眼专家比克隆完全知情专家在未见任务上泛化更好。理论分析表明泛化误差与√(I/m)成正比，其中I是专家可用的任务信息量，m是演示任务数量。

Conclusion: 克隆蒙眼专家在较少演示任务的情况下能实现更好的泛化，为物理世界基础模型提供了一种更高效的学习方法。
...
TL;DR: 提出HMAMP方法，使用对抗运动先验学习人类风格的操纵技能，在锤击任务中表现优于基线方法，并展示了真实机器人应用潜力。
...
Method: 使用对抗网络建模工具和物体操纵的复杂动力学及任务目标，判别器结合真实世界数据和模拟数据进行训练，生成符合人类运动统计特性的真实运动轨迹。

Result: 在锤击任务中，HMAMP能够学习人类风格的操纵技能，性能优于当前基线方法，并成功在真实机器人手臂上执行锤击任务。

Conclusion: HMAMP代表了开发能够以更自然直观方式与人类互动机器人的重要进展，通过学习类似人类的工具和物体操纵方式。
...
<details>
  <summary>Details</summary>
Motivation: 解决机器人操作策略泛化性差的问题，现有方法要么依赖2D视觉预训练（关注静态语义或场景几何），要么使用大规模视频预测模型（关注2D动态），无法联合学习操作所需的几何、语义和动态信息。
...
Conclusion: DynaRend能够统一捕获空间几何、未来动态和任务语义，学习到的表示可以通过动作价值图预测有效转移到下游机器人操作任务中。
...
<details>
  <summary>Details</summary>
Motivation: CoNi-MPC框架虽然能有效处理UAV控制，但缺乏环境信息导致障碍物避让困难，需要一种不依赖全局状态估计的避障解决方案。

Method: 利用UAV的单帧原始LiDAR数据生成调制矩阵，直接调整四旋翼速度实现避障，在UGV的非惯性坐标系中实时生成无碰撞轨迹。

Result: 算法计算效率高（每次迭代小于5ms），能在动态和不可预测环境中保持安全性，适用于静态和动态环境。

Conclusion: CoNi-OA为UAV-UGV协同系统提供了一种高效、实时的障碍物避让方法，无需全局状态估计或障碍物建模，具有广泛适用性。
...
<details>
  <summary>Details</summary>
Motivation: 传统3D扫描方法存在成本和可扩展性限制，特别是在机器人遍历数据中稀疏观察的地面区域容易出现视觉伪影。

Method: 采用3D高斯泼溅技术，引入地面感知高斯泼溅确保清洁可导航的地面平面，以及新颖的无网格可遍历性检查算法，通过直接分析渲染视图构建拓扑图。

Result: 系统能够从真实世界数据生成有效的大规模导航图。

Conclusion: NVSim框架成功克服了传统3D扫描的限制，能够从普通图像序列自动构建大规模可导航室内模拟器。
...
<details>
  <summary>Details</summary>
Motivation: 传统起重机轨迹规划方法难以直接处理复杂的物理和动态约束，如非线性摩擦和碰撞避免，这限制了起重机的运动速度和安全性。

Method: 利用微分平坦性框架，直接整合非线性摩擦和碰撞约束，仅在终点约束载荷摆动，实现激进运动。

Result: 对比仿真验证了方法的有效性，忽略干摩擦会导致执行器饱和和碰撞，摩擦建模是实现快速安全轨迹的基本要求。

Conclusion: 所提出的基于微分平坦性的轨迹生成方法能够有效处理复杂约束，是实现3D桥式起重机快速安全运动的关键技术。
...
<details>
  <summary>Details</summary>
Motivation: 传感器噪声协方差在实际应用中难以准确指定，受环境变化和前处理等因素影响，需要一种能平衡信息利用和计算效率的方法。

Method: 使用双层优化框架：下层使用不变扩展卡尔曼滤波进行轨迹估计，上层通过导数滤波器计算分析梯度来优化协方差参数。

Result: 在合成和真实数据集上的实验表明，该方法比现有基线方法具有更高的效率。

Conclusion: 该方法通过分解贝叶斯依赖关系实现并行计算，在噪声协方差估计方面表现出优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的团队定向问题(TOP)假设所有机器人合作追求单一目标，无法扩展到奖励稀缺环境中机器人竞争的场景。需要一种能够处理自利机器人、能量约束和随机转移的规划框架。

Method: 提出了随机奖励收集游戏(SPCG)框架，设计了两种算法：序数排名搜索(ORS)用于获取有效排名，虚构序数响应学习(FORL)用于学习对抗高排名对手的最佳响应策略。

Result: 实证评估表明：1) OR-conditioning的状态混叠使策略在大团队规模下更高效；2) FORL训练的策略在非均衡奖励分布下泛化能力更好；3) 学习策略达到了87%-95%的最优性。

Conclusion: SPCG框架成功扩展了TOP问题，能够有效处理自利机器人在竞争环境中的规划问题，提出的算法在可扩展性和泛化性方面表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 水下机器人的视觉惯性SLAM面临视觉退化和IMU运动激励不足的挑战，导致定位和建图精度受限。

Method: 利用立体相机直接深度估计，无需在IMU初始化时估计尺度；通过精确重力初始化，解耦俯仰和滚转角度，使用4自由度PnP问题求解姿态跟踪；提出偏差消除的4-DOF PnP估计器；动态运动时联合估计IMU协方差进行全6-DOF姿态优化。

Result: 在模拟和真实数据上的大量实验表明，GeVI-SLAM相比最先进方法实现了更高的精度和更好的稳定性。

Conclusion: GeVI-SLAM通过重力增强和4-DOF PnP方法，有效解决了水下SLAM中的视觉退化和IMU激励不足问题，显著提升了系统性能。
...
Method: 分层框架将巡检任务分为：(a)基于历史地图生成感兴趣区域的初始全局视图规划；(b)局部视图重规划以适应巡检场景的当前形态。

Result: 在真实地下矿井中使用四足机器人部署验证，该方法能够保持全局覆盖目标，同时实现局部自主对环境不确定性的鲁棒适应。

Conclusion: 所提出的分层框架能够在环境不确定性下完成巡检任务，通过全局规划和局部重规划的结合实现鲁棒的自主巡检。
...
Method: 采用最大后验概率估计框架，结合高斯过程运动先验进行高保真运动插值，交替进行GP运动状态更新和基于梯度的校准变量更新，并包含统计一致的序列初始化方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决机器人跳跃任务中奖励稀疏、动态行为探索困难的问题，实现精确控制和高性能跳跃，同时跨越Sim2Real差距。

Method: 使用课程强化学习框架，通过物理运动定律密集化稀疏奖励，采用参考状态初始化加速动态行为探索，不依赖参考轨迹，结合行走策略。

Result: 实验验证水平跳跃达1.25米（厘米级精度），垂直跳跃达1.0米，超越先前工作，成功跨越Sim2Real差距，并可扩展到全向跳跃。

Conclusion: 该方法有效解决了机器人跳跃控制的关键挑战，实现了高性能精确跳跃，展示了多功能动态运动能力，具有良好扩展性。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个仅依赖LiDAR、适用于大规模户外环境的定位系统，能够支持多种传感器模型并满足在线运行要求。

Method: 使用鸟瞰图投影聚焦感知地面区域，采用R2D2或SIFT进行关键点识别和BEV图像地图配准，将先验地图存储为2D栅格图像。

Result: 在SemanticKITTI和HeLiPR数据集上优于现有方法，多会话定位评估中平均轨迹误差低于50厘米，支持多种传感器模型，存储效率高（每平方公里仅需4MB）。

Conclusion: GroundLoc是一个高效、轻量级的LiDAR定位系统，在大规模户外环境中表现出色，满足实时运行要求且存储需求低。
...
<details>
  <summary>Details</summary>
Motivation: 随着智能驾驶功能在量产车中的集成度增加，确保其功能和鲁棒性面临更大挑战。相比传统道路测试，基于场景的虚拟测试在时间成本效率、可重复性和边缘案例探索方面具有显著优势。

Method: 使用Transformer增强的条件变分自编码器(CVAE-T)模型生成多智能体环岛交通场景，环岛场景具有高车辆动态和复杂布局特点。

Result: 模型能够准确重建原始场景并生成真实多样的合成场景，潜在空间分析显示部分解纠缠，多个潜在维度对场景属性（如车辆进入/退出时间和速度曲线）具有明显且可解释的影响。
...
<details>
  <summary>Details</summary>
Motivation: 解决截肢者使用动力大腿假肢时在复杂地形和障碍物导航方面的挑战，提高越障能力。

Method: 在健侧脚踝安装惯性传感器，使用遗传算法计算最优神经网络结构来预测大腿和膝关节角度，结合步态进展预测算法确定假肢膝关节电机的驱动角度。

Result: 当大腿角度数据添加的高斯噪声标准差小于1时，能有效消除噪声干扰，在150Hz下实现100%的步态相位估计准确率，大腿角度预测误差8.71%，膝关节角度预测误差6.78%。

Conclusion: 该方法能准确预测步态进展和关节角度，对动力大腿假肢的越障导航具有重要实用价值。
...
<details>
  <summary>Details</summary>
Motivation: 模仿学习策略在分布外场景中容易发生不可预测的故障，需要能够检测故障并自动恢复的弹性策略。

Method: 在模仿学习策略中嵌入OOD检测和识别功能，无需使用显式故障数据，并结合恢复启发式方法。

Result: 真实世界实验表明，Fare能够在两种不同策略架构中实现故障恢复，在复杂环境中实现鲁棒的长距离导航。

Conclusion: Fare框架成功构建了具有故障恢复能力的模仿学习策略，显著提升了视觉导航的鲁棒性。
...
Method: 建立分析框架，通过基础机器人-障碍物实验场景验证机器人行为，比较三种代表性物体感知控制器，使用源自设计考虑的指标进行评估。
...
Method: 提出了将物理计算嵌入软体机器人的框架，详细介绍了三种具体方法：模拟振荡器、物理储备池计算和物理算法计算的工作原理。

Result: 这些嵌入式物理计算机使软体机器人能够执行复杂行为，包括带避障的协调运动、有效载荷重量和方向分类，以及基于逻辑规则的可编程操作。

Conclusion: 本文详细阐述了这些物理计算方法的工作原理，综述了当前最新进展，并提出了未来发展的展望。
