...
Method: 使用局部POE模型在多个配置下进行识别，通过傅里叶基函数插值构建全局模型，参数化基于肩部和肘部关节角度。

Result: 在两个6自由度工业机器人上验证，最大定位误差减少超过50%，达到冷喷涂制造所需的亚毫米精度，训练效率显著高于神经网络方法。

Conclusion: 该配置依赖运动学校准框架能有效提高机器人精度，特别适用于存在较大配置依赖差异的机器人，双机器人协作任务验证了其实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 解决非抓取式操作中未知物理属性和复杂接触交互的挑战，扩展CI-MPC方法在多样化物体几何和多物体场景中的应用能力。

Method: 开发了Consensus Complementarity Control Plus (C3+)算法，整合了物体扫描、网格重建和硬件执行的完整流程，相比前代C3显著提高了求解速度。

Result: 在33个物体上实现98%的成功率，单物体任务平均耗时约0.5分钟，2-4物体任务分别耗时1.6、3.2和5.3分钟，支持实时多物体推动操作。

Conclusion: C3+算法成功扩展了CI-MPC的应用范围，能够高效处理复杂多物体接触场景，为机器人非抓取操作提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统无人机竞速方法虽然速度快但受限于预设轨道布局，缺乏现实世界的通用性。需要开发能够在任意起止状态间进行最小时间飞行的通用策略。

Method: 使用强化学习框架，以点质量模型轨迹作为代理奖励来近似最优飞行目标，并采用课程学习来高效扩展训练过程和实现泛化。

Result: 仿真实验验证了方法的有效性，与非线性模型预测控制跟踪PMM轨迹的方法相比表现良好。真实世界实验证明学习策略在室外环境中的鲁棒性，能够在小型ARM单板计算机上运行。

Conclusion: 该方法成功实现了通用化的最小时间飞行策略，能够在任意起止状态间进行敏捷飞行，并在真实环境中展示了良好的泛化能力和计算效率。
...
<details>
  <summary>Details</summary>
Motivation: Stewart平台并联运动学机器人在精密应用中需要微纳级运动控制，必须通过校准达到比应用要求更高的精度。传统正向运动学校准过于复杂，而逆运动学校准更为简便。

Method: 通过文献综述分析三种主要校准方法：外部仪器校准、约束校准和自校准，重点关注逆运动学为基础的校准技术。

Result: 研究发现研究人员主要关注平台位置和方向精度的提升，考虑单一或多重误差源，主要是结构误差，部分考虑环境因素，但都在无负载条件下进行校准。
...
<details>
  <summary>Details</summary>
Motivation: 商用和消费级3D打印机多为闭源平台，缺乏研究所需的灵活性和参数控制能力，需要开发专门用于聚合物FDM工艺研究的测试平台。

Method: 设计并制造了具有闭环位置反馈、热端和床温控制、环境温湿度监测的研究平台，采用新型旋转加热床机制配合摄像头进行原位摄影测量。

Result: 平台实现了对FDM工艺参数的精确控制和监测，通过旋转加热床和摄影测量技术能够记录打印过程中的几何变化。

Conclusion: 该开放式研究平台为聚合物FDM工艺研究提供了可重复实验的基础，旋转加热床设计有效支持了摄影测量重建功能。
...
Method: 使用3网格（位置/内容/时间）表示编码机器人运动，采用约束掩码解码确保相邻网格移动和工作空间边界，同时考虑任务图和动作顺序。

Result: 在53,755条轨迹数据集上训练，达到89.44%步骤准确率、93.32%精确率、89.44%召回率和90.40% F1分数，99.99%路径合法。在真实机器人测试中，端到端任务成功率达86.7%。

Conclusion: 路径结构表示使Transformer能够生成准确、可靠且可解释的机器人轨迹，为通用操作和仿真到现实迁移提供实用基础。
...
<details>
  <summary>Details</summary>
Motivation: 解决磁吸附爬墙机器人中磁脚附着力不确定性的问题，包括部分接触、气隙敏感性和概率性附着失败，以实现可靠的垂直表面爬行。

Method: 采用三阶段课程学习：1) 在平坦地面学习爬行步态；2) 逐渐旋转重力矢量并激活吸附模型；3) 注入随机吸附失败以训练滑落恢复能力。

Result: 学习到的策略在模拟中实现了高成功率、强吸附保持力和快速脱落恢复能力，在硬件实验中展示了在钢表面的稳健垂直爬行。

Conclusion: 将课程学习与真实吸附建模相结合，为磁吸附爬墙机器人在复杂环境中提供了弹性的仿真到现实框架。
...
<details>
  <summary>Details</summary>
Motivation: 机器人在视觉受限环境（如遮挡、光线差）中操作时，需要依赖接触反馈来感知环境，并利用结构先验来预测未探索区域，提高操作效率和鲁棒性。

Method: 框架包含三个紧密耦合的模块：(1) 基于关节力矩传感和接触粒子滤波的接触检测定位模块；(2) 利用接触历史构建部分占据地图并通过学习预测器外推的占据估计模块；(3) 考虑噪声的规划模块，计算避免碰撞的高效路径。

Result: 在模拟和真实UR10e机械臂上测试了两个家庭任务：在厨房水槽管道中操作阀门和在杂乱架子上取物。结果显示框架可靠完成任务，相比基线方法任务完成时间减少高达2倍，消融实验确认了各模块的贡献。

Conclusion: 该框架成功整合了接触反馈和结构先验，实现了在视觉受限环境中的鲁棒操作，显著提高了任务效率，为机器人在复杂环境中的操作提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决多卫星（超过三个）编队控制中的基本挑战：非完整约束、欠驱动、可扩展性和计算成本，这些挑战随着卫星数量增加和高非线性而出现。

Method: 采用学习辅助的时间积分电流控制方法，设计两轴线圈和基于气浮平台的地面实验装置，复制轨道动力学，并引入NODA-MMH算法进行基于模型的功率最优群控制。

Result: 实验验证了学习辅助时间积分电流控制的两个关键方面：增强平均系统动力学的可控性（具有理论保证的误差界）和分散式电流管理。

Conclusion: 该研究为基于磁力驱动的卫星群长期编队维持问题提供了有效的解决方案，并补充了相关教程论文的内容。
...
<details>
  <summary>Details</summary>
Motivation: 解决机器人在连续操作中利用先前观察信息进行精确定位的问题，优化导航效率。

Method: 使用循环变换器维护隐式潜在记忆，压缩传感器历史为紧凑表示，无需显式存储观测历史。

Result: 模型能有效维持场景表示，导航到在episode开始前观察到的目标位置，计算效率优于传统基于注意力历史的变换器。

Conclusion: Kinaema通过隐式记忆机制实现了高效的大场景定位和导航，为连续机器人操作提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 人类依赖记忆完成任务，但现有机器人策略缺乏此能力。直接使用长观察历史计算成本高且脆弱，随机采样历史会导致信息冗余或缺失

Method: 分层策略框架：高层策略训练选择跟踪历史相关关键帧，结合最新帧生成文本指令；底层策略执行指令。基于Qwen2.5-VL-7B-Instruct和π0.5模型微调，使用带最小语言标注的演示数据

Result: 在三个需要分钟级记忆的真实世界长程机器人操作任务中，MemER方法优于先前方法

Conclusion: 提出的分层框架兼容现有VLA模型，能有效处理长程依赖关系，为机器人策略赋予记忆能力
...
<details>
  <summary>Details</summary>
Motivation: 解决端到端自动驾驶停车方法在领域偏移（如天气和光照变化）下的鲁棒性问题，不依赖额外数据

Method: 集成视觉基础模型和基于扩散的运动规划，在CARLA中训练并在对抗性设置下进行零样本迁移

Result: 在所有测试的分布外场景中保持90%以上的停车成功率，在3D高斯泼溅重建的真实停车场中显示出有前景的仿真到真实迁移能力

Conclusion: DDP方法通过网络架构和算法设计显著提升了跨领域性能，为自动驾驶停车提供了鲁棒的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 模块化可重构机器人适合特定任务的空间操作，但形态组合爆炸阻碍了统一控制。需要解决模块化机器人控制的可扩展性和泛化性问题。

Method: 采用去中心化强化学习方案：轮式模块使用SAC算法进行运动控制，7自由度肢体模块使用PPO算法进行转向和操作控制，实现零样本泛化到未见配置。

Result: 转向策略实现期望角度与诱导角度之间的平均绝对误差为3.63度；操作策略在目标偏移标准上达到84.6%成功率；轮式策略相比基线平均电机扭矩降低95.4%，同时保持99.6%成功率。月球模拟现场测试验证了零样本集成能力。
...
Method: 利用NDF隐式表示目标接触几何，训练基于神经描述符的深度强化学习策略，使用触觉反馈来优化抓取姿态。

Result: 在模拟和真实世界操作任务(如插孔和瓶盖开启)中的零样本迁移验证显示，NeuralTouch显著提高了抓取精度和鲁棒性。

Conclusion: NeuralTouch为精确、接触丰富的机器人操作提供了一个通用框架，无需额外微调即可实现零样本迁移。
...
<details>
  <summary>Details</summary>
Motivation: 机器人操作系统受益于互补的传感模式，但当前点云方法难以捕捉细粒度细节，而RGB方法缺乏几何感知能力，限制了其精度和泛化能力。

Method: 引入PointMapPolicy方法，在不进行下采样的结构化点网格上调节扩散策略，使用xLSTM作为骨干网络，有效融合点图和RGB数据。

Result: 在RoboCasa和CALVIN基准测试以及真实机器人评估中，该方法在多样化操作任务中实现了最先进的性能。

Conclusion: PointMapPolicy方法通过结构化点网格和RGB数据的有效融合，显著提升了机器人操作系统的感知能力和任务执行性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决水下机器人手臂遥操作中缺乏有效反馈机制的问题，通过集成视觉和触觉反馈来提升操作的稳定性和准确性。

Method: 开发了MR-UBi系统，使用混合现实头戴显示器显示颜色和长度编码的扭矩条作为反应扭矩指示器，结合双边控制实现视觉和触觉反馈的集成。
...
<details>
  <summary>Details</summary>
Motivation: 当移动零件比移动工具更容易时，需要一种能够处理固定工具中心点同时考虑零件加工路径的轨迹规划方法。

Method: 使用B样条表示机器人路径，确保路径连续性和平滑轨迹，在计算机器人轨迹时考虑规定的方向和TCP的给定速度。

Result: 该方法在工业机器人移动任意定义零件的真实系统上进行了验证。

Conclusion: 基于B样条的轨迹规划方法能够有效处理固定工具中心点情况下的平滑轨迹生成，适用于零件移动场景。
...
<details>
  <summary>Details</summary>
Motivation: 在GNSS拒绝环境中，单个机器人携带所有传感器会增加尺寸、重量和功耗需求。将传感器分布在多个机器人上虽增强部署性，但带来了异步多模态数据融合的挑战。

Method: 使用因子图公式松散耦合地融合异步VIO、LIO和3D机器人间检测。提出基于插值的因子处理非同步测量，基于近似扫描匹配Hessian评估LIO退化，并提出基于VIO输出间Wasserstein距离的里程计数据加权方法。

Result: 在真实世界数据上的广泛评估显示，该方法在各种传感器退化情况下显著提高了定位精度。

Conclusion: 所提出的自适应多模态多机器人协同定位方法能够有效应对传感器退化问题，在GNSS拒绝环境中提供显著改进的定位性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决在未知动力学（如有效载荷不确定性）下机器人操作任务的挑战，需要主动探索和在线参数适应来实现准确的基于模型的控制。

Method: 将问题构建为双控制问题，预定义反馈策略结构包含显式适应机制。提出两种参考轨迹生成方法：1）在鲁棒最优控制中嵌入参数不确定性；2）最小化最优性损失来衡量参数相关信息对任务性能的敏感性。

Result: 两种方法都自然地考虑了Fisher信息，同时追求最优任务执行。在拾取-放置操作任务中表现出更快速、更准确的任务性能和系统辨识，同时确保稳定高效的控制。

Conclusion: 在设计参考轨迹时考虑控制因素能够实现更快的任务执行、更准确的系统辨识，并确保稳定高效的控制性能。
...
<details>
  <summary>Details</summary>
Motivation: 工业机器人执行拾放任务时通常长时间运行，能量消耗最小化具有重要意义。串联弹性驱动器能够利用弹性元件的振荡特性来降低能耗。

Method: 推导了SEA驱动并联机器人的动力学模型，建立了能量最小化的最优控制问题，同时优化操作轨迹和SEA刚度。

Result: 在两个并联机器人应用上测试了该方法，结果证实了该方法的有效性。

Conclusion: 通过激发本征运动并利用振荡特性，可以显著降低SEA驱动并联机器人在拾放任务中的能量消耗。
...
<details>
  <summary>Details</summary>
Motivation: 传统时间最优路径跟踪方法在最小化旅行时间时，在零路径速度处会产生奇异性，导致平滑轨迹生成困难且计算量大。

Method: 采用最大化路径速度的方法，将基础问题离散化后，优化变量呈线性关系，从而实现数值高效计算。

Result: 该方法能够规划平滑轨迹，同时保持较低的计算复杂度。

Conclusion: 提出的最大化路径速度方法有效解决了时间最优路径跟踪中的奇异性问题，实现了高效平滑的轨迹规划。
...
<details>
  <summary>Details</summary>
Motivation: 由于法律约束和机构所有权问题，真实结构坍塌事故中的空隙空间内部组成数据难以公开获取，而训练用的工程废墟堆也不愿公开其专有训练场地信息。

Method: 使用Unity开发多操作系统支持的模拟器，采用基于物理的方法构建随机废墟堆，允许在保留绝对真实情况的同时快速迭代模拟世界。

Result: 应用最先进的结构运动算法，展示了在模拟空隙空间中具有挑战性的视觉条件下感知性能如何下降。

Conclusion: RubbleSim为研究社区提供了一个可访问的解决方案，用于研究废墟环境中的机器人探索和感知问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有物体导航方法依赖静态轨迹和固定物体类别训练，忽略了现实世界需要持续适应演化场景的需求，因此需要研究持续物体导航问题。

Method: C-Nav框架包含：1）双路径抗遗忘机制：特征蒸馏确保表示一致性，特征重放确保策略一致性；2）自适应采样策略：选择多样化和信息丰富的经验，减少冗余和内存开销。

Result: 在多种模型架构上的广泛实验表明，C-Nav始终优于现有方法，即使与保留完整轨迹的基线相比也能实现更优性能，同时显著降低内存需求。

Conclusion: C-Nav为解决持续物体导航问题提供了有效解决方案，通过创新的抗遗忘机制和采样策略实现了性能与内存效率的平衡。
...
<details>
  <summary>Details</summary>
Motivation: 模型无关强化学习通常收敛到单一步态导致性能次优，而模型预测控制缺乏环境适应性。需要结合两者的优势来实现自适应和最优的四足运动策略。

Method: 使用MPPI算法与Dreamer模块结合，在每个时间步联合优化动作和步态变量。采用学习到的Dreamer奖励函数促进速度跟踪、能效、稳定性和平滑过渡，并惩罚剧烈步态变化。结合学习到的价值函数作为终端奖励，扩展到无限时域规划。

Result: 在Unitree Go1仿真中验证，在不同目标速度下平均能耗降低高达36.48%，同时保持准确跟踪和自适应、任务适当的步态。

Conclusion: 该框架成功实现了实时步态自适应，在连续步态空间中产生自适应和最优的四足运动策略，显著提高能效同时保持运动性能。
...
Result: 使用FieldGen训练的策略比基于遥操作的基线方法获得更高的成功率和改进的稳定性，同时显著减少了长期真实世界数据收集的人力投入。
...
<details>
  <summary>Details</summary>
Motivation: 机器学习在机器人领域的应用日益广泛，但仿真环境与现实世界之间存在显著差异（现实差距），这阻碍了从仿真到现实的成功迁移。理解现实差距的根本原因和解决方案是当前机器人学的重要挑战。

Method: 通过综述现有文献，分析现实差距的成因，总结包括领域随机化、现实到仿真迁移、状态和动作抽象、仿真-现实协同训练等解决方案，并评估相关指标。

Result: 研究发现多种技术可以有效缩小现实差距，促进仿真到现实的迁移，但在不同机器人平台（如运动、导航、操作）上的效果各异。

Conclusion: 虽然已有技术取得了一定进展，但现实差距问题仍然存在，需要更深入理解其根本原因并开发更有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个能够实现逼真渲染的机器人操作模拟器，支持从真实机器人数据学习策略的闭环开发，减少对真实机器人的依赖。

Method: 提出GSDF（高斯场景描述文件）资产格式，将高斯网格表示与机器人URDF和其他对象结合，结合物理引擎构建逼真的模拟环境。

Result: 构建了包含3种机器人配置和40多个对象的GSDF数据库，展示了多种应用：零样本sim2real像素到动作策略学习、自动化DAgger数据收集、可复现的基准测试等。

Conclusion: GSWorld为机器人操作提供了一个强大且逼真的模拟平台，支持多种sim2real应用，显著减少了真实机器人实验的需求。
...
<details>
  <summary>Details</summary>
Motivation: 解决机器人导航中学习通用策略与适应特定机器人物理约束之间的基本挑战，使单一高层规划器能部署到物理特性不同的轮式和腿式机器人上。

Method: 设计分层架构：通用规划器从开放世界数据学习语义规划，专业感知模型在低成本模拟中学习机器人物理能力；通过图像空间路径提案和感知评估接口实现解耦。

Result: 在真实世界实验中，VAMOS在室内和复杂室外导航中比最先进的基于模型和端到端学习方法获得更高成功率；支持轮式和腿式机器人的跨具体化导航，通过拒绝物理不可行计划使单机器人可靠性提高3倍。

Conclusion: 分层设计是跨具体化导航的关键，专业感知模型对于具体化基础至关重要，使单一高层规划器能够部署到物理特性不同的机器人上，显著提升导航可靠性。
