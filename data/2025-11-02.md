...
TL;DR: D2C框架使用LLM代理进行结构化辩论，共同优化机器人形态设计和奖励函数，通过迭代辩论产生多样化且高效的机器人设计


<details>
  <summary>Details</summary>
Motivation: 解决机器人形态与控制协同设计的长期挑战，克服设计空间庞大和身体-行为紧密耦合的问题

Method: 设计代理提出形态修改，控制代理设计相应奖励函数，多元评审团在仿真中评估并提供反馈，通过多轮辩论迭代优化

Result: 在四足机器人运动基准测试中，D2C发现的机器人设计比默认设计行进距离远73%，且无需显式多样性目标就能产生多样化形态

Conclusion: 基于LLM的多代理辩论结合物理基础反馈，是自动化机器人设计的有前景新范式
...
TL;DR: 提出了一种基于泊松方程和拉普拉斯引导场的风险感知安全滤波器，通过数学方法编码环境理解和风险级别，保证机器人安全导航的同时优先避开高风险障碍物


<details>
  <summary>Details</summary>
Motivation: 现实世界中的机器人系统需要语义理解环境以确定安全行动，需要开发风险感知的安全表示方法

Method: 采用两步法：1) 通过泊松方程的狄利克雷问题生成安全函数；2) 通过拉普拉斯方程的狄利克雷问题合成安全引导场，结合两者构建风险感知安全滤波器

Result: 在仿真中验证了该方法，能够根据障碍物风险级别生成风险感知的安全行为

Conclusion: 该方法能够将先验的障碍物风险理解直接整合到安全滤波器中，产生风险感知的安全行为
...
TL;DR: 开发了一种针对柔性电阻式触觉传感器的曲率感知校准模型，通过神经网络预测局部曲率，显著提高了在曲面上的力测量精度和一致性。


<details>
  <summary>Details</summary>
Motivation: 现有柔性触觉传感器主要在平面上校准，但在实际应用中需要贴合曲面，导致精度和一致性下降，限制了其在机器人抓取、假肢等领域的可靠性。

Method: 为广泛使用的电阻式触觉传感器设计校准模型，训练多层感知器神经网络从无负载时的基线传感器输出预测局部曲率，实现一维曲面上的精确力估计。

Result: 在5种不同曲率的日常物体上验证，力范围为2-8N，曲率感知校准在所有表面上保持一致的力精度（R2=0.91），而平面校准会随曲率增加而低估力。

Conclusion: 曲率感知建模显著提高了柔性触觉传感器的精度、一致性和可靠性，使其能够在实际应用中提供可靠的性能。
...
TL;DR: 提出了一种新的基于欧拉轴角的姿态控制方法，相比传统的四元数控制方法，能够保证闭环系统具有唯一的平衡点，并在大角度误差时提供更好的比例控制效果。


<details>
  <summary>Details</summary>
Motivation: 传统四元数姿态控制方法存在两个问题：不保证闭环系统有唯一的平衡姿态误差四元数；当绕欧拉轴的旋转误差超过π弧度时，比例控制效果会随着系统状态远离稳定平衡点而减弱。

Method: 开发了一种新的姿态控制律，更有效地利用姿态误差的欧拉轴角信息，保证唯一的闭环平衡姿态误差四元数，并提供比例控制力使用的更大灵活性。通过构建严格的Lyapunov函数证明了闭环旋转系统的唯一平衡点是均匀渐近稳定的。

Result: 通过数值模拟和数十次小型四旋翼飞行器的实时翻滚恢复机动测试，验证了所提方法的性能和功能。与高性能四元数控制器相比，在稳定时间方面实现了更优越的飞行性能。

Conclusion: 基于欧拉轴角的姿态控制方法在保证唯一闭环平衡点的同时，能够提供比传统四元数控制方法更优越的控制性能，特别是在大角度误差情况下。
...
TL;DR: DARTS是一个基于无人机和AI的实时交通事件检测系统，通过无人机的高机动性和热成像技术实现自适应监控，在真实道路测试中比传统方法提前12分钟检测到追尾事故。


<details>
  <summary>Details</summary>
Motivation: 传统交通事件检测方法存在检测与验证分离、灵活性有限、需要密集基础设施等问题，限制了在变化的事件热点区域的适应性和可扩展性。

Method: DARTS整合了无人机的高机动性和空中视角进行自适应监控，使用热成像技术提升低能见度性能和隐私保护，采用轻量级深度学习框架进行实时车辆轨迹提取和事件检测。

Result: 在自收集数据集上达到99%的检测准确率，在佛罗里达州75号州际公路实地测试中，比当地交通管理中心提前12分钟检测并验证了追尾事故，并能监控事件引发的拥堵传播。

Conclusion: DARTS展示了更灵活和集成的实时交通事件检测系统的潜力，具有显著提高现代交通管理运营效率和响应能力的意义，其灵活部署架构表明在偏远地区和资源受限环境中具有可扩展性和成本效益。
...
TL;DR: 提出了一种利用协作微型无人机群和康普顿相机定位放射性材料的新方法，通过实时融合稀疏测量数据来估计辐射源位置，并实现动态反馈控制无人机运动。


<details>
  <summary>Details</summary>
Motivation: 传统辐射检测方法存在灵敏度不足、设备笨重等问题，需要开发轻量化、高灵敏度的辐射检测系统，利用协作无人机群实现快速辐射源定位和追踪。

Method: 采用重量仅40g的单探测器康普顿相机作为微型辐射探测器，通过融合多个康普顿相机的测量数据实时估计辐射源位置，并利用动态反馈控制无人机群的运动以最大化信息获取。

Result: 实现了在极端稀疏测量条件下实时定位辐射源，能够稳定控制无人机群紧密协作，快速定位并追踪移动辐射源。

Conclusion: 该方法展示了轻量化康普顿相机与协作无人机群结合在辐射检测领域的巨大潜力，为快速响应辐射事件提供了有效解决方案。
...
TL;DR: 开发了一种能够在模拟和现实中可靠导航并超越对手的新型赛车和超车智能体，在F1Tenth车辆上部署并取得了87%的超车成功率。


<details>
  <summary>Details</summary>
Motivation: 虽然自主赛车在计时赛场景中取得了显著进展，但在轮对轮比赛和超车方面仍然严重受限，特别是在现实驾驶场景中，现有算法难以安全可靠地完成超车操作。

Method: 在F1Tenth竞赛平台上开发了新型赛车和超车智能体，通过在模拟和现实中训练对抗对手来学习可靠导航和超车行为。

Result: 该智能体在真实世界中与运行不同竞争算法的对手比赛，训练对抗对手的智能体实现了87%的超车成功率，而仅训练赛车的智能体只有56%的成功率。

Conclusion: 对抗对手的训练能够实现有意识的超车行为，显著提高了超车成功率，证明了该方法在自主轮对轮赛车中的有效性。
...
TL;DR: 提出了一种基于图神经网络(GNN)的强化学习框架，用于控制张拉整体机器人的运动，该方法在真实机器人上实现了从仿真到硬件的直接迁移。


<details>
  <summary>Details</summary>
Motivation: 张拉整体机器人具有高韧性和可部署性，但由于其欠驱动和高度耦合的动力学特性，在运动控制方面面临重大挑战。

Method: 将图神经网络(GNN)集成到Soft Actor-Critic(SAC)算法中，通过将机器人的物理拓扑表示为图来捕捉组件间的耦合关系。

Result: 在3杆张拉整体机器人上验证了三种运动基元，显示出优越的样本效率、对噪声和刚度变化的鲁棒性，以及改进的轨迹精度。学习到的策略无需微调即可从仿真直接迁移到硬件。
...
TL;DR: 该研究以Moxie社交机器人为案例，探讨当儿童与机器人建立情感纽带后服务突然终止时的责任归属问题。研究发现责任被视为机器人公司、父母、开发者和政府共同承担，但责任分配因政治意识形态和是否为人父母而异。


<details>
  <summary>Details</summary>
Motivation: 社交机器人如Moxie旨在与儿童建立强烈情感联系，但其突然停服会对儿童造成显著困扰和痛苦。当这些服务终止时，儿童情感纽带被破坏的责任归属问题引发复杂思考。

Method: 通过对72名美国参与者进行定性调查，以Moxie停服为案例研究，分析不同群体对责任归属的看法。

Result: 研究发现责任被视为共享责任，涉及机器人公司、父母、开发者和政府。责任归因因政治意识形态和父母身份而异。参与者对机器人服务是否应继续的看法高度两极分化。

Conclusion: 本研究提出了一个基于实证的共享责任框架，用于保护儿童-机器人陪伴关系，详细说明了责任如何分配和争议，为减轻机器人停服带来的情感伤害提供了具体的设计和政策建议。
...
TL;DR: 研究探讨了机器人拟人化程度对人类保护性反应的影响，发现中等拟人化机器人引发最强的愤怒反应，道德推理随拟人化程度增加从技术评估转向对施虐者品德的谴责。


<details>
  <summary>Details</summary>
Motivation: 随着具有拟人特征的机器人日益普及，需要了解不同拟人化程度如何影响人类对机器人虐待的道德反应，将CASA理论和恐怖谷理论扩展到道德领域。

Method: 邀请201名参与者观看低、中、高三种拟人化程度机器人被虐待的视频，通过自我报告问卷、自动面部表情分析的生理数据和定性反思进行三角测量分析。

Result: 保护性反应呈非线性，中等拟人化机器人引发最强的生理愤怒表达；自我报告的愤怒和愧疚在中等和高拟人化机器人中显著更高；道德推理随拟人化程度增加从财产损害评估转向施虐者品德谴责。
...
TL;DR: 本文探讨了具身智能作为微型机器人设计原则，强调结构与功能协同设计的重要性，通过多个机器人平台展示了从物理结构和环境交互中涌现智能行为的方法。


<details>
  <summary>Details</summary>
Motivation: 传统机器人架构将感知、计算和执行解耦，而具身智能通过将身体形态、材料特性和环境交互整合到设计过程中，为微型机器人提供更智能、可扩展和鲁棒的解决方案。

Method: 采用协同设计方法，同时开发物理结构和行为功能，将反馈回路、决策逻辑、感知机制和智能驱动策略嵌入到机器人系统的物理属性中。

Result: 开发了Bee++、RoBeetle、SMALLBug等多个微型机器人平台，这些平台通过结构动力学和物理交互展现出智能行为，验证了具身智能设计的有效性。

Conclusion: 协同设计不仅是约束条件下的经验优化方法，更是实现具身智能的关键推动者，为毫米到厘米尺度的机器人提供了可扩展且鲁棒的替代方案。
...
TL;DR: 提出了一种基于混合状态树的动力学TAMP框架，将符号和数值状态统一表示，通过视觉语言模型引导搜索，显著提高了复杂任务的成功率和规划效率。


<details>
  <summary>Details</summary>
Motivation: 现有TAMP方法在长时域问题中由于过度运动采样而成本高昂，LLMs虽然提供常识先验但缺乏3D空间推理能力，无法保证几何或动力学可行性。
...
Conclusion: 该框架通过VLM引导有效解决了TAMP中的运动采样效率问题，在复杂任务中表现出显著优势。
...
TL;DR: 提出自适应轨迹优化算法，通过分段保守碰撞检测和位姿修正，解决移动机器人在狭窄通道中的轨迹规划问题，显著提高成功率和规划速度。


<details>
  <summary>Details</summary>
Motivation: 移动机器人在杂乱环境中的轨迹规划面临挑战，特别是在狭窄通道中，传统方法经常失败或生成次优路径。

Method: 采用两阶段方法：1) 分段保守碰撞检测，递归细分风险轨迹段消除碰撞风险；2) 基于穿透方向和线搜索的位姿修正，确保每个位姿无碰撞且远离障碍物。

Result: 仿真结果显示，相比最先进方法，成功率提高1.69倍，规划时间加快3.79倍。真实世界实验验证机器人能安全通过狭窄通道并保持快速规划性能。

Conclusion: 自适应轨迹优化算法有效解决了狭窄通道中的轨迹规划问题，显著提升了成功率和效率。
...
TL;DR: 提出了一种结合CNN和Vision Transformer的新方法，用于在动态障碍物存在时的3D地图自定位，相比现有方法精度提升显著。


<details>
  <summary>Details</summary>
Motivation: 使用廉价单目相机在3D地图上进行自定位是实现自动驾驶的关键，但现有基于CNN的方法在动态障碍物存在时效果不佳。

Method: 将擅长提取局部特征的CNN与擅长提取全局特征的Vision Transformer相结合，利用全局特征处理动态障碍物问题。

Result: 在CG数据集上，动态障碍物存在时的精度提升率比无动态障碍物时高1.5倍；在公开数据集上定位误差比SOTA方法减小20.1%；机器人平均定位误差为7.51cm。

Conclusion: 提出的CNN与Vision Transformer结合方法能有效处理动态障碍物，显著提升自定位精度，优于现有最佳方法。
...
TL;DR: PHUMA是一个基于物理约束的人形机器人运动数据集，通过大规模人类视频和物理约束重定向技术解决现有方法中的物理伪影问题，在运动模仿任务中表现优于Humanoid-X和AMASS。


<details>
  <summary>Details</summary>
Motivation: 现有运动模仿方法依赖稀缺昂贵的动作捕捉数据，而基于互联网视频的方法存在物理伪影问题，限制了可扩展性和多样性。

Method: 利用大规模人类视频，通过数据筛选和物理约束重定向技术，强制执行关节限制、确保地面接触、消除脚部滑动，生成物理可靠的大规模运动数据。

Result: 在未见运动模仿和路径跟随任务中，PHUMA训练的策略显著优于Humanoid-X和AMASS，在多样化运动模仿方面取得重大进展。

Conclusion: PHUMA通过物理约束的数据处理方法，成功解决了基于视频的运动数据中的物理伪影问题，为人形机器人运动模仿提供了高质量、大规模的数据集。
...
TL;DR: 提出了Thor人形机器人框架，通过力自适应躯干倾斜奖励函数和解耦的强化学习架构，显著提升了人形机器人在接触丰富环境中的力交互能力。


<details>
  <summary>Details</summary>
Motivation: 解决人形机器人在接触丰富环境中保持全身稳定性并产生类人自适应响应的挑战，提升其在服务、工业和救援应用中的实用性。

Method: 基于机器人受力分析设计FAT2奖励函数，采用解耦的强化学习架构分别控制上半身、腰部和下半身，各组件共享全局观测并联合更新参数。

Result: 在Unitree G1机器人上部署，向后移动时峰值拉力达167.7N（约48%体重），向前移动时145.5N，相比最佳基线分别提升68.9%和74.7%；能拉动130N负载架和单手打开60N防火门。
...
TL;DR: AgriGS-SLAM是一个用于果园环境的视觉-LiDAR SLAM框架，结合直接LiDAR里程计和多相机3D高斯泼溅渲染，能够在不同季节条件下实现实时3D场景理解。
...
Method: 耦合直接LiDAR里程计和闭环检测与多相机3D高斯泼溅渲染，通过批量光栅化恢复遮挡下的果园结构，使用统一梯度驱动的地图生命周期管理来保留细节并限制内存使用。

Result: 在苹果和梨园的不同季节（休眠期、花期、收获期）部署测试，相比现有3DGS-SLAM基线方法，AgriGS-SLAM提供了更清晰、更稳定的重建效果和更平稳的轨迹，同时保持实时性能。
...
TL;DR: 本文提出了基于共形几何代数的多臂机器人系统协作任务空间理论框架，通过几何基元的相似变换将复杂系统抽象为类似单臂系统，并推导了相应的雅可比矩阵，实现了经典操作空间控制技术的集成应用。


<details>
  <summary>Details</summary>
Motivation: 人类环境中的许多任务需要多个运动链的协作行为，如搬运大型物体或灵巧操作。由于这些复杂系统通常具有高自由度，协调其运动建模非常困难。

Method: 基于共形几何代数定义协作几何基元，通过相似变换抽象复杂机器人系统，推导解析和几何雅可比矩阵，集成到经典操作空间控制技术中。

Result: 在双手机器人、仿人机器人和多指手的实验中验证了该方法，包括达到期望几何基元的最优控制实验和使用微分运动学控制的遥操作实验。

Conclusion: 该工作为协作操作控制框架奠定了理论基础，几何基元自然地将零空间结构嵌入控制器，可用于引入次要控制目标，为未来应用提供了方向。
...
TL;DR: 本研究使用强化学习实现了林业集材机从抓取到完整装载木材过程的自动化，在模拟环境中训练智能体完成定位、抓取、运输和放置木材的全流程操作。


<details>
  <summary>Details</summary>
Motivation: 林业集材机操作对驾驶员来说是身心俱疲的挑战，部分自动化可以减轻操作员压力。研究旨在扩展之前仅关注抓取的研究，实现完整的木材装载过程自动化。

Method: 在NVIDIA Isaac Gym中开发了拖车式林业集材机模拟模型和虚拟环境，采用强化学习代理和课程学习方法进行训练。

Result: 训练出的最佳智能体能够从随机位置抓取木材并运输到车厢，成功率达到了94%。

Conclusion: 该研究是强化学习在林业集材机自动化应用中的重要进展，为未来实际应用奠定了基础。
...
Method: Hi-ORS通过在线微调期间过滤负奖励样本来稳定价值估计，并采用奖励加权的监督训练目标提供密集的中间步骤监督。

Result: 在三个真实世界任务和两种体现中，Hi-ORS在1.5小时真实世界训练后显著优于RL和IL基线，在有效性和效率方面都有大幅提升。

Conclusion: 微调后的策略表现出强大的测试时可扩展性，能够可靠执行复杂的错误恢复行为以获得更好的性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有方法依赖有限或个体记忆的局限性，无法实现长期学习、异构团队扩展或故障恢复，需要统一的内存表示。

Method: 引入STEM记忆，集成空间场景几何、时间事件历史和具身配置文件，采用脑-小脑框架，高层脑模型进行全局规划，低层控制器本地执行。
...
<details>
  <summary>Details</summary>
Motivation: 传统机器人对自身运动学理解有限，局限于预设任务，无法高效利用工具。需要解决工具使用的四个关键要素：理解预期结果、选择合适工具、确定最佳工具方向、执行精确操作。

Method: 通过扩展机器人逆运动学求解器，使其能够学习使用不同长度工具的顺序动作。将仿真学习的动作轨迹与工具集成，实现从仿真到现实世界的技能迁移。

Result: 扩展逆运动学求解器误差小于1cm；训练策略在仿真中平均误差为8cm；使用两种不同长度工具时性能几乎无差别。

Conclusion: 该研究在工具使用的四个基本方面取得进展，使机器人能够掌握跨不同任务的复杂工具操作技能。
...
TL;DR: FLYINGTRUST是一个用于四旋翼飞行器视觉导航算法的高保真可配置基准测试框架，通过评估平台动力学和场景结构对导航鲁棒性的联合影响，支持系统化早期评估。


<details>
  <summary>Details</summary>
Motivation: 四旋翼飞行器的视觉导航算法在不同车辆平台和场景几何之间迁移时性能差异很大，增加了现场部署的成本和风险，需要系统化的早期评估方法。

Method: FLYINGTRUST使用最大推重比和轴最大角加速度两个物理可解释指标建模车辆能力，结合多样化场景库和异构平台集，采用标准化评估协议和复合评分方法。

Result: 比较代表性优化基和基于学习的导航方法发现，导航成功率可预测地依赖于平台能力和场景几何，不同算法在不同条件下表现出不同的偏好和失效模式。

Conclusion: 算法设计、评估和选择必须同时考虑平台能力和场景结构，未来需要开发在多样化平台和场景中保持鲁棒性的方法。
...
TL;DR: 提出了一种用于连续体机器人连续时间状态估计的滑动窗口滤波器，在保持滤波器方法精度的同时实现在线操作，并以超实时速度运行。


<details>
  <summary>Details</summary>
Motivation: 现有连续体机器人状态估计方法难以平衡精度和计算效率，滑动窗口方法限于简化离散时间近似且缺乏随机表示，而随机滤波器方法受限于测量速度，连续时间方法目前仅限于离线操作。

Method: 开发了专门针对连续体机器人的随机滑动窗口滤波器，用于连续时间状态估计，结合了滑动窗口和连续时间方法的优势。

Result: 该方法在保持滤波器方法精度的同时，实现了在线操作和超实时运行速度，是首个专门为连续体机器人设计的随机滑动窗口滤波器。

Conclusion: 该工作为连续体机器人状态估计提供了一个有前景的研究方向，解决了现有方法在精度、效率和在线操作方面的限制。
...
TL;DR: REALMS2是一个基于ROS 2的多机器人系统框架，用于行星勘探和地图构建，采用vSLAM技术生成地图，通过网状网络实现鲁棒通信，在ESA-ESRIC挑战赛中成功绘制了约60%的区域。


<details>
  <summary>Details</summary>
Motivation: 解决多机器人系统在太空勘探中的挑战，特别是应对地外环境的通信延迟和中断问题，为行星资源勘探提供创新解决方案。

Method: 基于ROS 2框架，结合视觉SLAM技术进行地图生成，使用网状网络建立鲁棒的ad hoc网络，通过单一图形用户界面控制所有漫游车。

Result: 在ESA-ESRIC挑战赛的第二次现场测试中，使用三个同构漫游车成功绘制了约60%的区域，并有效处理了通信延迟和中断。

Conclusion: REALMS2系统展示了在异质多机器人勘探任务中的有效性，能够应对地外环境的挑战，为未来太空资源勘探提供了可行的技术方案。
...
Method: 分层框架：高层使用DQN进行离散子目标选择，低层使用TD3执行连续速度控制。设计了包含方向、距离、避障、动作平滑度等要素的奖励函数，并集成了LiDAR安全门机制。

Result: 在ROS+Gazebo平台上评估显示，相比单一算法（DQN或TD3）和基于规则的规划器，该系统在成功率和样本效率方面均有提升，对未见过的障碍物配置具有更好的泛化能力，控制变化更平滑。

Conclusion: 分层DQN-TD3框架有效结合了离散决策和连续控制的优势，在复杂导航任务中表现出色，为机器人路径规划提供了实用的解决方案。
...
TL;DR: 提出三种启发式LFI变体（EDGE、MODE、CENTRE）来解决支持集错误设定问题，通过在推理过程中自适应调整支持集，提高参数推断和策略学习的效果。


<details>
  <summary>Details</summary>
Motivation: 传统LFI方法假设固定的支持集，但错误设定的支持集会导致次优且虚假确定的后验分布，影响机器人学习性能。

Method: 开发三种启发式LFI变体，每种以不同方式解释后验模式在推理步骤中的移动，并在LFI步骤中同时调整支持集和后验推断。

Result: 在随机动态基准测试中验证了支持集错误设定问题，在可变形线性物体操作任务中实现了更精细的长度和刚度分类，提升了策略学习的鲁棒性。

Conclusion: 启发式支持集自适应方法能够改善参数推断质量，当用作基于仿真的策略学习的领域分布时，能产生更鲁棒的以物体为中心的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于扩散的模仿学习方法难以同时实现快速采样和强多模态能力，需要在推理速度和模式覆盖之间做出权衡。

Method: HCP运行一个短随机前缀到自适应切换时间，然后应用一步一致性跳跃生成最终动作。采用时变一致性蒸馏，结合轨迹一致性目标和去噪匹配目标。

Result: 在仿真和真实机器人上，HCP仅需25步SDE加一次跳跃就能接近80步DDPM教师的准确性和模式覆盖度，同时显著降低延迟。

Conclusion: 多模态不需要缓慢的推理，切换时间将模式保持与速度解耦，为机器人策略提供了实用的准确性与效率权衡。
...
TL;DR: 提出了一种在单消费级GPU上实现30Hz帧率和480Hz轨迹频率的多视角VLA实时控制方法，使大型VLA模型能够完成动态实时任务。


<details>
  <summary>Details</summary>
Motivation: 解决大型视觉语言动作模型在实时动态任务中运行速度慢的问题，使VLA能够处理之前被认为无法实现的实时控制任务。

Method: 引入一系列策略消除模型推理中的开销，包括优化推理过程，并提出了完整的流式推理框架用于VLA的实时机器人控制。

Result: 在真实世界实验中，采用该策略的pi0策略在抓取下落笔任务中达到了100%的成功率。

Conclusion: 证明了通过优化策略可以在单消费级GPU上实现VLA的高频实时控制，为动态实时机器人任务提供了可行解决方案。
